...
sudo cat /dev/vdb | tail -c +1M | head -c 1024 | grep -a -q "type bare foo"

# no filesystem partition structure is read directly from the partition device 
# node
sudo cat /dev/vdb1 | grep -q -a "no fs foo"
...
grep -q "file foo DECLARED FS" /tmp/foo2/foo.txt

# this one doesn't have a declared filesystem in the gadget.yaml
grep -q "file foo" /tmp/foo3/foo.txt
