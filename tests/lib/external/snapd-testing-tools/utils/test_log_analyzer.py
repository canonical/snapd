...
spec = spec_from_loader(
    "log-analyzer", SourceFileLoader("log-analyzer", os.path.join(dir_path, "log-analyzer")))
log_analyzer = module_from_spec(spec)
spec.loader.exec_module(log_analyzer)


def create_data(num_executed_no_fail, num_fail_execution, num_fail_restore, num_fail_prepare, num_not_executed):
    # The order will be:
    ...
    exec_param = ["test_" + str(i) for i in range(num_executed_no_fail + num_fail_execution +
                                                  num_fail_prepare + num_fail_restore + num_not_executed)]

    # The tasks that executed are those that didn't fail plus those that failed during execution or restore
    spread_logs = [{'type': 'phase', 'verb': 'Executing', 'task': param}
                   for param in exec_param[:num_executed_no_fail + num_fail_execution + num_fail_restore]]
    ...
    spread_logs.append({'type': 'result',
                        'result_type': 'Failed',
                        'level': 'tasks',
    ...
    end = num_executed_no_fail+num_fail_execution+num_fail_restore+num_fail_prepare
...
class TestLogAnalyzer(unittest.TestCase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.filtered_exec_param_mix_success_abort, self.spread_logs_mix_success_abort = create_data(
            num_executed_no_fail=10,
            num_fail_execution=0,
            num_fail_restore=0,
            num_fail_prepare=0,
    ...
    def test_reexecute_tasks__mixed(self):
        actual = log_analyzer.list_rexecute_tasks(
            self.exec_param_mixed, self.filtered_exec_param_mixed, self.spread_logs_mixed)
    ...
    def test_reexecute_tasks__no_fail(self):
        actual = log_analyzer.list_rexecute_tasks(
            self.exec_param_no_failed, self.filtered_exec_param_no_failed, self.spread_logs_no_failed)
        self.assertEqual(0, len(actual))
    ...
    def test_reexecute_tasks__no_exec(self):
        actual = log_analyzer.list_rexecute_tasks(
            self.exec_param_no_exec, self.filtered_exec_param_no_exec, self.spread_logs_no_exec)
    ...
    def test_list_executed__mix_success_abort(self):
        actual = log_analyzer.list_executed_tasks(
            self.filtered_exec_param_mix_success_abort, self.spread_logs_mix_success_abort)
        expected = set(["test_" + str(i) for i in range(10)])
        self.assertSetEqual(expected, actual)
    ...
    def test_reexecute_tasks__mix_success_abort(self):
        actual = log_analyzer.list_rexecute_tasks(
            self.exec_param_mix_success_abort, self.filtered_exec_param_mix_success_abort, self.spread_logs_mix_success_abort)
        expected = set(["test_" + str(i) for i in range(10, 20)])
        self.assertSetEqual(expected, actual)
