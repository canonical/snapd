...
update_core_snap_with_snap_exec_snapctl() {
    ...
    # core snap. To accomplish that, we'll just bind mount over them. This is
    # much faster than a full repack.
    core="$(readlink -f /snap/core/current || readlink -f /snap/ubuntu-core/current)"
    mount --bind /usr/lib/snapd/snap-exec ${core}/usr/lib/snapd/snap-exec
    mount --bind /usr/bin/snapctl ${core}/usr/bin/snapctl
