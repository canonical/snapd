...
func (s *SquashfsTestSuite) TestInstallViaSquashfsWorks(c *C) {
	...
	c.Assert(osutil.FileExists(filepath.Join(dirs.SnapBlobDir, "hello-app.developer_1.10.snap")), Equals, true)

	// ensure the right unit is created
	mup := systemd.MountUnitPath("/snaps/hello-app.developer/1.10", "mount")
	content, err := ioutil.ReadFile(mup)
	c.Assert(err, IsNil)
	c.Assert(string(content), Matches, "(?ms).*^Where=/snaps/hello-app.developer/1.10")
	c.Assert(string(content), Matches, "(?ms).*^What=/var/lib/snappy/snaps/hello-app.developer_1.10.snap")
}
...
func (s *SquashfsTestSuite) TestRemoveViaSquashfsWorks(c *C) {
	...
	c.Assert(err, IsNil)
	c.Assert(osutil.FileExists(filepath.Join(dirs.SnapBlobDir, "hello-app.developer_1.10.snap")), Equals, false)
