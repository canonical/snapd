...
func addPackageDesktopFiles(s *snap.Info) error {
	...
	for _, df := range desktopFiles {
		...
		installedDesktopFileName := filepath.Join(dirs.SnapDesktopFilesDir, fmt.Sprintf("%s_%s", s.ZName(), filepath.Base(df)))
...
func removePackageDesktopFiles(s *snap.Info) error {
	glob := filepath.Join(dirs.SnapDesktopFilesDir, s.ZName()+"_*.desktop")
