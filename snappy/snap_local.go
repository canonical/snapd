...
// SnapBag is a slice of *Snaps that share their name (+origin)
type SnapBag []*Snap

// Best Snap in the bag (and its index).
//
// If there is an active part, that. Otherwise, the last part in the slice.
//
// (-1, nil) if bag is nil or empty.
func (bag SnapBag) Best() (idx int, part *Snap) {
	idx = -1

	for idx, part = range bag {
		if part.IsActive() {
			return idx, part
		}
	}
...
// Map this SnapBag into a map[string]interface{}, augmenting it with the
// given (purportedly remote) Part.
//
// It is a programming error (->panic) to call Map on a nil/empty SnapBag with
// a nil Part. SnapBag or remotePart may be empty/nil, but not both of them.
//
// Also may panic if the remote part is nil and Best() is nil.
func (bag SnapBag) Map(remotePart Part) map[string]interface{} {
