...
`
	c.Assert(string(content), Matches, "(?ms).*"+regexp.QuoteMeta(fmt.Sprintf(needle, dirs.GlobalRootDir))+".*")
}

var (
	// %[1]s After=.. or Before=...
	// %[2]s Type=...
	// %[3]s architecture name
	// %[4]s dirs.GlobalRootDir
...
[Service]
ExecStart=/usr/bin/ubuntu-core-launcher snap.xkcd-webserver.xkcd-webserver snap.xkcd-webserver.xkcd-webserver %[4]ssnap/xkcd-webserver/44/bin/foo start
Restart=on-failure
...
ExecStop=/usr/bin/ubuntu-core-launcher snap.xkcd-webserver.xkcd-webserver snap.xkcd-webserver.xkcd-webserver %[4]ssnap/xkcd-webserver/44/bin/foo stop
ExecStopPost=/usr/bin/ubuntu-core-launcher snap.xkcd-webserver.xkcd-webserver snap.xkcd-webserver.xkcd-webserver %[4]ssnap/xkcd-webserver/44/bin/foo post-stop
TimeoutStopSec=30
%[2]s
...
`
	expectedServiceAppWrapper = fmt.Sprintf(expectedServiceWrapperFmt,
		"After=snapd.frameworks.target\nRequires=snapd.frameworks.target",
		"Type=simple\n",
		arch.UbuntuArchitecture(),
		dirs.GlobalRootDir,
	)
	expectedServiceFmkWrapper = fmt.Sprintf(expectedServiceWrapperFmt,
		"Before=snapd.frameworks.target\nAfter=snapd.frameworks-pre.target\nRequires=snapd.frameworks-pre.target",
		"Type=dbus\nBusName=foo.bar.baz",
		arch.UbuntuArchitecture(),
		dirs.GlobalRootDir,
	)
	expectedSocketUsingWrapper = fmt.Sprintf(expectedServiceWrapperFmt,
		"After=snapd.frameworks.target snap.xkcd-webserver.xkcd-webserver.socket\nRequires=snapd.frameworks.target snap.xkcd-webserver.xkcd-webserver.socket",
		"Type=simple\n",
		arch.UbuntuArchitecture(),
		dirs.GlobalRootDir,
	)
	expectedTypeForkingFmkWrapper = fmt.Sprintf(expectedServiceWrapperFmt,
		"After=snapd.frameworks.target\nRequires=snapd.frameworks.target",
		"Type=forking\n",
		arch.UbuntuArchitecture(),
		dirs.GlobalRootDir,
