...
func (s *SnapTestSuite) TestFindSnapsByNameWithDeveloper(c *C) {
	...
	parts := FindSnapsByName("hello-app."+testDeveloper, installed)
	c.Assert(parts, HasLen, 1)
	c.Assert(parts[0].Name(), Equals, "hello-app")
}
...
func (s *SnapTestSuite) TestFindSnapsByNameAndVersion(c *C) {
	...
	parts := FindSnapsByNameAndVersion("hello-app."+testDeveloper, "1.10", installed)
	c.Check(parts, HasLen, 1)
	parts = FindSnapsByNameAndVersion("bad-app."+testDeveloper, "1.10", installed)
	c.Check(parts, HasLen, 0)
	parts = FindSnapsByNameAndVersion("hello-app.badDeveloper", "1.10", installed)
	c.Check(parts, HasLen, 0)
	parts = FindSnapsByNameAndVersion("hello-app."+testDeveloper, "2.20", installed)
	c.Check(parts, HasLen, 0)
