...
func (s *BuildSuite) TestAssetsBuildsSnapbuild(c *check.C) {
	...
	cmd := "go build -o acceptance-tests/bin/snapbuild " + snapbuildPkg
	buildCall := s.execCalls[cmd]
...
func (s *BuildSuite) TestAssetsRenamesBuiltBinary(c *check.C) {
	...
	cmd := "tests.test " + testsBinDir + AcceptanceTestName
...
func (s *BuildSuite) TestBuildCmdDoesNotIncludeFilteredPkgs(c *check.C) {
	var items = []struct {
		...
		{"acceptance-tests/pkg1\nanotherpkg/pkg2\ninitialpkg/acceptance-tests/pkg3", "-coverpkg anotherpkg/pkg2"},
