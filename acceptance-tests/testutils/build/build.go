...
const (
	listCmd         = "go list ./..."
	buildTestCmdFmt = "go test%s -c ./acceptance-tests/tests"

	snapbuildPkg = "./acceptance-tests/testutils/build/snapbuild"

	// AcceptanceTestName is the name of the test binary.
	AcceptanceTestName = "acceptance.test"
	defaultGoArm       = "7"
	testsBinDir        = "acceptance-tests/bin/"
	baseBuildCmd       = "go test -c -tags acceptancecoverage "
...
// Assets builds the snappy and acceptance tests binaries for the target
// architecture.
...
func buildTests(arch, testBuildTags string) {
	...
	osRename("tests.test", testsBinDir+AcceptanceTestName)
}
...
func getBinaryBuildCmd(binary, coverpkg string) string {
	...
	// which is under the acceptance-tests directory. The
	// acceptance-tests/test-wrapper script (Test-Command's entry point of
...
func filterPkgs(list string) string {
	...
	filterPattern := `.*acceptance-tests|helper|osutil|progress`
