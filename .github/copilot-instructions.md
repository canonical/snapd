...
- **State managers** implement `StateManager` interface with `StartUp()`, `Ensure()`, and `Stop()` methods
...
`snap-confine` uses capabilities to set up mount namespace, AppArmor profiles, then execs `snap-exec` which runs the actual snap binary.
...
- Unexported symbols are package-scoped, not file-scoped—name accordingly
...
**Test requirements:**
- Test both `do` and `undo` handlers symmetrically
