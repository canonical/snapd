...
func (ts *HTestSuite) TestBasic(c *C) {
	...
	c.Assert(env, DeepEquals, strutil.NewEnvironmentDelta(
...
func (s *HTestSuite) TestSnapRunSnapExecEnv(c *C) {
	...
	for _, withHomeEnv := range []bool{true, false} {
		...
		env := snapEnv(info)
		c.Check(env, DeepEquals, strutil.NewEnvironmentDelta(
...
func (s *HTestSuite) TestExecEnvNoRenameTMPDIRForNonClassic(c *C) {
	...
	c.Assert(err, IsNil)
	c.Assert(env.Get("TMPDIR"), Equals, "/var/tmp")
	c.Assert(env.Contains(PreservedUnsafePrefix+"TMPDIR"), Equals, false)
}
...
func (s *HTestSuite) TestExecEnvRenameTMPDIRForClassic(c *C) {
	...
	c.Assert(err, IsNil)
	c.Assert(env.Contains("TMPDIR"), Equals, false)
	c.Assert(env.Get(PreservedUnsafePrefix+"TMPDIR"), Equals, "/var/tmp")
}
