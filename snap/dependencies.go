...
func getDependenciesFor(plugs []string, slots []string, base string, dependenciesInner map[string][]dependency, forbiddenInterfacesInner []string) ([]string, error) {
	baseVersion := 0
	...
	for _, plug := range plugs {
		...
		for _, dep := range deps {
			...
			if slicesContains(dependenciesList, dep.Name) {
			...
			dependenciesList = append(dependenciesList, dep.Name)
		}
	}
