...
`))
	c.Assert(err, IsNil)
	c.Check(info.ZName(), Equals, "snap")
...
`))
	c.Assert(err, IsNil)
	c.Check(info.ZName(), Equals, "foo")
	c.Check(info.Version, Equals, "1.2")
	c.Check(info.Type, Equals, snap.TypeApp)
	c.Check(info.ZSummary(), Equals, "foo app")
	c.Check(info.ZDescription(), Equals, "Foo provides useful services\n")
