...
func (s *DeltaTestSuite) TestSupportedDeltaFormats(c *C) {
	c.Assert(squashfs.SupportedDeltaFormats(), DeepEquals, []string{"snap-delta-v1-xdelta3", "xdelta3"})
}
...
func (s *DeltaTestSuite) TestApplyDeltaSnapXdelta3VersionMismatch(c *C) {
	...
	c.Assert(err, ErrorMatches, `incompatible major version 2 \(needs to be <= 1\)`)
}
