...
// TODO: the setup here is copied out of wrappers/services_test.go from the
// migration of DeriveSnapdDBusConfig from wrappers to snap, this should be
// cleaned up somehow someway
func (s *infoSuite) TestDeriveSnapdDBusConfig(c *C) {
	...
	info := snaptest.MockSnapWithFiles(c, `name: snapd
version: 1.0
type: snapd`, &snap.SideInfo{Revision: snap.R(1)}, [][]string{
		// system services
	...
	sessionContent, systemContent, err := snap.DeriveSnapdDBusConfig(info)
	c.Assert(err, IsNil)
