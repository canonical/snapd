...
func (s *VerityTestSuite) TestReadVeritySuperblockSuccess(c *C) {
	// testdisk.verity is generated by:
	// - dd if=/dev/zero of=testdisk bs=8M count=1
	// - veritysetup format testdisk testdisk.verity
	sb, err := dmverity.ReadVeritySuperblock("testdata/testdisk.verity")
	c.Check(err, IsNil)
...
func (s *VerityTestSuite) TestReadEmptySuperBlockError(c *C) {
	...
	sb, err := dmverity.ReadVeritySuperblock(testDiskPath)
	c.Assert(err, IsNil)
