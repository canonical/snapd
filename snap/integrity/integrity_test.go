...
func (s *IntegrityTestSuite) testGenerateDmVerityData(c *C, params *generateDmVerityDataParams) {
	...
	restore = integrity.MockOsRename(func(src, dst string) error {
		c.Assert(src, Equals, snapPath+".verity")
		c.Assert(dst, Equals, snapPath+".verity_"+rootHash)
		return nil
	})
	defer restore()
	...
	c.Check(err, IsNil)
	c.Check(hashFileName, Equals, snapPath+".verity_"+rootHash)
...
func (s *IntegrityTestSuite) TestLookupDmVerityDataSuccess(c *C) {
	...
	digest := "test"
	verityFilePath := snapPath + ".verity_" + digest
