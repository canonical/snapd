...
func TestParseUEvent(t *testing.T) {
	...
	for _, sample := range samples {
		...
		if err != nil || uevent == nil {
			if !sample.Valid { // Fake uevent: test passed
				continue
			}
			t.Fatalf("Unable to parse real uevent")
		}

		if uevent.Action != sample.UEvent.Action {
			t.Fatalf("Wrong action (got: %s, wanted: %s)", uevent.Action.String(), ADD.String())
		}

		if uevent.KObj != sample.UEvent.KObj {
			t.Fatalf("Wrong action (got: %s, wanted: %s)", uevent.KObj, sample.UEvent.KObj)
		}
		...
		for k, v := range sample.UEvent.Env {
			...
			if !found {
				t.Fatalf("Unable to find %s=%s env var from uevent", k, v)
			}
		}
	}
}
