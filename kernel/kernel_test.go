...
func (s *kernelYamlTestSuite) TestReadKernelYamlSad(c *C) {
	...
	err := os.MkdirAll(filepath.Dir(kernelYamlPath), 0o755)
	c.Assert(err, IsNil)
	err = os.WriteFile(kernelYamlPath, []byte(`invalid-kernel-yaml`), 0o644)
	c.Assert(err, IsNil)
...
func (s *kernelYamlTestSuite) TestReadKernelYamlHappy(c *C) {
	...
	err := os.MkdirAll(filepath.Dir(kernelYamlPath), 0o755)
	c.Assert(err, IsNil)
	err = os.WriteFile(kernelYamlPath, mockKernelYaml, 0o644)
	c.Assert(err, IsNil)
