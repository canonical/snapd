...
#include "snap-wrap-args.h"
#include "snap-wrap-args.c"
...
{
	// Test that typical invocation of snap-wrap is parsed correctly.
	...
	test_argc_argv(&argc, &argv,
		       "/usr/lib/snapd/snap-wrap", "snap.SNAP_NAME.APP_NAME",
	...
	g_assert_cmpint(argc, ==, 3);
	g_assert_cmpstr(argv[0], ==, "/usr/lib/snapd/snap-wrap");
...
static void test_sc_cleanup_args()
{
	...
	int argc;
	char **argv;
	test_argc_argv(&argc, &argv, "/usr/lib/snapd/snap-wrap",
...
static void test_sc_nonfatal_parse_args__typical_classic()
{
	...
	char **argv;
	test_argc_argv(&argc, &argv,
		       "/usr/lib/snapd/snap-wrap", "--classic",
		       "snap.SNAP_NAME.APP_NAME", "/usr/lib/snapd/snap-exec",
		       "--option", "arg", NULL);
...
{
	// Test that typical legacy invocation of snap-wrap via the
...
{
	// Test that snap-wrap --version is detected.
	struct sc_error *err __attribute__ ((cleanup(sc_cleanup_error))) = NULL;
	struct sc_args *args __attribute__ ((cleanup(sc_cleanup_args))) = NULL;
	...
	char **argv;
	test_argc_argv(&argc, &argv,
		       "/usr/lib/snapd/snap-wrap", "--version", "ignored",
		       "garbage", NULL);
...
static void test_sc_nonfatal_parse_args__no_security_tag()
{
	...
	int argc;
	char **argv;
	test_argc_argv(&argc, &argv, "/usr/lib/snapd/snap-wrap", NULL);
