...
#include "../libsnap-wrap-private/classic.h"
#include "../libsnap-wrap-private/cleanup-funcs.h"
#include "../libsnap-wrap-private/secure-getenv.h"
#include "../libsnap-wrap-private/snap.h"
...
int main(int argc, char **argv)
{
	...
	if (!apparmor.is_confined && apparmor.mode != SC_AA_NOT_APPLICABLE
		...
		// users trying to leverage the snap-wrap in the core snap to
		// escalate privileges.
		die("snap-wrap has elevated permissions and is not confined"
	...
	if (geteuid() == 0) {
		if (classic_confinement) {
			 ...
			 * - snap-wrap skips using the snap-specific mount namespace
			 * - snap-wrap skips using device cgroups
			 * - snapd sets up a lenient AppArmor profile for snap-wrap to use
			 * - snapd sets up a lenient seccomp profile for snap-wrap to use
			 */
			debug
			    ("skipping sandbox setup, classic confinement in use");
		} else {
			/* snap-wrap uses privately-shared /run/snapd/ns to store
			 * bind-mounted mount namespaces of each snap. In the case that
			 * snap-wrap is invoked from the mount namespace it typically
			 ...
			 * outer namespace.
			 *
			 * In order to operate in such an environment snap-wrap must
