...
build_debian_or_ubuntu_package() {
    ...
    cp "$top_dir/snap-wrap-$pkg_version.tar.gz" "snap-wrap_$pkg_version.orig.tar.gz"
    tar -zxf "snap-wrap_$pkg_version.orig.tar.gz" 

    # Apply the debian directory from downstream packaging to form a complete source package
    mv "distro-packaging/debian" "snap-wrap-$pkg_version/debian"
    ...
    ( cd "snap-wrap-$pkg_version" && dch --controlmaint --newversion "${pkg_version}-1" "Automatic CI build")

    # Build an unsigned source package
    ( cd "snap-wrap-$pkg_version" && dpkg-buildpackage -uc -us -S )
...
case "$release_ID" in
    ubuntu|debian)
        ...
        dpkg -i snap-wrap_*.deb || apt-get -f install -y
