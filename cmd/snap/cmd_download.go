...
func init() {
	addCommand("download", shortDownloadHelp, longDownloadHelp, func() flags.Commander {
		...
		"revision": i18n.G("Download the given revision of a snap. When downloading components, download the component associated with the given snap revision."),
		// TRANSLATORS: This should not start with a lowercase letter.
		...
		name: "<snap+component...>",
		// TRANSLATORS: This should not start with a lowercase letter.
		desc: i18n.G("Snap and component names"),
	}})
}
...
func downloadDirect(snapName string, components []string, opts tooling.DownloadSnapOptions) error {
	...
	if opts.OnlyComponents {
		if len(compRefs) == 1 {
			...
			fmt.Fprintf(Stdout, i18n.G("Fetching components %q\n"), snapName, strutil.Quoted(compRefs))
		}
	} else {
		...
		default:
			fmt.Fprintf(Stdout, i18n.G("Fetching snap %q and components %q\n"), snapName, strutil.Quoted(compRefs))
		}
	}
	...
	assertsPath, err := downloadAssertions(dl.Info, dl.Path, compInfos, tsto, opts)
	if err != nil {
		return err
	}
