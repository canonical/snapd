...
func (s *SnapSuite) TestList(c *check.C) {
	...
	s.RedirectClientToTestServer(func(w http.ResponseWriter, r *http.Request) {
		...
		case 0:
			...
			fmt.Fprintln(w, `{"type": "sync", "result": [{"name": "foo", "status": "active", "version": "4.2", "publisher": {"id": "bar-id", "username": "bar", "display-name": "Bar"}, "revision":17, "tracking-channel": "potatoes"}]}`)
		default:
...
func (s *SnapSuite) TestListAll(c *check.C) {
	...
	s.RedirectClientToTestServer(func(w http.ResponseWriter, r *http.Request) {
		...
		case 0:
			...
			fmt.Fprintln(w, `{"type": "sync", "result": [{"name": "foo", "status": "active", "version": "4.2", "publisher": {"id": "bar-id", "username": "bar", "display-name": "Bar"}, "revision":17, "tracking-channel": "stable"}]}`)
		default:
			c.Fatalf("expected to get 1 requests, now on %d", n+1)
		}
...
func (s *SnapSuite) TestListWithQuery(c *check.C) {
	...
	s.RedirectClientToTestServer(func(w http.ResponseWriter, r *http.Request) {
		...
		case 0:
			...
			fmt.Fprintln(w, `{"type": "sync", "result": [{"name": "foo", "status": "active", "version": "4.2", "publisher": {"id": "bar-id", "username": "bar", "display-name": "Bar"}, "revision":17, "tracking-channel": "1.10/stable/fix1234"}]}`)
		default:
			c.Fatalf("expected to get 1 requests, now on %d", n+1)
		}
...
func (s *SnapSuite) TestListWithNotes(c *check.C) {
	...
	s.RedirectClientToTestServer(func(w http.ResponseWriter, r *http.Request) {
		...
		case 0:
			...
			c.Check(r.URL.Path, check.Equals, "/v2/snaps")
			fmt.Fprintln(w, `{"type": "sync", "result": [
{"name": "foo", "status": "active", "version": "4.2", "publisher": {"id": "bar-id", "username": "bar", "display-name": "Bar"}, "revision":17, "trymode": true}
