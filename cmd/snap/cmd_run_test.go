...
func (s *SnapSuite) TestInvalidParameters(c *check.C) {
	invalidParameters := []string{"run", "--hook=config-changing", "--command=command-name", "snap-name"}
	...
	invalidParameters = []string{"run", "--hook=config-changing", "foo", "bar", "snap-name"}
	_, err = snaprun.Parser().ParseArgs(invalidParameters)
	c.Check(err, check.ErrorMatches, ".*too many arguments for hook \"config-changing\": bar.*")
}
...
func (s *SnapSuite) TestSnapRunHookIntegration(c *check.C) {
	...
	_, err := snaprun.Parser().ParseArgs([]string{"run", "--hook=config-changing", "snapname"})
	c.Assert(err, check.IsNil)
	c.Check(execArg0, check.Equals, "/usr/bin/ubuntu-core-launcher")
	c.Check(execArgs, check.DeepEquals, []string{
		"/usr/bin/ubuntu-core-launcher",
		"snap.snapname.hook.config-changing",
		"snap.snapname.hook.config-changing",
		"/usr/lib/snapd/snap-exec",
		"--hook=config-changing", "snapname"})
	c.Check(execEnv, testutil.Contains, "SNAP_REVISION=42")
}
...
func (s *SnapSuite) TestSnapRunHookUnsetRevisionIntegration(c *check.C) {
	...
	_, err := snaprun.Parser().ParseArgs([]string{"run", "--hook=config-changing", "-r=unset", "snapname"})
	c.Assert(err, check.IsNil)
	c.Check(execArg0, check.Equals, "/usr/bin/ubuntu-core-launcher")
	c.Check(execArgs, check.DeepEquals, []string{
		"/usr/bin/ubuntu-core-launcher",
...
func (s *SnapSuite) TestSnapRunHookSpecificRevisionIntegration(c *check.C) {
	...
	_, err := snaprun.Parser().ParseArgs([]string{"run", "--hook=config-changing", "-r=41", "snapname"})
	c.Assert(err, check.IsNil)
	c.Check(execArg0, check.Equals, "/usr/bin/ubuntu-core-launcher")
	c.Check(execArgs, check.DeepEquals, []string{
		"/usr/bin/ubuntu-core-launcher",
...
func (s *SnapSuite) TestSnapRunHookInvalidRevisionIntegration(c *check.C) {
	_, err := snaprun.Parser().ParseArgs([]string{"run", "--hook=config-changing", "-r=invalid", "snapname"})
	c.Assert(err, check.NotNil)
