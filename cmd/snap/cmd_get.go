...
type cmdGet struct {
	Positional struct {
		...
		Field string `positional-arg-name:"<field>" description:"field of interest within the configuration"`
...
func (x *cmdGet) Execute(args []string) error {
	...
	return getConfig(x.Positional.Snap, x.Positional.Field)
}

func getConfig(snapName, configField string) error {
	cli := Client()
	config, err := cli.GetConfig(snapName, configField)
	if err != nil {
		return err
	}
