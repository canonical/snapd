...
func maybeCheckSystemKeyMismatch(cli *client.Client) (changeID string, err error) {
	...
	for i := 0; i < retryCount; i++ {
		...
		// an error, which could be antything from socket being down, to
		...
		if !restarting && errors.Is(err, client.ErrMismatchAdviceUnsupported) {
			...
			// to date, but since we're misisng an API there is now way for us
			// to tell snapd to do the update
