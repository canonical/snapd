...
func main() {
	...
	if osutil.IsSymlink(filepath.Join(dirs.SnapBinariesDir, snapApp)) {
		...
		if snapPath != "" && desktopFile != "" {
			...
			if err != nil {
				fmt.Fprintf(Stderr, i18n.G("unable to access privileged desktop launcher: unable unable to get session bus: %v\n"), err)
				os.Exit(46)
			}
			...
			if call.Err != nil {
				fmt.Fprintf(Stderr, i18n.G("failed to launch %s via the privileged desktop launcher: %v\n"), desktopFile, call.Err)
				os.Exit(46)
			}
