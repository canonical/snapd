...
func (s *SnapSuite) TestConnectivityHappy(c *check.C) {
	...
	rest := mylog.Check2(snap.Parser(snap.Client()).ParseArgs([]string{"debug", "connectivity"}))
...
func (s *SnapSuite) TestConnectivityUnhappy(c *check.C) {
	...
	_ := mylog.Check2(snap.Parser(snap.Client()).ParseArgs([]string{"debug", "connectivity"}))
