...
func (s *SnapSuite) TestBuyFreeSnapFails(c *check.C) {
	...
	c.Assert(err, check.NotNil)
	c.Check(err.Error(), check.Equals, "cannot buy snap \"hello\": it is free")
...
func (s *SnapSuite) TestBuySnap(c *check.C) {
	...
	c.Check(s.Stdout(), check.Equals, "Do you want to buy \"hello\" from \"canonical\" for 2.99GBP? (Y/n): Buy state: Complete\n")
	c.Check(s.Stderr(), check.Equals, "")
	c.Check(getCount, check.Equals, 1)
