...
func (s *SnapSuite) TestDebugChanges(c *C) {
	...
	c.Assert(os.WriteFile(stateFile, stateJSON, 0o644), IsNil)
...
func (s *SnapSuite) TestDebugTasksWithCycles(c *C) {
	dir := c.MkDir()
	stateFile := filepath.Join(dir, "test-state.json")
	c.Assert(os.WriteFile(stateFile, stateCyclesJSON, 0o644), IsNil)
...
func (s *SnapSuite) TestDebugIsSeededNo(c *C) {
	dir := c.MkDir()
	stateFile := filepath.Join(dir, "test-state.json")
	c.Assert(os.WriteFile(stateFile, []byte("{}"), 0o644), IsNil)
...
func (s *SnapSuite) TestDebugConnections(c *C) {
	dir := c.MkDir()
	stateFile := filepath.Join(dir, "test-state.json")
	c.Assert(os.WriteFile(stateFile, stateConnsJSON, 0o644), IsNil)
