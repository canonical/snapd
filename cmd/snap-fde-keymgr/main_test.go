...
func (s *mainSuite) TestAddKey(c *C) {
	...
	defer restore()
	c.Assert(os.WriteFile(filepath.Join(d, "authz.key"), []byte{1, 1, 1}, 0o644), IsNil)
	err := main.Run([]string{
		"add-recovery-key",
		"--devices", "/dev/vda4",
...
func (s *mainSuite) testAddKeyIdempotent(c *C, tc addKeyTestCase) {
	...
	c.Assert(os.WriteFile(filepath.Join(d, "recovery.key"), rkey[:], 0o600), IsNil)
...
func (s *mainSuite) TestRemoveKey(c *C) {
	...
	c.Assert(os.WriteFile(filepath.Join(d, "recovery.key"), []byte{0, 0, 0}, 0o644), IsNil)
