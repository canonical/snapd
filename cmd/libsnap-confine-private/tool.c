...
/**
 * sc_open_snapd_tool returns a file descriptor of the given internal
 *executable.
 *
 * The executable is located based on the location of the currently executing
 *process. The returning file descriptor can be used with fexecve function, like
 *in sc_call_snapd_tool.
 **/
 ...
 *
 * The idea with calling with an open file descriptor is to allow calling
 *executables across mount namespaces, where the executable may not be visible
 *in the new filesystem anymore. The caller establishes an open file descriptor
 *in one namespace and later on performs the call in another mount namespace.
 *
 * The environment vector has special support for expanding the string
 *"SNAPD_DEBUG=x". If such string is present, the "x" is replaced with either
 *"0" or "1" depending on the result of is_sc_debug_enabled().
 **/
...
void sc_call_snap_update_ns(int snap_update_ns_fd, const char *snap_name, struct sc_apparmor *apparmor) {
    ...
    char *argv[] = {"snap-update-ns",
                    /* This tells snap-update-ns we are calling from
                       snap-confine and locking is in place */
...
void sc_call_snap_update_ns_as_user(int snap_update_ns_fd, const char *snap_name, struct sc_apparmor *apparmor) {
    ...
    char *argv[] = {"snap-update-ns",
                    /* This tells snap-update-ns we are calling from
                       snap-confine and locking is in place */
                    "--from-snap-confine",
                    /* This tells snap-update-ns that we want to process the
                       per-user profile */
