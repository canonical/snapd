				     ...
				     const gchar * subdirname)
{
	...
	sc_ownership ownership = {.uid = (uid_t) - 1,.gid = (gid_t) - 1 };
	int err = sc_nonfatal_mkpath(dirname, 0755, ownership);
	g_assert_cmpint(err, ==, 0);
	...
	err = sc_nonfatal_mkpath(dirname, 0755, ownership);
	g_assert_cmpint(err, ==, 0);
	...
	err = sc_nonfatal_mkpath(subdirname, 0755, ownership);
	g_assert_cmpint(err, ==, 0);
	g_assert_cmpint(errno, ==, 0);
}
