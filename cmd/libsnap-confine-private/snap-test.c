...
static void test_sc_security_tag_validate(void) {
    ...
    g_assert_true(sc_security_tag_validate("snap.network-manager.NetworkManager", "network-manager"));
    ...
    g_assert_true(sc_security_tag_validate("snap.foo_instance.bar-baz", "foo_instance"));
    g_assert_true(sc_security_tag_validate("snap.foo_instance.hook.bar-baz", "foo_instance"));
    g_assert_true(sc_security_tag_validate("snap.foo_bar.hook.bar-baz", "foo_bar"));

    // Now, test the names we know are bad
    g_assert_false(sc_security_tag_validate("pkg-foo.bar.0binary-bar+baz", "bar"));
    ...
    g_assert_false(sc_security_tag_validate("snap.foo_toolonginstance.bar-baz", "foo"));
    g_assert_false(sc_security_tag_validate("snap.foo_inst@nace.bar-baz", "foo"));
    g_assert_false(sc_security_tag_validate("snap.foo_in-stan-ce.bar-baz", "foo"));
    ...
    g_assert_true(sc_security_tag_validate("snap.123test.hook.configure", "123test"));

    // regression test snap.eon-edg-shb-pulseaudio.hook.connect-plug-i2c
    g_assert_true(sc_security_tag_validate("snap.foo.hook.connect-plug-i2c", "foo"));
...
static void test_sc_snap_drop_instance_key_short_dest_max(void) {
    if (g_test_subprocess()) {
        ...
        /* 40 chars (max valid length), pretend dest is the same length, no
