...
static void test_parse_mountinfo_entry__sysfs(void) {
    const char *line =
        "19 25 0:18 / /sys rw,nosuid,nodev,noexec,relatime shared:7 - sysfs "
        "sysfs rw";
...
static void test_parse_mountinfo_entry__snapd_ns(void) {
    const char *line =
        "104 23 0:19 /snapd/ns /run/snapd/ns rw,nosuid,noexec,relatime - tmpfs "
        "tmpfs rw,size=99840k,mode=755";
    sc_mountinfo_entry *entry = sc_parse_mountinfo_entry(line);
    g_assert_nonnull(entry);
    g_test_queue_destroy((GDestroyNotify)sc_free_mountinfo_entry, entry);
...
static void test_parse_mountinfo_entry__snapd_mnt(void) {
    const char *line =
        "256 104 0:3 mnt:[4026532509] /run/snapd/ns/hello-world.mnt rw - nsfs "
        "nsfs rw";
    sc_mountinfo_entry *entry = sc_parse_mountinfo_entry(line);
    g_assert_nonnull(entry);
    g_test_queue_destroy((GDestroyNotify)sc_free_mountinfo_entry, entry);
...
static void test_parse_mountinfo_entry__one_tag(void) {
    const char *line =
        "1 2 3:4 root mount-dir mount-opts tag:1 - fs-type mount-source "
        "super-opts";
    sc_mountinfo_entry *entry = sc_parse_mountinfo_entry(line);
    g_assert_nonnull(entry);
    g_test_queue_destroy((GDestroyNotify)sc_free_mountinfo_entry, entry);
...
static void test_parse_mountinfo_entry__many_tags(void) {
    const char *line =
        "1 2 3:4 root mount-dir mount-opts tag:1 tag:2 tag:3 tag:4 - fs-type "
        "mount-source super-opts";
    sc_mountinfo_entry *entry = sc_parse_mountinfo_entry(line);
    g_assert_nonnull(entry);
    g_test_queue_destroy((GDestroyNotify)sc_free_mountinfo_entry, entry);
    g_assert_cmpint(entry->mount_id, ==, 1);
...
static void test_parse_mountinfo_entry__empty_source(void) {
    const char *line =
        "304 301 0:45 / /snap/test-snapd-content-advanced-plug/x1 rw,relatime "
        "- tmpfs  rw";
    sc_mountinfo_entry *entry = sc_parse_mountinfo_entry(line);
    g_assert_nonnull(entry);
    g_test_queue_destroy((GDestroyNotify)sc_free_mountinfo_entry, entry);
...
static void test_parse_mountinfo_entry__broken_octal_escaping(void) {
    ...
    const char *line =
        "2074 27 0:54 / /tmp/strange-dir rw,relatime shared:1039 - tmpfs "
        "no\\888thing rw\\";
...
static void test_parse_mountinfo_entry__unescaped_whitespace(void) {
    ...
    const char *line =
        "2074 27 0:54 / /tmp/strange\rdir rw,relatime shared:1039 - tmpfs "
        "tmpfs rw";
    struct sc_mountinfo_entry *entry = sc_parse_mountinfo_entry(line);
    g_assert_nonnull(entry);
    g_test_queue_destroy((GDestroyNotify)sc_free_mountinfo_entry, entry);
...
static void test_parse_mountinfo_entry__broken_9p_superblock(void) {
    ...
    const char *line =
        "1146 77 0:149 / /Docker/host rw,noatime - 9p drvfs "
        "rw,dirsync,aname=drvfs;path=C:\\Program "
        "Files\\Docker\\Docker\\resources;symlinkroot=/mnt/"
        ",mmap,access=client,msize=262144,trans=virtio";
    struct sc_mountinfo_entry *entry = sc_parse_mountinfo_entry(line);
    g_assert_nonnull(entry);
    g_test_queue_destroy((GDestroyNotify)sc_free_mountinfo_entry, entry);
