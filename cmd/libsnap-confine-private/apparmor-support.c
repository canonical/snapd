...
void sc_init_apparmor_support(struct sc_apparmor *apparmor) {
    ...
    if (aa_is_enabled() != true) {
        switch (errno) {
            ...
            case ECANCELED:
                debug(
                    "apparmor is available on the system but has been disabled "
                    "at boot");
                break;
            case EPERM:
                // NOTE: fall-through
            case EACCES:
                debug(
                    "insufficient permissions to determine if apparmor is "
                    "enabled");
    ...
    if (aa_change_onexec(profile) < 0) {
        ...
        if (secure_getenv("SNAPPY_LAUNCHER_INSIDE_TESTS") == NULL) {
            ...
            if (errno == ENOENT) {
                fprintf(stderr,
                        "missing profile %s.\n"
                        "Please make sure that the snapd.apparmor service is "
                        "enabled and started\n",
                        profile);
                exit(1);
            } else {
