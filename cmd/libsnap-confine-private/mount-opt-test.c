...
static void test_sc_mount_cmd(void) {
    ...
    const char *expected =
        "mount -t fstype "
        "--rbind --move --make-rshared --make-rslave --make-rprivate "
        "--make-runbindable "
        "-o "
        "ro,nosuid,nodev,noexec,sync,remount,mand,dirsync,noatime,nodiratime,"
        "silent,"
...
static void test_sc_do_mount(gconstpointer snap_debug) {
    ...
    if (GPOINTER_TO_INT(snap_debug) == 0) {
        g_test_trap_assert_stderr(
            "cannot perform operation: mount -t ext4 -o ro /foo /bar: "
            "Permission denied\n");
    } else {
        /* with snap_debug the debug output hides the actual mount commands
         * *but* they are still shown if there was an error
         */
        g_test_trap_assert_stderr(
            "DEBUG: performing operation: (disabled) use debug build to see "
            "details\n"
            "cannot perform operation: mount -t ext4 -o ro /foo /bar: "
            "Permission denied\n");
    }
...
static void test_sc_do_umount(gconstpointer snap_debug) {
    ...
    if (GPOINTER_TO_INT(snap_debug) == 0) {
        g_test_trap_assert_stderr(
            "cannot perform operation: umount --lazy /foo: Permission "
            "denied\n");
    } else {
        /* with snap_debug the debug output hides the actual mount commands
         * *but* they are still shown if there was an error
