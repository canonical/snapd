...
int main(int argc, char **argv) {
    ...
    if (sc_cgroup_is_v2()) {
        /* When we are using cgroup v2 then we don't need to use SETUID and
         * SETGID capabilities and we can drop them from the permitted set.
         * When we are using cgroup v1 then we actually need them to manipulate
         * the freezer, so set them in the effective set. We drop the
         * capability from effective and inheritable as well as permitted
         * because we may end up running as root that comes with all the
         * capabilities from get go, and thus we may get effective=permitted on
         * startup. For unprivileged users we only need to drop from permitted
         * but it's no harm to drop them unconditionally.
         */
...
static void enter_non_classic_execution_environment(sc_invocation *inv, struct sc_apparmor *aa, uid_t real_uid,
    ...
    if (!sc_cgroup_is_v2()) {
        ...
        // We are the child so let's move our parent to the freezer.
        //
        ...
        if ((ruid != 0 || euid != 0 || suid != 0) || (rgid != 0 || egid != 0 || sgid != 0)) {
            ...
            cap_t caps_setuid SC_CLEANUP(sc_cleanup_cap_t) = cap_get_proc();
            if (caps_setuid == NULL) {
                die("cannot obtain current caps");
            }
            if (cap_set_flag(caps_setuid, CAP_EFFECTIVE, SC_ARRAY_SIZE(snap_confine_caps_extra_cgroup_v1),
                             snap_confine_caps_extra_cgroup_v1, CAP_SET) != 0) {
                die("cannot fill effective capability set (setuid)");
            }
            if (cap_set_proc(caps_setuid) != 0) {
                die("cannot set privileged capabilities");
            }
