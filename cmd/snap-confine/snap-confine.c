...
static void sc_restore_process_state(const sc_preserved_process_state *proc_state) {
    ...
    if (fstat(inner_cwd_fd, &file_info_inner) < 0) {
        die("cannot stat path of working directory in the execution "
            "environment");
    }
...
int main(int argc, char **argv) {
    ...
    // snap-confine needs to run as root for cgroup/udev/mount/apparmor/etc
    // setup.
    if (effective_uid != 0) {
    ...
    // Do no get snap context value if running a hook (we don't want to
    // overwrite hook's SNAP_COOKIE)
    ...
    if (!apparmor.is_confined && apparmor.mode != SC_AA_NOT_APPLICABLE && getuid() != 0 && geteuid() == 0) {
        ...
        die("snap-confine has elevated permissions and is not confined"
            ...
            "Please make sure that the snapd.apparmor service is enabled and "
            "started.");
    }
    ...
    if (!invocation.classic_confinement || sc_feature_enabled(SC_FEATURE_PARALLEL_INSTANCES)) {
         ...
         * constructs, the said directory does not contain mount entries for
         * preserved namespaces as those are only visible in the main, outer
         * namespace.
         *
...
static void enter_classic_execution_environment(const sc_invocation *inv, gid_t real_gid, gid_t saved_gid) {
     ...
     *   - recursively bind mount SNAP_MOUNT_DIR/<snap>_<key> on top of
     * SNAP_MOUNT_DIR/<snap>
     *   - recursively bind mount /var/snap/<snap>_<key> on top of
     * /var/snap/<snap>
     *
    ...
    if (unshare(CLONE_NEWNS) < 0) {
        die("cannot unshare the mount namespace for parallel installed classic "
            "snap");
    }
    ...
    if (!sc_streq(inv->snap_instance, inv->snap_name)) {
        debug(
            "(experimental) setting up environment for classic snap instance "
            "%s",
            inv->snap_instance);
