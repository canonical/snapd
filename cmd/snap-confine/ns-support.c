...
void sc_initialize_mount_ns(unsigned int experimental_features) {
    ...
    if (experimental_features & SC_FEATURE_PARALLEL_INSTANCES) {
        ...
        debug(
            "(experimental) ensuring snap mount and data directories are mount "
            "points");
...
static int sc_inspect_and_maybe_discard_stale_ns(int mnt_fd, const sc_invocation *inv, int snap_discard_ns_fd) {
    ...
    if (child == 0) {
        ...
        if (prctl(PR_SET_PDEATHSIG, SIGINT, 0, 0, 0) < 0) {
            die("cannot set parent process death notification signal to "
                "SIGINT");
        }
        ...
        // Send this back to the parent: 3 - force discard 2 - prefer discard, 1
        // - keep. Note that we cannot just use 0 and 1 because of the semantics
        // of eventfd(2).
    ...
    if (waitpid(child, &status, 0) < 0) {
        die("cannot wait for the support process for mount namespace "
            "inspection");
    }
    ...
    switch (value) {
        ...
        case SC_DISCARD_SHOULD:
            if (sc_is_mount_ns_in_use(inv->snap_instance)) {
                // Some processes are still using the namespace so we cannot
                // discard it as that would fracture the view that the set of
                // processes inside have on what is mounted.
        ...
        case SC_DISCARD_MUST:
            debug(
                "preserved mount namespace is stale and base snap has changed, "
                "discarding");
            break;
    }
...
static void helper_fork(struct sc_mount_ns *group, struct sc_apparmor *apparmor) {
    ...
    if (pid == 0) {
        ...
        // Glibc defines pid as a signed 32bit integer. There's no standard way
        // to print pid's portably so this is the best we can do.
...
static void helper_main(struct sc_mount_ns *group, struct sc_apparmor *apparmor, pid_t parent) {
    ...
    if (kill(parent, 0) < 0) {
        switch (errno) {
            case ESRCH:
                // When snap-confine executes it will fork a helper process.
                // That process establishes an elaborate dance to ensure both
                // itself and the parent are operating exactly as specified, so
                // that no processes are left behind for unbound amount of time.
                // As a part of that dance the child pings the parent to ensure
                // it is still alive after establishing a notification signal to
                // be sent in case the parent dies. This is a race avoidance
                // mechanism, we set up the notification and then check if the
                // parent is alive by the time we are done.
                //
                // In the case when the parent does go away we used to call
                // abort(). On some distributions this would trigger an unclean
                // process termination error report to be sent. One such example
                // is the Ubuntu error tracker. Since the parent process can be
                // legitimately interrupted and killed, this should not generate
                // an error report. As such, perform clean exit in this specific
                // case.
                debug("parent process has terminated");
                exit(0);
            default:
