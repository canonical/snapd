...
func (s *baseInitramfsMountsSuite) setupSeedWithIntegrityData(c *C, modelAssertTime time.Time, integrityData []asserts.SnapIntegrityData) {
	// pretend /run/mnt/ubuntu-seed has a valid seed
	s.seedDir = boot.InitramfsUbuntuSeedDir
...
func (s *baseInitramfsMountsSuite) setupRunDirWithIntegrityData(c *C, integrityData []asserts.SnapIntegrityData) {
	_ = s.setupRunDir(c, integrityData)
}

func (s *baseInitramfsMountsSuite) setupRunDir(c *C, integrityData []asserts.SnapIntegrityData) *seedtest.TestingSeed20 {
...
func (s *initramfsMountsSuite) testRecoverMode(c *C, expErr error) {
