...
func (s *initramfsMountsSuite) SetUpTest(c *C) {
	...
	s.seedDir = boot.EarlyUbuntuSeed
...
func (s *initramfsMountsSuite) TestInitramfsMountsInstallModeStep1(c *C) {
	...
	restore := main.MockOsutilIsMounted(func(path string) (bool, error) {
		...
		switch n {
		case 1:
			c.Check(path, Equals, boot.EarlyUbuntuSeed)
			return false, nil
		}
...
func (s *initramfsMountsSuite) TestInitramfsMountsInstallModeStep2(c *C) {
	...
	restore := main.MockOsutilIsMounted(func(path string) (bool, error) {
		...
		case 1:
			...
			return true, nil
		case 2:
			c.Check(path, Equals, filepath.Join(boot.EarlyRunMnt, "base"))
			return false, nil
		case 3:
			c.Check(path, Equals, filepath.Join(boot.EarlyRunMnt, "kernel"))
			return false, nil
		case 4:
			c.Check(path, Equals, filepath.Join(boot.EarlyRunMnt, "snapd"))
			return false, nil
		case 5:
			c.Check(path, Equals, boot.EarlyUbuntuData)
			return false, nil
		}
		return false, fmt.Errorf("unexpected number of calls: %v", n)
	})
...
func (s *initramfsMountsSuite) TestInitramfsMountsInstallModeStep4(c *C) {
	...
	modeEnv := dirs.SnapModeenvFileUnder(boot.EarlyWritable)
	c.Check(modeEnv, testutil.FileEquals, `mode=install
recovery_system=20191118
`)
	cloudInitDisable := filepath.Join(boot.EarlyWritable, "etc/cloud/cloud-init.disabled")
...
func (s *initramfsMountsSuite) TestInitramfsMountsRunModeStep1(c *C) {
	...
	restore := main.MockOsutilIsMounted(func(path string) (bool, error) {
		...
		case 1:
			...
			return false, nil
		case 2:
			c.Check(path, Equals, boot.EarlyUbuntuBoot)
			return false, nil
		case 3:
			c.Check(path, Equals, boot.EarlyUbuntuData)
			return false, nil
...
func (s *initramfsMountsSuite) TestInitramfsMountsRunModeStep2(c *C) {
	...
	err := modeEnv.WriteTo(boot.EarlyWritable)
	c.Assert(err, IsNil)
...
func (s *initramfsMountsSuite) TestInitramfsMountsRunModeBaseSnapUpgradeFailsHappy(c *C) {
	...
	tryBaseSnap := filepath.Join(boot.EarlyWritable, dirs.SnapBlobDir, "core20_124.snap")
...
`, boot.EarlyRunMnt))
	...
	newModeenv, err := boot.ReadModeenv(boot.EarlyWritable)
	c.Assert(err, IsNil)
...
func (s *initramfsMountsSuite) TestInitramfsMountsRunModeBaseSnapUpgradeHappy(c *C) {
	...
	tryBaseSnap := filepath.Join(dirs.SnapBlobDirUnder(boot.EarlyWritable), "core20_124.snap")
	err = os.MkdirAll(filepath.Dir(tryBaseSnap), 0755)
	c.Assert(err, IsNil)
	err = ioutil.WriteFile(tryBaseSnap, []byte{0}, 0644)
	c.Assert(err, IsNil)
...
func (s *initramfsMountsSuite) TestInitramfsMountsRunModeKernelSnapUpgradeHappy(c *C) {
	...
	restore := main.MockOsutilIsMounted(func(path string) (bool, error) {
		...
		case 2:
			...
			return true, nil
		case 3:
			c.Check(path, Equals, filepath.Join(boot.EarlyUbuntuData))
			return true, nil
		case 4:
			c.Check(path, Equals, filepath.Join(boot.EarlyRunMnt, "base"))
			return true, nil
