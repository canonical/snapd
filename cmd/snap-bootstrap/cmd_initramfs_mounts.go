...
var (
	...
	installCreateSaveBootstrappedContainer = install.CreateSaveBootstrappedContainer
)
...
func doSetupRunSystem(mst *initramfsMountsState, model *asserts.Model, factoryReset bool, sysSnaps map[snap.Type]*seed.Snap) error {
	...
	var encryptionSupport device.EncryptionType
	if factoryReset {
	...
	logger.Noticef("OK: Setting up file system for new system...\n")
	if factoryReset {
		installedSystem, err = gadgetInstall.FactoryReset(model, gadgetMountDir, kernelBootInfo.KSnapInfo, bootDevice, options, installObserver, timings.New(nil))
	} else {
	...
	if useEncryption {
		if factoryReset {
			...
			saveBoostrapContainer, err := installCreateSaveBootstrappedContainer(saveNode)
			if err != nil {
				return err
			}
