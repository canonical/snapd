...
type initramfsMountsState struct {
	...
	firstBoot     bool
	// essentialSnaps stores the data necessary to load integrity data for the
	// essential snaps that are used in this boot. These come from the seed when
	// booting in install/recovery/factoryreset mode or first boot in run mode.
	// Otherwise these are are loaded from ubuntu-data.
	essentialSnaps map[string]*essentialSnapInfo
}
...
func (mst *initramfsMountsState) LoadEssentialSnapRevisions(modeEnv *boot.Modeenv, mounts map[snap.Type]snap.PlaceInfo) error {
	...
	for _, sn := range mounts {
		decl, err := asserts.SnapDeclarationFromNameAndAuthority(fsdb, sn.SnapName(), modeEnv.BrandID)
		if err != nil {
			return err
		}

		rev, err := asserts.SnapRevisionFromSnapIdAndRevisionNumber(fsdb, decl.SnapID(), sn.SnapRevision().N)
		if err != nil {
			return err
		}
