...
func (s *snapExecSuite) TestSnapExecHookIntegration(c *C) {
	...
	err := snapExecHook("snapname", "42", "config-changing")
	c.Assert(err, IsNil)
	c.Check(execArgv0, Equals, fmt.Sprintf("%s/snapname/42/meta/hooks/config-changing", dirs.SnapMountDir))
...
func (s *snapExecSuite) TestSnapExecHookRealIntegration(c *C) {
	...
	hookPath := filepath.Join("meta", "hooks", "config-changing")
	...
	// run it
	os.Args = []string{"snap-exec", "--hook=config-changing", "snapname"}
	err := run()
	c.Assert(err, IsNil)

	output, err := ioutil.ReadFile(canaryFile)
	c.Assert(err, IsNil)
	c.Assert(string(output), Equals, "config-changing\n\n")
}
