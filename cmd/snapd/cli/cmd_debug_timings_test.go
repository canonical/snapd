...
func (s *SnapSuite) TestGetDebugTimings(c *C) {
	...
	restore := cli.MockIsStdinTTY(true)
	defer restore()
	...
	for _, test := range timingsTests {
		...
		_, err := cli.Parser(cli.Client()).ParseArgs(strings.Fields(test.args))
		if test.error != "" {
...
func (s *SnapSuite) TestSortTimingsTasks(c *C) {
	...
	testData := []struct {
		ChangeTimings map[string]cli.ChangeTimings
		Expected      []string
	}{{
		// nothing to do
		ChangeTimings: map[string]cli.ChangeTimings{},
	...
	for _, data := range testData {
		tasks := cli.SortTimingsTasks(data.ChangeTimings)
