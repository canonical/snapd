...
func (s *SnapSuite) TestGetDebugTimings(c *C) {
	...
	for _, test := range timingsTests {
		...
		_, err := snap.Parser(snap.Client()).ParseArgs(strings.Fields(test.args))
		if test.error != "" {
...
func (s *SnapSuite) TestSortTimingsTasks(c *C) {
	...
	testData := []struct {
		ChangeTimings map[string]snap.ChangeTimings
		Expected      []string
	}{{
		// nothing to do
		ChangeTimings: map[string]snap.ChangeTimings{},
	...
	for _, data := range testData {
		tasks := snap.SortTimingsTasks(data.ChangeTimings)
