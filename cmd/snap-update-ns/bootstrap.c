...
// validate_instance_name performs full validation of the given snap instance
// name.
...
void process_arguments(int argc, char *const *argv, const char **snap_name_out, bool *should_setns_out,
    ...
    // Find the name of the called program. If it is ending with ".test" then do
    // nothing. NOTE: This lets us use cgo/go to write tests without running the
    // bulk of the code automatically.
    //
    if (argv == NULL || argc < 1) {
        bootstrap_errno = 0;
    ...
    // Ensure that the snap instance name is valid so that we don't blindly
    // setns into something that is controlled by a potential attacker.
