...
func planWritableMimic(dir string) ([]*Change, error) {
	...
	changes = append(changes, &Change{
		Action: Mount, Entry: osutil.Entry{
	...
	changes = append(changes, &Change{
		Action: Mount, Entry: osutil.Entry{Name: "tmpfs", Dir: dir, Type: "tmpfs"},
	})
	...
	for _, fi := range entries {
		ch := &Change{Action: Mount, Entry: osutil.Entry{
	...
	changes = append(changes, &Change{
		Action: Unmount, Entry: osutil.Entry{Name: "none", Dir: safeKeepingDir},
	})
	return changes, nil
}
...
func execWritableMimic(plan []*Change) ([]*Change, error) {
	...
	for i, change := range plan {
		...
		undoChange := &Change{
			Action: Mount,
			Entry:  osutil.Entry{Dir: change.Entry.Dir, Name: change.Entry.Name, Type: change.Entry.Type, Options: change.Entry.Options},
		}
