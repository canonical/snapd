...
func (c *Change) createPath(path string, pokeHoles bool, upCtx MountProfileUpdateContext) ([]*Change, error) {
	...
	case "ensure-dir":
		uid = sys.UserID(upCtx.UID())
		gid = sys.GroupID(upCtx.GID())
		if uid == 0 {
		...
		for parentTargetDir, entriesNeedingDir := range affectedTargetCreationDirs {
			...
			for _, entry := range entriesNeedingDir {
				...
				if !exists {
					...
					if entry.XSnapdKind() == "ensure-dir" {
						logger.Debugf("NeededChanges() detected \"ensure-dir\" entry and will add it as independent entry")
					}
