...
%if 0%{?with_test_keys}
%define option_test_keys true
%else
%define option_test_keys false
%endif

%if 0%{?with_bundled}
%define feature_vendored enabled
%else
%define feature_vendored disabled
%endif

%if 0%{?with_selinux}
%define feature_selinux enabled
%define feature_libselinux enabled
%else
%define feature_selinux disabled
%define feature_libselinux disabled
%endif

%if %{with snap_symlink}
%define option_snap_symlink true
%else
%define option_snap_symlink false
%endif

%if 0%{?with_multilib}
%define option_nvidia_biarch true
%define option_32bit_libdir -D32bit-libdir=%{_prefix}/lib
%else
%define option_nvidia_biarch false
%endif

%meson \
    --libexecdir=%{_libexecdir}/snapd \
    -Ddownstream_version_suffix=-%{release} \
    -Dvendored=%{feature_vendored} \
    -Dlibapparmor=disabled \
    -Dapparmor=disabled \
    -Dlibselinux=%{feature_libselinux} \
    -Dselinux=%{feature_libselinux} \
    -Dsnapd_environment_file=%{_sysconfdir}/sysconfig/snapd \
    -Denvd=%{_prefix}/lib/environment.d \
    -Dsetcap=false \
    -Dsnapmountdir=/var/lib/snapd/snap \
    -Dsnap-symlink=%{option_snap_symlink} \
    -Dsecboot=false \
    -Dtest-keys=%{option_test_keys} \
    -Dgoldflags='%__global_ldflags' \
    -Dstatic-checks=false \
    -Dnvidia-biarch=%{option_nvidia_biarch} \
    %{?option_32bit_libdir} \
    -Dgotags="rpm_crashtraceback"
...
- New upstream release 2.78
 ...
 - FDE: update secboot to rev dd95d85 - support thunderbolt security
   level 0 on UC
 - components: account for components in seed refresh triggering
 - components: prevent components in the seed from being removed
 - components: set $SNAP_COMPONENTS in the application's environment,
   with the path to the snap's own components directory
 - confdb: make error message on map keys for JSON config values more
   specific
 - confdb: support reading/writing to builtin confdbs
 - confdb: enforce validation set constraints before changing state
 - remote device management: update the prepare-serial-request
   registration body to use hardware-id-key-sha3-384 field name
 - packaging: introduce a downstream version suffix to manipulate
   rather than the full version
 - packaging: workaround dh_golang, drop prepare-debian-build-tree
 - security logging: change LogAdminActivity and
   LogUnauthorizedAccess api and structs to specify Who, What and
   Why/-Not
 - interfaces: content | support for sharing content from components
   using the $SNAP_COMPONENT(...) syntax
 - interfaces: shutdown | allow RebootWithFlags
 - interfaces: daemon-notify | allow disconnected path
   /systemd/notify
 - interfaces: microceph-support | add user-identity-switching plug
   attribute
 - interfaces: u2f-devices | add Gemalto eToken Fusion to fallback
   list
 - interfaces: ubuntu-drivers-observe | add new interface
 - interfaces: power-control | allow standardized charge threshold
   sysfs attributes
 - LP: #2161845 interfaces: fix snapd system-key comparison always
   failing
 - LP: #2161950 interfaces: unity7 | allow Chromium
   StatusNotifierItem object paths
 - LP: #2164382 interfaces: u2f-devices | use fido-id udev builtin to
   identify devices
 - LP: #2164689 interfaces: {personal,system}-files | fix profile if
   path has spaces
 - LP: #2106306 check user before deleting snap user data during snap
   removal
 ...
 - Attempt to refresh just essential snaps when disk space check
   fails
 - Properly consider new and existing revisions during disk space
   checks
 - Add disk-reservation.size option and migrate experimental disk
   space checks
 - Optimize perl and python rules in apparmor template for core24+
   snaps
 - Fix same kernel occupying multiple boot partitions for Little
   Kernel bootloader
 ...
 - Fix hybrid remodeling when gadget defines a partition with a
   system-seed role
 - Add seed allowlist to recovery-system creation
 - Improve robustness of seed refresh prerequisites by properly
   merging them
