...
%changelog
* Fri May 30 2025 Ernest Lotter <ernest.lotter@canonical.com>
- New upstream release 2.70
 - FDE: Fix reseal with v1 hook key format
 - FDE: set role in TPM keys
 - FDE: refactor listing of the disks based on run mode model and
 ...
 - AppArmor prompting (experimental): add handling for expiration of
   kernel listener and requests
 ...
 - AppArmor prompting: simplify the listener
 ...
 - Confdb: load ephemeral data when reading data via 'snapctl get'
 - Confdb: load ephemeral data when reading data via 'snap get'
 - Confdb: rename {plug}-view-changed hook to observe-view-{plug}
 - Confdb: rename confdb assertion to confdb-schema
 - Confdb: change operator grouping in confdb-control assertion
 - Confdb: add confdb-control API
 ...
 - Non-suid snap-confine: replace snap-confine suid with capabilities
   to achieve the required permissions
 - Initial support for dynamic security profiles updates
 ...
 - When running snap, on system key mismatch, wait for a pending
   security profiles regeneration
 ...
 - Start/stop ssh.socket for core24+ when enabling/disabling the ssh
   service
 - Modified core-initrd to use systemctl reboot instead of
   /sbin/reboot
 ...
 - Added feature to inject proxy store assertions in build image
 ...
 - Interfaces: add support for reinitialization in AppArmor security
   backend
