...
`+t.plugSnippet,
		...
		c.Check(s.sysdLog, DeepEquals, [][]string{
			{"enable", "snap.hello-snap.svc1.service"},
			{"daemon-reload"},
		}, comment)
...
func (s *servicesTestSuite) TestAddSnapServicesWithDisabledServicesNowApp(c *C) {
	...
	// enable calls
	c.Assert(r.Calls(), DeepEquals, [][]string{
		{"systemctl", "enable", "snap.hello-snap.svc1.service"},
		{"systemctl", "daemon-reload"},
	})
}
...
func (s *servicesTestSuite) TestStartServices(c *C) {
	...
	err := wrappers.StartServices(info.Services(), nil, nil, &progress.Null, s.perfTimings)
	c.Assert(err, IsNil)

	c.Assert(s.sysdLog, DeepEquals, [][]string{
		{"is-enabled", filepath.Base(svcFile)},
...
`, &snap.SideInfo{Revision: snap.R(12)})
	...
	c.Assert(s.sysdLog, DeepEquals, [][]string{
		{"--user", "--global", "is-enabled", filepath.Base(svcFile)},
...
func (s *servicesTestSuite) TestNoStartDisabledServices(c *C) {
	...
	c.Assert(r.Calls(), DeepEquals, [][]string{
		{"systemctl", "is-enabled", filepath.Base(svcFile)},
	})
}
...
`, &snap.SideInfo{Revision: snap.R(12)})
	...
	c.Check(sysdLog, DeepEquals, [][]string{
		...
		{"--user", "--global", "enable", svc2Name}, // this one fails
...
`, &snap.SideInfo{Revision: snap.R(12)})
	...
	err := wrappers.StartServices(svcs, nil, nil, &progress.Null, s.perfTimings)
	c.Assert(err, ErrorMatches, "failed")
...
`, &snap.SideInfo{Revision: snap.R(12)})
	...
	err := wrappers.StartServices(apps, nil, nil, &progress.Null, s.perfTimings)
	c.Assert(err, ErrorMatches, "failed")
...
`, &snap.SideInfo{Revision: snap.R(12)})
	...
	c.Check(sysdLog, DeepEquals, [][]string{
		{"--user", "--global", "is-enabled", svc1Name},
		{"--user", "--global", "is-enabled", svc2Name},
		{"--user", "start", svc1Name},
...
`, &snap.SideInfo{Revision: snap.R(12)})
	...
	err = wrappers.StartServices(sorted, nil, nil, &progress.Null, s.perfTimings)
	c.Assert(err, IsNil)
