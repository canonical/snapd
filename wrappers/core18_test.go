...
func makeMockSnapdSnap(c *C) *snap.Info {
	...
	c.Assert(err, IsNil)
	err = os.MkdirAll(dirs.SnapBusPolicyDir, 0755)
	c.Assert(err, IsNil)
	err = os.MkdirAll(dirs.SnapBusSessionPolicyDir, 0755)
	c.Assert(err, IsNil)
...
`, dirs.GlobalRootDir)
	for _, entry := range [][]string{{
		...
		filepath.Join(dirs.SnapBusPolicyDir, "snapd.system-services.conf"),
		"<busconfig/>",
	}, {
		filepath.Join(dirs.SnapBusSessionPolicyDir, "snapd.session-services.conf"),
		"<busconfig/>",
	}} {
...
func (s *servicesTestSuite) TestAddSnapServicesForSnapdOnClassic(c *C) {
	...
	c.Check(osutil.FileExists(filepath.Join(dirs.SnapBusPolicyDir, "snapd.system-services.conf")), Equals, false)
	c.Check(osutil.FileExists(filepath.Join(dirs.SnapBusSessionPolicyDir, "snapd.session-services.conf")), Equals, false)
...
func (s *servicesTestSuite) TestRemoveSnapServicesForFirstInstallSnapdOnCore(c *C) {
	...
	units := [][]string{
		...
		{filepath.Join(dirs.SnapBusPolicyDir, "snapd.system-services.conf"), "from-snapd"},
		{filepath.Join(dirs.SnapBusSessionPolicyDir, "snapd.session-services.conf"), "from-snapd"},
