...
// undoFirstInstallSnapdServicesOnCore removes the snapd services generated by a prior call
...
func undoFirstInstallSnapdServicesOnCore(s *snap.Info, inter interacter) error {
	...
	if snapType := s.GetType(); snapType != snap.TypeSnapd {
		return fmt.Errorf("cannot remove snapd services of non-snapd snap %q", s.InstanceName())
	}
