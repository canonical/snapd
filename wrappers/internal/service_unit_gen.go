...
func serviceStopTimeout(app *snap.AppInfo) time.Duration {
	...
	return ensureMinSystemdDuration(tout, "stop-timeout")
}
...
func ensureMinSystemdDuration(in timeout.Timeout, field string) time.Duration {
	...
	if inDur > 0 && inDur < time.Microsecond {
		...
		logger.Noticef("Warning: %s set to 1us as it cannot be smaller than that", field)
...
`
	...
	wrapperData := struct {
		...
		StartTimeout:    ensureMinSystemdDuration(appInfo.StartTimeout, "start-timeout"),
		RestartDelay:    ensureMinSystemdDuration(appInfo.RestartDelay, "restart-delay"),
		WatchdogTimeout: ensureMinSystemdDuration(appInfo.WatchdogTimeout, "watchdog-timeout"),
