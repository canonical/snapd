...
func RemoveMountUnitFiles(s *snap.Info, inter interacter) error {
	...
	// One mount unit is generated by the backend to mount the snap's squashfs
	// and is handled separately by backend.RemoveSnapFiles(). So, we exclude
	// it from here.
	snapMountPoint := dirs.StripRootDir(s.MountDir())
