...
func makeRunnableSystem(model *asserts.Model, bootWith *BootableSet, trusted *TrustedAssets, encryption *EncryptionParameters, bootEntryUpdater BootEntryUpdater, makeOpts makeRunnableOptions) error {
	if model.Grade() == asserts.ModelGradeUnset {
	...
	if trusted != nil {
		currentTrustedBootAssets = trusted.trackedAssets
		currentTrustedRecoveryBootAssets = trusted.trackedRecoveryAssets
	}
	...
	if bootEntryUpdater != nil {
		if err := bootEntryUpdater.Update(); err != nil {
...
// running in between.
func MakeRunnableSystem(model *asserts.Model, bootWith *BootableSet, trusted *TrustedAssets, encryption *EncryptionParameters, bootEntryUpdater BootEntryUpdater) error {
	return makeRunnableSystem(model, bootWith, trusted, encryption, bootEntryUpdater, makeRunnableOptions{
...
func MakeRunnableStandaloneSystem(model *asserts.Model, bootWith *BootableSet, trusted *TrustedAssets, encryption *EncryptionParameters, bootEntryUpdater BootEntryUpdater, unlocker Unlocker) error {
...
func MakeRunnableStandaloneSystemFromInitrd(model *asserts.Model, bootWith *BootableSet, trusted *TrustedAssets, encryption *EncryptionParameters, bootEntryUpdater BootEntryUpdater) error {
	// TODO consider merging this back into MakeRunnableSystem but need
	// to consider the properties of the different input used for sealing
	return makeRunnableSystem(model, bootWith, trusted, encryption, bootEntryUpdater, makeRunnableOptions{
		Standalone: true,
...
func MakeRunnableSystemAfterDataReset(model *asserts.Model, bootWith *BootableSet, trusted *TrustedAssets, encryption *EncryptionParameters, bootEntryUpdater BootEntryUpdater) error {
	return makeRunnableSystem(model, bootWith, trusted, encryption, bootEntryUpdater, makeRunnableOptions{
		AfterDataReset: true,
		SeedDir:        dirs.SnapSeedDir,
	})
