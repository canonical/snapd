...
func (s *sealSuite) TestResealKeyToModeenvWithSystemFallback(c *C) {
	...
	for idx, tc := range []struct {
		...
		resealKeysCalls := 0
		restore = boot.MockResealKeyForBootChains(func(unlocker boot.Unlocker, method device.SealingMethod, rootdirArg string, params *boot.ResealKeyForBootChainsParams, options boot.ResealKeyToModeenvOptions) error {
			resealKeysCalls++
