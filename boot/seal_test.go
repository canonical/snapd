	...
	for _, tc := range []struct {
		...
		restore = boot.MockSecbootSealKeys(func(keys []secboot.SealKeyRequest, params *secboot.SealKeysParams) error {
			...
			case 1:
				...
				c.Check(keys, DeepEquals, []secboot.SealKeyRequest{{Key: myKey, KeyName: "ubuntu-data-key", KeyFile: dataKeyFile}})
			case 2:
				...
				c.Check(keys, DeepEquals, []secboot.SealKeyRequest{{Key: myKey, KeyName: "ubuntu-data-key", KeyFile: dataKeyFile}, {Key: myKey2, KeyName: "ubuntu-save-key", KeyFile: saveKeyFile}})
			default:
...
func (s *sealSuite) TestSealToModeenvWithFdeHookHappy(c *C) {
	...
	c.Check(runFDESetupHookParams, DeepEquals, []*boot.FDESetupHookParams{
		{Key: secboot.EncryptionKey{1, 2, 3, 4}, KeyName: "ubuntu-data-key", Models: []*asserts.Model{model}},
		{Key: secboot.EncryptionKey{1, 2, 3, 4}, KeyName: "ubuntu-data-key", Models: []*asserts.Model{model}},
		{Key: secboot.EncryptionKey{5, 6, 7, 8}, KeyName: "ubuntu-save-key", Models: []*asserts.Model{model}},
	})
