...
func (client *Client) GetConfig(snapName, configField string) (configuration string, err error) {
	_, err = client.doSync("GET", "/v2/snaps/"+snapName+"/config/"+configField, nil, nil, nil, &configuration)
	if err != nil {
		return "", err
	}
