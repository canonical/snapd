...
func (cs *clientSuite) TestClientRemoveUser(c *C) {
	removed := mylog.Check2(cs.cli.RemoveUser(&client.RemoveUserOptions{}))
	...
	removed = mylog.Check2(cs.cli.RemoveUser(&client.RemoveUserOptions{Username: "one-user"}))
...
func (cs *clientSuite) TestClientRemoveUserError(c *C) {
	removed := mylog.Check2(cs.cli.RemoveUser(nil))
	c.Assert(err, ErrorMatches, "cannot remove a user without providing a username")
	c.Assert(removed, IsNil)
	removed = mylog.Check2(cs.cli.RemoveUser(&client.RemoveUserOptions{}))
	c.Assert(err, ErrorMatches, "cannot remove a user without providing a username")
...
func (cs *clientSuite) TestClientCreateUser(c *C) {
	_ := mylog.Check2(cs.cli.CreateUser(nil))
	c.Assert(err, ErrorMatches, "cannot create a user without providing an email")
	_ = mylog.Check2(cs.cli.CreateUser(&client.CreateUserOptions{}))
	c.Assert(err, ErrorMatches, "cannot create a user without providing an email")
	...
	rsp := mylog.Check2(cs.cli.CreateUser(&client.CreateUserOptions{Email: "one@email.com", Sudoer: true, Known: true}))
	c.Assert(cs.req.Method, Equals, "POST")
	c.Assert(cs.req.URL.Path, Equals, "/v2/users")
...
func (cs *clientSuite) TestClientCreateUsers(c *C) {
	for _, test := range createUsersTests {
		...
		results := mylog.Check2(cs.cli.CreateUsers(test.options))
		if test.error != "" {
		...
		for _, req := range cs.reqs {
			...
			data := mylog.Check2(io.ReadAll(req.Body))
...
func (cs *clientSuite) TestUsers(c *C) {
	...
	users := mylog.Check2(cs.cli.Users())
	c.Check(err, IsNil)
