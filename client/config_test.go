...
func (cs *clientSuite) TestClientGetConfigCallsEndpoint(c *check.C) {
	cs.cli.GetConfig("snap-name", "test-field")
	c.Check(cs.req.Method, check.Equals, "GET")
	c.Check(cs.req.URL.Path, check.Equals, "/v2/snaps/snap-name/config/test-field")
}
...
func (cs *clientSuite) TestClientGetConfig(c *check.C) {
	...
	value, err := cs.cli.GetConfig("snap-name", "test-field")
	c.Assert(err, check.IsNil)
