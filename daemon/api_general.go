...
func sysInfoPost(c *Command, r *http.Request, user *auth.UserState) Response {
	...
	// TODO handle error indicating that client's version is too new for current
	// snapd
	if err != nil {
		if errors.Is(err, interfaces.ErrSystemKeyMismatchVersionTooHigh) {
			// a newer client, we can't really do anything about it
			// TODO specific error
		}
