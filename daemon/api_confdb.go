...
func handleConfdbControlAction(c *Command, r *http.Request, user *auth.UserState) Response {
	...
	if err := decoder.Decode(&a); err != nil {
		return BadRequest("cannot decode request body into action: %v", err)
	}
	...
	// cc, err = devMgr.SignConfdbControl(cc.Groups(), cc.Revision()+1)
	// if err != nil {
	// 	return InternalError(err.Error())
	// }
	...
	return AssertResponse([]asserts.Assertion{cc}, false)
}
...
func getOrCreateConfdbControl(st *state.State, devMgr *devicestate.DeviceManager) (*asserts.ConfdbControl, error) {
	...
	as, err := db.Find(asserts.ConfdbControlType, map[string]string{
	...
	return as.(*asserts.ConfdbControl), nil
}
