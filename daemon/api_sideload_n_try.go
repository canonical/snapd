...
func sideloadInfo(st *state.State, uploads []*uploadedContainer, flags sideloadFlags) (*sideloadedInfo, *apiError) {
	...
	// already installed.
	uploadedOrInstalledSnapFunc := uploadedOrInstalledSnapInfoFunc(st, snaps)
	...
	for _, upload := range potentialComponents {
		...
		compInfo, snapInfo, compErr := readComponentInfo(st, upload, flags, deviceCtx.Model(), uploadedOrInstalledSnapFunc)
		if compErr != nil {
...
func sideloadSnap(_ context.Context, st *state.State, upload *uploadedContainer, flags sideloadFlags) (*state.Change, *apiError) {
	...
	if snapErr != nil {
		...
		compInfo, snapInfo, compErr = readComponentInfo(st, upload, flags, model, installedSnapInfoFunc(st))
		if compErr != nil {
			logger.Noticef("cannot sideload as a snap: %v", snapErr)
			logger.Noticef("cannot sideload as a component: %v", compErr)
...
// installedSnapInfoFunc returns a function that looks for the components
// associated snap in the set of installed snaps.
func installedSnapInfoFunc(st *state.State) func(string, naming.ComponentRef) (*snap.Info, *apiError) {
...
func uploadedOrInstalledSnapInfoFunc(st *state.State, snaps []sideloadSnapInfo) func(string, naming.ComponentRef) (*snap.Info, *apiError) {
	return func(instanceName string, cref naming.ComponentRef) (*snap.Info, *apiError) {
