...
func (s *snapctlSuite) TestSnapctlGetFeatures(c *check.C) {
	...
	req, err := http.NewRequest("POST", "/v2/snapctl", buf)
	c.Assert(err, check.IsNil)
	daemon.AddUcrednetToRequest(req, daemon.NewUcrednet("some-snap", "", 100, dirs.SnapSocket))
...
func (s *snapctlSuite) TestSnapctlGenericError(c *check.C) {
	...
	req, err := http.NewRequest("POST", "/v2/snapctl", buf)
	c.Assert(err, check.IsNil)
	daemon.AddUcrednetToRequest(req, daemon.NewUcrednet("some-snap", "", 0, dirs.SnapSocket))
	rsp := s.errorReq(c, req, nil, actionIsExpected)
