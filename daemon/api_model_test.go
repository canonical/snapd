...
func (s *apiSuite) TestGetModelHasNewestModelAssertion(c *check.C) {
	oldModel := s.brands.Model("my-brand", "my-old-model", modelDefaults)
	newModel := s.brands.Model("my-brand", "my-old-model", modelDefaults, map[string]interface{}{
		"revision": "2",
	})
	...
	// make a new get request to the model endpoint to make sure that the
	// revision key is missing from the model assertion we get
	req, err := http.NewRequest("GET", "/v2/model", nil)
	response := getModel(appsCmd, req, nil)
	...
	// set it and validate that this is what we was passed to
	// devicestateRemodel
	req2, err := http.NewRequest("POST", "/v2/model", bytes.NewBuffer(data))
	c.Assert(err, check.IsNil)
	rsp := postModel(appsCmd, req2, nil).(*resp)
	c.Assert(rsp.Status, check.Equals, 202)
	c.Check(devicestateRemodelGotModel, check.DeepEquals, newModel)

	// make another get request to the model endpoint to get the new model
	req3, err := http.NewRequest("GET", "/v2/model", nil)
	response2 := getModel(appsCmd, req3, nil)

	// check that we get an assertion response
	c.Assert(response2, check.FitsTypeOf, &assertResponse{})

	// check that there is only one assertion
	assertions2 := response2.(*assertResponse).assertions
	c.Assert(assertions2, check.HasLen, 1)

	// check that one of the assertion keys matches what's in the model we
	// provided
	assert2 := assertions2[0]
	revision2 := assert2.Header("revision")
	c.Assert(revision2, check.FitsTypeOf, "")
	c.Assert(revision2.(string), check.Equals, "2")
}
