...
func (r *AspectRequest) validate() error {
	if r.Account == "" {
		...
		return fmt.Errorf(emptyFieldFmt, "directory")
	} else if r.Aspect == "" {
...
func getAspect(c *Command, r *http.Request, _ *auth.UserState) Response {
	...
	value, err := aspectstate.Get(c.d.state, req.Account, req.Directory, req.Aspect, req.Field)
	if err != nil {
...
func setAspect(c *Command, r *http.Request, _ *auth.UserState) Response {
	...
	err := aspectstate.Set(c.d.state, req.Account, req.Directory, req.Aspect, req.Field, req.Value)
	if err != nil {
		if errors.Is(err, &aspects.NotFoundError{}) {
			return NotFound(err.Error())
		}
