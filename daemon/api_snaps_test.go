...
func (s *snapsSuite) TestRefreshEnforceTryEnforceValidationSetsError(c *check.C) {
	...
	defer daemon.MockSnapstateEnforceSnaps(func(_ context.Context, _ *state.State, _ map[string]*asserts.ValidationSet, validErr *snapasserts.ValidationSetsValidationError, _ int) ([]*state.TaskSet, []string, error) {
...
func (s *snapsSuite) TestRefreshEnforceWithSnapsIsAnError(c *check.C) {
	...
	defer daemon.MockSnapstateEnforceSnaps(func(context.Context, *state.State, map[string]*asserts.ValidationSet, *snapasserts.ValidationSetsValidationError, int) ([]*state.TaskSet, []string, error) {
...
func (s *snapsSuite) TestRefreshEnforceSetsReplacedWithSpecificSequences(c *check.C) {
	...
	var called bool
	restore = daemon.MockSnapstateEnforceSnaps(func(_ context.Context, _ *state.State, validationSets map[string]*asserts.ValidationSet, validErr *snapasserts.ValidationSetsValidationError, _ int) ([]*state.TaskSet, []string, error) {
		called = true
		c.Assert(validationSets, check.DeepEquals, map[string]*asserts.ValidationSet{
			"foo/bar=2": updateValset,
			"foo/baz":   installValset,
		})
		return nil, []string{"update-snap", "install-snap"}, nil
	})
	defer restore()

	d := s.daemon(c)
	inst := &daemon.SnapInstruction{Action: "refresh", ValidationSets: []string{"foo/baz", "foo/bar=2"}}
...
func (s *snapsSuite) TestRefreshEnforceSetsNoConflict(c *check.C) {
	restore := daemon.MockAssertstateTryEnforceValidationSets(func(st *state.State, validationSets []string, userID int, snaps []*snapasserts.InstalledSnap, ignoreValidation map[string]bool) error {
		return nil
	})
	defer restore()

	restore = daemon.MockSnapstateEnforceSnaps(func(context.Context, *state.State, map[string]*asserts.ValidationSet, *snapasserts.ValidationSetsValidationError, int) ([]*state.TaskSet, []string, error) {
