...
func (s *cacheSuite) TestCleanupWithCachePolicy(c *C) {
	s.cm = store.NewCacheManagerWithPolicy(c.MkDir(), store.CachePolicy{
		MaxItems:     3,
...
func (s *cachePolicySuite) TestMaxSize(c *C) {
	...
	now := time.Now()
	// files will be removed starting form the oldest until we meet the size limit
...
func (s *cachePolicySuite) TestApplyMultipleLimits(c *C) {
	...
	now := time.Now()
	// arrange a scenario where files will be dropped in order to satiety different limits in the policy
...
func (s *cachePolicySuite) TestZeroMaxItems(c *C) {
	// test that MaxItems=0 removes all candidates
	cp := store.CachePolicy{
		MaxItems:     0,
		MaxSizeBytes: 10 * 1024 * 1024,
		MaxAge:       30 * 24 * time.Hour,
...
func (s *cachePolicySuite) TestZeroMaxSize(c *C) {
	// test that MaxSize=0 removes all candidates
	cp := store.CachePolicy{
		MaxItems:     10,
		MaxSizeBytes: 0,
		MaxAge:       30 * 24 * time.Hour,
	...
	c.Assert(len(toRemove), Equals, 2)
	c.Check(entryNames(toRemove), DeepEquals, []string{"file1", "file2"})
}
