...
func (bc *bootstrappedContainer) RegisterKey(primaryKey []byte, unlockKey []byte) {
	...
	if _, err := unixAddKey("user", fmt.Sprintf("ubuntu-fde:%s:unlock", uuidDevlink), unlockKey, -4); err != nil {
		logger.Noticef("warning: cannot register unlock key for %s: %v", uuidDevlink, err)
	}
	if _, err := unixAddKey("user", fmt.Sprintf("ubuntu-fde:%s:aux", uuidDevlink), primaryKey, -4); err != nil {
