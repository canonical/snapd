...
func ResealKey(key KeyDataLocation, params *ResealKeyParams) (UpdatedKeys, error) {
	loadedKey := &defaultKeyLoader{}
	keyData, _, err := key.readTokenAndGetWriter()
	if err != nil {
	...
	case keyData != nil:
		...
		case "fde-hook-v2":
			resealKind = hookResealKind
		default:
			logger.Noticef("unknown platform %s, assuming it is TPM", keyData.PlatformName())
		}
	default:
		panic("unexpected")
	}
	...
	case tpmResealKind:
		...
		return ResealKeys(keyParams, params.IncrementRevocationCounter)
