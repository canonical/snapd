...
func UnlockEncryptedVolumeUsingProtectorKey(activation ActivateContext, disk disks.Disk, name string, key []byte) (UnlockResult, error) {
	...
	if foundPlainKey {
		...
		if err := activation.ActivateContainer(context.Background(), container, sb_luks2.WithVolumeName(mapperName), sb.WithLegacyKeyringKeyDescriptionPaths(encdev)); err != nil {
			return unlockRes, err
		}
	} else {
