...
// Error returned if PartitionWithFsLabel cannot find the filesystem label.
type FsLabelNotFoundError struct {
	SearchLabel string
}

func (e FsLabelNotFoundError) Error() string {
	return fmt.Sprintf("filesystem label %q not found", e.SearchLabel)
}

var _ = error(FsLabelNotFoundError{})
...
func UnlockVolumeUsingSealedKeyIfEncrypted(disk Disk, name string, sealedEncryptionKeyFile string, opts *UnlockVolumeUsingSealedKeyOptions) (UnlockResult, error) {
	...
	if err == nil {
		...
		var errNotFound FsLabelNotFoundError
...
func UnlockEncryptedVolumeUsingProtectorKey(disk Disk, name string, key []byte) (UnlockResult, error) {
