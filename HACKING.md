...
The nested suites use some environment variables to configure the suite and the tests inside it, the most important ones are the described bellow:
    ...
    SPREAD_WORK_DIR: It is path to the directory where all the nested assets and images are stored
    SPREAD_NESTED_TYPE: Use core for ubuntu core nested systems or classic instead.
    SPREAD_CORE_CHANNEL: The images are created using ubuntu-image snap, use it to define the default branch
    SPREAD_CORE_REFRESH_CHANNEL: The images can be refreshed to a specific channel, use it to specify the channel
    SPREAD_USE_CLOUD_INIT: Use cloud init to make initial system configuration instead of user assertion
    SPREAD_ENABLE_KVM: Enable kvm in the qemu command line
    SPREAD_ENABLE_TPM: re boot in the nested vm in case it is supported (just supported on UC20)
