...
func (s *seed20) newIntegrityDataFromAssertion(snapRev *asserts.SnapRevision) (*integrity.IntegrityData, error) {
	...
	for i, sid := range snapIntegrityData {
		...
		// should be used based on extra information (i.e model, device).
		if i > 0 {
			break
		}
		...
		default:
			return nil, fmt.Errorf("Unknown type %q for integrity data.", sid.Type)
		}
...
func (s *seed20) findIntegrityData(snapRef naming.SnapRef, handler ContainerHandler, snapsDir string, tm timings.Measurer) (*integrity.IntegrityData, error) {
	var snapRev *asserts.SnapRevision
	...
	integrityData, err := s.newIntegrityDataFromAssertion(snapRev)
	if err != nil {
		return nil, err
	}

	// XXX: return the first integrity data found in the array
...
func (s *seed20) lookupSnap(snapRef naming.SnapRef, modelSnap *asserts.ModelSnap, optSnap *internal.Snap20, channel string, handler ContainerHandler, snapsDir string, tm timings.Measurer) (*Snap, error) {
	...
	timings.Run(tm, "find-integrity-params", fmt.Sprintf("find integrity params for snap %q", snapRef.SnapName()), func(nested timings.Measurer) {
		integrityData, err = s.findIntegrityData(snapRef, handler, snapsDir, tm)
	})
	if err != nil {
		return nil, err
	}
