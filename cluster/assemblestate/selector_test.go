...
func (s *SelectorSuite) TestAddRoutesValidation(c *check.C) {
	...
	_, _, err := selector.AddRoutes("peer", invalid, identified)
	...
	_, _, err = selector.AddRoutes("peer", negative, identified)
	...
	_, _, err = selector.AddRoutes("peer", oob, identified)
	c.Assert(err, check.ErrorMatches, "invalid index in routes")
}
...
func (s *SelectorSuite) TestAddRoutesCounts(c *check.C) {
	...
	added, total, err := sel.AddRoutes("peer", r, identified)
...
func (s *SelectorSuite) TestVerifyRoutes(c *check.C) {
	...
	_, _, err := sel.AddRoutes("peer", r, func(_ assemblestate.RDT) bool { return false })
	c.Assert(err, check.IsNil)
	...
	sel.VerifyRoutes(func(_ assemblestate.RDT) bool { return true })
	expected := assemblestate.Routes{
		Devices:   []assemblestate.RDT{"a", "b"},
		Addresses: []string{"ip"},
		Routes:    []int{0, 1, 0},
...
func (s *SelectorSuite) TestAddAuthoritativeRouteAndPublish(c *check.C) {
	...
	expected := assemblestate.Routes{
		Devices:   []assemblestate.RDT{self, two, one},
...
func (s *SelectorSuite) TestPublishSelectsLowSourceRoutes(c *check.C) {
	...
	_, _, err := sel.AddRoutes(peer, assemblestate.Routes{
		Devices:   []assemblestate.RDT{peer, self},
		Addresses: []string{"peer-addr"},
		Routes:    []int{0, 1, 0},
	}, func(assemblestate.RDT) bool { return true })
	c.Assert(err, check.IsNil)
	...
	add := func(addr string, bumps int) {
		...
		for i := 0; i < bumps; i++ {
			...
			_, _, err := sel.AddRoutes(src, r, func(assemblestate.RDT) bool { return true })
			c.Assert(err, check.IsNil)
		}
	}
...
func (s *SelectorSuite) TestRoutes(c *check.C) {
	...
	_, _, err := sel.AddRoutes("peer", in, func(assemblestate.RDT) bool { return true })
	c.Assert(err, check.IsNil)
...
func (s *SelectorSuite) TestRoutesOnlyIdentified(c *check.C) {
	...
	_, _, err := sel.AddRoutes("peer", in, identified)
	c.Assert(err, check.IsNil)
	...
	sel.VerifyRoutes(func(assemblestate.RDT) bool { return true })
