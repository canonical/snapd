...
type Transport interface {
	...
	Serve(ctx context.Context, ln net.Listener, cert tls.Certificate, pv PeerVerifier) error
...
// PeerVerifier enables a [Transport] to verify peer connections.
type PeerVerifier interface {
	...
	// previsouly been authenticated via a call to AuthenticateAndCommit. The
	// [VerifiedPeer] allows that peer to change the state of the assemble
	// session.
...
type VerifiedPeer interface {
	...
	// RDT returns the RDT of this verified peer.
	RDT() DeviceToken
}
...
func (t *HTTPSTransport) Serve(ctx context.Context, ln net.Listener, cert tls.Certificate, pv PeerVerifier) error {
	mux := http.NewServeMux()
	mux.Handle("/assemble/auth", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		t.handleAuth(w, r, pv)
	}))
	mux.Handle("/assemble/routes", t.trustedHandler(t.handleRoutes, pv))
	mux.Handle("/assemble/unknown", t.trustedHandler(t.handleUnknown, pv))
	mux.Handle("/assemble/devices", t.trustedHandler(t.handleDevices, pv))
...
func (t *HTTPSTransport) trustedHandler(next func(http.ResponseWriter, *http.Request, VerifiedPeer), pv PeerVerifier) http.HandlerFunc {
...
func (t *HTTPSTransport) handleAuth(w http.ResponseWriter, r *http.Request, pv PeerVerifier) {
	if r.Method != "POST" {
		w.WriteHeader(405)
		return
	}
	...
	cert := r.TLS.PeerCertificates[0].Raw
	if err := pv.AuthenticateAndCommit(auth, cert); err != nil {
		w.WriteHeader(403)
