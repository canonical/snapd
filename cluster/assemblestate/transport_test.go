...
func (m *testPeerVerifier) AuthenticateAndCommit(auth assemblestate.Auth, cert []byte) error {
...
func (m *testPeerVerifier) VerifyPeer(cert []byte) (assemblestate.VerifiedPeer, error) {
...
func (m *testVerifiedPeer) RDT() assemblestate.DeviceToken {
	if m.RDTFunc == nil {
		panic("unexpected call to RDT")
	}
	return m.RDTFunc()
}
...
func (s *transportSuite) TestHTTPSTransportServeAuthRoute(c *check.C) {
	...
	go func() {
		defer wg.Done()
		_ = transport.Serve(ctx, ln, testServerCert, pv)
	}()
...
func (s *transportSuite) TestHTTPSTransportServeAuthRejectsFailedAuthentication(c *check.C) {
	...
	stats := transport.Stats()
	c.Assert(stats.Received, check.Equals, int64(0), check.Commentf("no messages should be received when authentication fails"))
