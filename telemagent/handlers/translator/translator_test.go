...
func (ts *translatorSuite) TestAuthConnect(c *C) {
	err := ts.handler.AuthConnect(ts.ctx, false, nil, nil, nil, nil)
	c.Check(err, IsNil)
}
...
func (ts *translatorSuite) TestNoSession(c *C) {
	err := ts.handler.AuthConnect(context.Background(), false, nil, nil, nil, nil)
	c.Check(err, NotNil)
}
...
func (ts *translatorSuite) TestAuthConnectWithCertificate(c *C) {
	...
	err := ts.handler.AuthConnect(ctxWithCertificate, false, nil, nil, nil, nil)
	c.Check(err, IsNil)
}
