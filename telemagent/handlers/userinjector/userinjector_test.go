...
func (us *userinjectorSuite) TestAuthConnect(c *C) {
	err := us.handler.AuthConnect(us.ctx, false, nil, nil, nil, nil)
	c.Check(err, IsNil)
}
...
func (us *userinjectorSuite) TestNoSession(c *C) {
	err := us.handler.AuthConnect(context.Background(), false, nil, nil, nil, nil)
	c.Check(err, NotNil)
}
...
func (us *userinjectorSuite) TestAuthConnectWithCertificate(c *C) {
	...
	err := us.handler.AuthConnect(ctxWithCertificate, false, nil, nil, nil, nil)
	c.Check(err, IsNil)
}
