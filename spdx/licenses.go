...
//
// jq < json/licenses.json '.licenses | .[] | select(.isDeprecatedLicenseId == false) | select(.isOsiApproved == true) | .licenseId' | sort | sed -e 's/$/,/'
var osi = []string{
	"0BSD",
	"AAL",
	"AFL-1.1",
...
// jq < json/licenses.json '.licenses | .[] | select(.isDeprecatedLicenseId == false) | .licenseId' | sort | sed -e 's/$/,/'
