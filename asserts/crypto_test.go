...
func (s *cryptoSuite) TestSignWithKeyAndVerifyWithKey(c *C) {
	...
	signature, err := asserts.SignWithKey(data, priv)
	c.Assert(err, IsNil)

	pub := priv.PublicKey()
	err = asserts.VerifyWithKey(data, signature, pub)
	c.Check(err, IsNil)
}
...
func (s *cryptoSuite) TestVerifyWithKeyMismatch(c *C) {
	...
	// sign with first key
	data := []byte("some data to verify")
	signature, err := asserts.SignWithKey(data, privOne)
	c.Assert(err, IsNil)

	// try to verify with wrong public key (from second key pair)
	pubTwo := privTwo.PublicKey()
	err = asserts.VerifyWithKey(data, signature, pubTwo)
	c.Check(err, NotNil)
...
func (s *cryptoSuite) TestVerifyWithKeyDataMismatch(c *C) {
	...
	err = asserts.VerifyWithKey([]byte("different data"), signature, pub)
	c.Check(err, NotNil)
...
func (s *cryptoSuite) TestVerifyWithKeyWrongSignature(c *C) {
	...
	signatureOne, err := asserts.SignWithKey(dataOne, priv)
	c.Assert(err, IsNil)

	dataTwo := []byte("data two")
	signatureTwo, err := asserts.SignWithKey(dataTwo, priv)
	c.Assert(err, IsNil)

	err = asserts.VerifyWithKey(dataOne, signatureOne, pub)
	c.Check(err, IsNil)

	// try to verify dataOne with signatureTwo (wrong signature for this data)
	err = asserts.VerifyWithKey(dataOne, signatureTwo, pub)
	c.Check(err, NotNil)
	c.Check(err, ErrorMatches, ".*hash tag doesn't match")
}
