...
type sealedPrivKeySuite struct {
	signingDB    *asserts.Database
	signingKeyID string
	db           *asserts.Database
}

var _ = Suite(&sealedPrivKeySuite{})

func (spks *sealedPrivKeySuite) SetUpTest(c *C) {
	cfg0 := &asserts.DatabaseConfig{
		KeypairManager: asserts.NewMemoryKeypairManager(),
	}
	db0, err := asserts.OpenDatabase(cfg0)
	...
	doSign := func(cont []byte, cfg *packet.Config) (*packet.Signature, error) {
		privk := testPrivKey0
	...
	pk := asserts.SealedOpenPGPPrivateKey(&testPrivKey0.PublicKey, doSign)
	err = db0.ImportKey("canonical", pk)
	c.Assert(err, IsNil)
	spks.signingKeyID = pk.PublicKey().ID()
...
func (spks *sealedPrivKeySuite) TestSign(c *C) {
	headers := map[string]string{
		"authority-id": "canonical",
		"primary-key":  "a",
	}
	a1, err := spks.signingDB.Sign(asserts.TestOnlyType, headers, nil, spks.signingKeyID)
	c.Assert(err, IsNil)

	err = spks.db.Check(a1)
	c.Check(err, IsNil)
}
