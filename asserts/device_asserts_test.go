...
func (mods *modelSuite) TestDecodeOK(c *C) {
	...
	c.Check(model.ModelDisplayName(), Equals, "Baz 3000")
...
func (mods *modelSuite) TestDecodeModelDisplayNameIsOptional(c *C) {
	withTimestamp := strings.Replace(modelExample, "TSLINE", mods.tsLine, 1)
	encoded := strings.Replace(withTimestamp, "model-display-name: Baz 3000\n", "model-display-name: \n", 1)
	a, err := asserts.Decode([]byte(encoded))
	c.Assert(err, IsNil)
	model := a.(*asserts.Model)
...
func (mods *modelSuite) TestDecodeInvalid(c *C) {
	...
	invalidTests := []struct{ original, invalid, expectedErr string }{
		...
		{"model-display-name: Baz 3000\n", "model-display-name:\n  - xyz\n", `"model-display-name" header must be a string`},
