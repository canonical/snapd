...
func (mods *modelSuite) TestDecodeOK(c *C) {
	...
	c.Assert(err, IsNil)
	c.Check(baseCoreVersion, Equals, 18)
...
func (mods *modelSuite) TestBaseCoreVersion(c *C) {
	for _, tc := range []struct {
		...
		{
			// a hybrid classic model does carry a core base; callers
			// that must exclude hybrid check Classic() themselves
			comment: "hybrid classic with base",
			encoded: strings.Replace(strings.Replace(classicModelWithSnapsExample, "TSLINE", mods.tsLine, 1), "OTHER", "", 1),
			version: 20,
		},
	} {
