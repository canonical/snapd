...
func (s *configAssetTestSuite) TestTrivialFromConfigAssert(c *C) {
	e := mylog.Check2(bootloader.EditionFromConfigAsset(bytes.NewBufferString(`# Snapd-Boot-Config-Edition: 321
next line
one after that`)))

	c.Assert(e, Equals, uint(321))

	e = mylog.Check2(bootloader.EditionFromConfigAsset(bytes.NewBufferString(`# Snapd-Boot-Config-Edition: 932
# Snapd-Boot-Config-Edition: 321
one after that`)))

	c.Assert(e, Equals, uint(932))

	e = mylog.Check2(bootloader.EditionFromConfigAsset(bytes.NewBufferString(`# Snapd-Boot-Config-Edition: 1234
one after that
# Snapd-Boot-Config-Edition: 321
`)))
...
this is some
this too`), 0644)
	e := mylog.Check2(bootloader.EditionFromDiskConfigAsset(p))
...
func (s *configAssetTestSuite) TestRealConfig(c *C) {
	...
	e := mylog.Check2(bootloader.EditionFromConfigAsset(bytes.NewReader(grubConfig)))
...
func (s *configAssetTestSuite) TestRealRecoveryConfig(c *C) {
	...
	e := mylog.Check2(bootloader.EditionFromConfigAsset(bytes.NewReader(grubRecoveryConfig)))

	c.Assert(e, Equals, uint(2))
}

func (s *configAssetTestSuite) TestNoConfig(c *C) {
	_ := mylog.Check2(bootloader.EditionFromConfigAsset(bytes.NewReader(nil)))
...
func (s *configAssetTestSuite) TestNoFile(c *C) {
	...
	p := filepath.Join(d, "foo")
	// file does not exist
	_ := mylog.Check2(bootloader.EditionFromDiskConfigAsset(p))
...
func (s *configAssetTestSuite) TestUnreadableFile(c *C) {
	...
	d := c.MkDir()
	p := filepath.Join(d, "foo")
	mylog.Check(os.WriteFile(p, []byte("foo"), 0000))
...
func (s *configAssetTestSuite) TestNoEdition(c *C) {
	_ := mylog.Check2(bootloader.EditionFromConfigAsset(bytes.NewReader([]byte(`this is some script
...
func (s *configAssetTestSuite) TestBadEdition(c *C) {
	_ := mylog.Check2(bootloader.EditionFromConfigAsset(bytes.NewReader([]byte(`# Snapd-Boot-Config-Edition: random
data follows
`))))
...
data follows
`)
	bs := mylog.Check2(bootloader.ConfigAssetFrom(script))
