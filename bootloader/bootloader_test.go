...
func (s *bootenvTestSuite) TestInstallBootloaderConfigFromGadget(c *C) {
	for _, t := range []struct {
		...
		rootDir := c.MkDir()
		err := os.WriteFile(filepath.Join(mockGadgetDir, t.gadgetFile), t.gadgetFileContent, 0o644)
		c.Assert(err, IsNil)
...
func (s *bootenvTestSuite) TestBootloaderFind(c *C) {
	for _, tc := range []struct {
		...
		c.Logf("tc: %v", tc.name)
		rootDir := c.MkDir()
		err := os.MkdirAll(filepath.Join(rootDir, filepath.Dir(tc.sysFile)), 0o755)
		c.Assert(err, IsNil)
		err = os.WriteFile(filepath.Join(rootDir, tc.sysFile), nil, 0o644)
		c.Assert(err, IsNil)
...
func (s *bootenvTestSuite) TestBootloaderForGadget(c *C) {
	for _, tc := range []struct {
		...
		gadgetDir := c.MkDir()
		rootDir := c.MkDir()
		err := os.MkdirAll(filepath.Join(rootDir, filepath.Dir(tc.gadgetFile)), 0o755)
		c.Assert(err, IsNil)
		err = os.WriteFile(filepath.Join(gadgetDir, tc.gadgetFile), nil, 0o644)
		c.Assert(err, IsNil)
