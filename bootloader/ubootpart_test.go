...
func (s *ubootpartTestSuite) TestUbootPartRuntimePresentViaEFI(c *C) {
	// At runtime with EFI, Present() finds the partition via EFI disk
	opts := &bootloader.Options{PrepareImageTime: false}
	...
	// Create the EFI boot device symlink
	...
	// Mock disk with boot-state partition
	...
	// Create the boot-state partition symlink
...
func (s *ubootpartTestSuite) TestUbootPartRuntimePresentIgnoresUnrelatedPartlabel(c *C) {
	// A removable device might expose a by-partlabel/ubuntu-boot-state
	// symlink even though the boot disk does not have that partition. The
	// old Present() implementation checked by-partlabel directly and
	// would false-positive here; the new one uses envDevice() which
	// consults the EFI boot disk first.
	opts := &bootloader.Options{PrepareImageTime: false}

	const loaderDevicePartUUID = "LoaderDevicePartUUID-4a67b082-0a4c-41cf-b6c7-440b29bb8c4f"
	restoreEFI := efi.MockVars(map[string][]byte{
		loaderDevicePartUUID: bootloadertest.UTF16Bytes("DDDD-EEEE-FFFF"),
	}, nil)
	defer restoreEFI()
	...
	err := os.MkdirAll(byPartUUID, 0755)
	c.Assert(err, IsNil)
	mockPartDev := filepath.Join(s.rootdir, "/dev/sda1")
	err = os.WriteFile(mockPartDev, nil, 0644)
	c.Assert(err, IsNil)
	err = os.Symlink(mockPartDev, filepath.Join(byPartUUID, "dddd-eeee-ffff"))
	c.Assert(err, IsNil)

	// The EFI boot disk does NOT have ubuntu-boot-state
	noBootStateDisk := &disks.MockDiskMapping{
		Structure: []disks.Partition{
			{
				PartitionLabel: "ubuntu-seed",
				PartitionUUID:  "seed-partuuid",
			},
		},
		DiskHasPartitions: true,
		DevNum:            "no-bootstate-dev",
	}
	restoreDisk := disks.MockPartitionDeviceNodeToDiskMapping(map[string]*disks.MockDiskMapping{
		mockPartDev: noBootStateDisk,
	})
	defer restoreDisk()

	// Create a by-partlabel symlink from an unrelated removable device.
	// The old by-partlabel check would see this and incorrectly report
	// ubootpart as present.
	byPartLabel := filepath.Join(s.rootdir, "/dev/disk/by-partlabel")
	err = os.MkdirAll(byPartLabel, 0755)
	c.Assert(err, IsNil)
	removableDev := filepath.Join(s.rootdir, "/dev/sdb3")
	err = os.WriteFile(removableDev, nil, 0644)
	c.Assert(err, IsNil)
	err = os.Symlink(removableDev, filepath.Join(byPartLabel, "ubuntu-boot-state"))
	c.Assert(err, IsNil)
