...
This document describes the snapd release process and is primarily intended for snapd release engineers. We include the release process here publicly for both transparency and documentation purposes, though the process requires various administrative privileges unavailable to most.

*Note: At the beginning of every six-month cycle, in agreement with stakeholders, snapd creates a release plan that includes cut dates. Those cut dates determine what each release contains. If you are interested in getting particular content into a given release, be mindful of snapd's date-driven release process and check the release plan.*

## Release process

The snapd release process consists of putting together content, crafting changelogs, building and releasing both snaps and debs along with cross-distro artifacts, and publicizing the release.
...
- a GPG key in both GitHub and Launchpad
- ability to create a release branch and push a tag to the snapd GitHub repository
- snappy-dev group membership in Launchpad
- permission to run autopkgtests in Launchpad (autopkgtest-requesters group membership)
- ability to promote snapd via snapcraft

### Release phases

1. Create the release content by gathering code, curating changelogs, and running the full snapd test suite on the proposed content.
2. snapd snap and snapd deb building and testing are done partially in parallel
	- Build and promote snapd snap and ensure it gets through both internal snapd beta testing as well as certification testing.
	- Build, patch (if required), upload snapd debs, and get them into the official PPA
3. Build and release cross-distro artifacts
...
Create a branch with the name `release/<version-number>` from the master branch on [github.com/canonical/snapd](github.com/canonical/snapd). If you are creating a dot release, you should not create a new branch but rather use the corresponding `release/<major>.<minor>` branch.
...
NOTE: All releases require their own new SRU bug, with the exception of dot releases that intend to supersede a previous release (major or dot) that will not be released

Besides listing the Ubuntu release targets in the description, they should also be selected as release targets. To select them, use the "Target to series" link and select the relevant releases. Once selected, they will appear in the table towards the top of the page.
...
NEWS.md contains lists of functional changes in each snapd release for snapd developers. The release engineer should carefully go through all release content and create a comprehensive and clear list of functional changes that are part of the release.
...
- Request relevant feature developers to summarize grouped comments
...
4. Generate changelogs using the NEWS.md created in the previous step
	...
	- Do not commit the markdown file generated called `snapd-<version>-GitHub-release.md`. It can be used to help create the GitHub release, if so desired. Otherwise, it can be ignored.
	- Double-check changed files
		- `git diff | diffstat`
		- `git diff release/2.75 | diffstat` (any recent previous tag will do)
		- the same files should get touched, all inside the packaging/tree
...
5. Open the PR against `release/<version>`
	- The PR description should be formatted as follows (only include the cherry picked section if you have cherry-picked commits):
		...
		```
6. Add the "Run nested" label and add the senior engineer and all team members as reviewers who have content in NEWS.md.
7. Analyze test results to ensure all failures are understood and do not have an impact on the release quality
	- If there are failures, add a comment in the PR with the test failure analysis

### 4. Merge PR and add tag to commit in canonical/snapd repo

Prerequisites: 
- The PR must be fully approved with all failures explained in a PR comment.
- You must already have the same GPG key successfully uploaded to both GitHub and Launchpad
- You must have permissions to push to the canonical/snapd release branch

**IMPORTANT: the tag must be signed with your GPG key uploaded both to GitHub and Launchpad**

1. Use rebase-merge to merge the PR into the `release/<version>` branch
2. Push the version tag (must be signed with a GPG key present in both GitHub and Launchpad) to canonical/snapd repo
...
### 5. Merge changelogs back into master

1. Create a new branch in your fork called `changelogs-<version>` from the tag you just pushed (2.XX)
2. Merge master (with `--no-ff` option) into the `changelogs-<version>` branch (if there are conflicts, always choose changes from master) (`git merge --no-ff master`)
...
### 6. Garden the GitHub milestone

Ensure there are only either future milestones or one single open milestone.

## Create snapd snap

### 1. Set up the snapd snap builds for `beta/<version>`
...
6. Once you are done with the setup, save, click on the package (https://launchpad.net/~snappy-dev/+snap/snapd-2.XX) and Request Builds.
7. Once the builds have completed, ensure the versions are correct by checking `snapcraft status snapd | grep beta/<version>`

### 2. Release to latest/beta
...
2. Find the revisions for the snap you just pushed out. It will be `beta/<version>`. You can find the revisions by running `snapcraft status snapd`
...
4. Once snapd beta testing and certification testing has concluded, sync with snapd QA and promote to candidate
...
6. Publish the GitHub release created in the "Upload tarballs to GitHub release" section. The cross-distro releases, at this point, may proceed.

### 6. Run WSL smoke tests

Steps:
- Go to [Smoke · Workflow runs · canonical/snapd-wsl-tests (github.com)](https://github.com/canonical/snapd-wsl-tests/actions/workflows/smoke.yaml) where you can run the workflow called “smoke” by hand.
...
Prerequisites:
- Two weeks have passed since snapd was promoted to candidate
...
2. Since the debian symlink was removed from the source, we need to create the debian folder:  `ln -sfn packaging/ubuntu-16.04 debian`
...
5. For each target release, change the changelogs in `build-area/snapd-<version>` to reflect that target and then package it.
...
### 2. Upload tarballs to GitHub release
...
1. Create a GitHub release under https://github.com/canonical/snapd/releases, following the example of previous releases. The changelog.py script from the changelog generation step creates a .md file that can be used in this step, or you can just use NEWS.md and `sed`.
...
### 3. (Optional but highly recommended) Use a testing PPA to test builds and autopkgtests
...
### 4. Upload to snappy-dev/image PPA
...
### 5. Request sponsorship for archive upload to proposed only
...
Steps:
1. Request upload to proposed only (see the [sponsorship documentation](https://documentation.ubuntu.com/project/how-ubuntu-is-made/processes/sponsorship/) for process details). Be sure to remind the sponsor to add a block on the archive upload so that it does not immediately go into the release pocket once automated tests have passed on proposed.
...
Steps:
1. Ensure that the LP Bugs satisfy the [SRU Bug template](https://documentation.ubuntu.com/project/SRU/reference/bug-template/) for at least parts `Impact` and `Test Plan`. Other parts should be added where applicable. The LP bugs related to a release are shown here: https://ubuntu-archive-team.ubuntu.com/pending-sru.html. Each bug should include a template at the top of its description that explains the impact and reproduction steps.
...
### 8. Request upload to archive release pocket

## Cross-distro releases
...
- On your openSUSE machine, install the osc tool, then checkout the snapd package from OBS: `osc checkout system:snappy`
