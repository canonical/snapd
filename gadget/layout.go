...
func IsRoleMBR(ls LaidOutStructure) bool {
	return ls.GadgetStructure.Role == schemaMBR
}
...
func resolveVolumeContent(gadgetRootDir, kernelRootDir string, kernelInfo *kernel.Info, ps *LaidOutStructure, filter ResolvedContentFilterFunc) ([]ResolvedContent, error) {
	...
	if len(ps.GadgetStructure.Content) == 0 {
		return nil, nil
	}

	content := make([]ResolvedContent, 0, len(ps.GadgetStructure.Content))
	for idx := range ps.GadgetStructure.Content {
		resolvedSource, kupdate, err := resolveContentPathOrRef(gadgetRootDir, kernelRootDir, kernelInfo, ps.GadgetStructure.Content[idx].UnresolvedSource)
		if err != nil {
			return nil, fmt.Errorf("cannot resolve content for structure %v at index %v: %v", ps, idx, err)
		}
		rc := ResolvedContent{
			VolumeContent:  &ps.GadgetStructure.Content[idx],
...
func layOutStructureContent(gadgetRootDir string, ps *LaidOutStructure, known map[string]*LaidOutStructure) ([]LaidOutContent, error) {
	...
	content := make([]LaidOutContent, len(ps.GadgetStructure.Content))
	previousEnd := quantity.Offset(0)

	for idx, c := range ps.GadgetStructure.Content {
