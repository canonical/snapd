...
func diskWithSystemSeed(lv *gadget.LaidOutVolume) (device string, err error) {
	for _, vs := range lv.LaidOutStructure {
		...
		if vs.GadgetStructure.Role == gadget.SystemSeed {
			device, err = gadget.FindDeviceForStructure(vs.GadgetStructure)
			if err != nil {
...
func maybeEncryptPartition(odls *onDiskAndLaidoutStructure, encryptionType secboot.EncryptionType, sectorSize quantity.Size, perfTimings timings.Measurer) (fsParams *mkfsParams, encryptionKey keys.EncryptionKey, err error) {
	...
	if mustEncrypt && roleNeedsEncryption(laidOut.GadgetStructure.Role) {
		timings.Run(perfTimings, fmt.Sprintf("make-key-set[%s]", laidOut.GadgetStructure.Role),
			fmt.Sprintf("Create encryption key set for %s", laidOut.GadgetStructure.Role),
