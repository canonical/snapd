...
func buildPartitionList(dl *gadget.OnDiskVolume, odv *gadget.LaidOutVolume, opts *CreateOptions) (sfdiskInput *bytes.Buffer, toBeCreated []gadget.LaidOutStructure, err error) {
	...
	buf := &bytes.Buffer{}
	for _, laidOut := range odv.LaidOutStructure {
		if !laidOut.GadgetStructure.IsPartition() {
			continue
		}
		...
		if !opts.CreateAllMissingPartitions && !gadget.IsCreatableAtInstall(laidOut.GadgetStructure) {
