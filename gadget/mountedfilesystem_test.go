...
func (s *mountedfilesystemTestSuite) SetUpTest(c *C) {
	s.dir = c.MkDir()
	s.backup = c.MkDir()
	s.mockKernelRoot = c.MkDir()
}

func (s *mountedfilesystemTestSuite) mustResolveContentPathsForStructure(c *C, ps *gadget.LaidOutStructure) {
	err := gadget.ResolveContentPathsForStructure(s.dir, s.mockKernelRoot, nil, ps.VolumeStructure)
	c.Assert(err, IsNil)
}
...
func (s *mountedfilesystemTestSuite) TestMountedWriterTrivialValidation(c *C) {
	...
	err = gadget.ResolveContentPathsForStructure("", "", nil, ps.VolumeStructure)
	c.Assert(err, IsNil)
...
func (s *mountedfilesystemTestSuite) TestMountedUpdaterTrivialValidation(c *C) {
	...
	for _, tc := range []struct {
		...
		err := gadget.ResolveContentPathsForStructure("", "", nil, testPs.VolumeStructure)
		c.Assert(err, IsNil)
...
func (s *mountedfilesystemTestSuite) TestMountedUpdaterObserverPreserveNewFile(c *C) {
	...
	rw, err := gadget.NewMountedFilesystemUpdater(psForObserver, s.backup,
		func(to *gadget.LaidOutStructure) (string, error) {
			...
			s.mustResolveContentPathsForStructure(c, psForObserver)
			return outDir, nil
		},
		obs)
	c.Assert(err, IsNil)
