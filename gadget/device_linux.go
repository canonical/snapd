...
func FindDeviceForStructure(ps *LaidOutStructure) (string, error) {
	...
	if ps.GadgetStructure.Name != "" {
		byPartlabel := filepath.Join(dirs.GlobalRootDir, "/dev/disk/by-partlabel/", disks.BlkIDEncodeLabel(ps.GadgetStructure.Name))
		candidates = append(candidates, byPartlabel)
	}
	if ps.GadgetStructure.HasFilesystem() {
		fsLabel := ps.GadgetStructure.Label
		if fsLabel == "" && ps.GadgetStructure.Name != "" {
			// when image is built and the structure has no
			// filesystem label, the structure name will be used by
			// default as the label
			fsLabel = ps.GadgetStructure.Name
		}
		if fsLabel != "" {
