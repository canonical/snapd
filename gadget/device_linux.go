...
func FindDeviceForStructure(ps *LaidOutStructure) (string, error) {
	...
	if ps.Name != "" {
		byPartlabel := filepath.Join(dirs.GlobalRootDir, "/dev/disk/by-partlabel/", strutil.EncodeHexBlkIDFormat(ps.Name))
	...
	if ps.HasFilesystem() {
		...
		if fsLabel != "" {
			byFsLabel := filepath.Join(dirs.GlobalRootDir, "/dev/disk/by-label/", strutil.EncodeHexBlkIDFormat(fsLabel))
