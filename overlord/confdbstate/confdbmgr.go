...
func (m *ConfdbManager) doLoadDataIntoChange(t *state.Task, _ *tomb.Tomb) error {
	...
	return readViewIntoChange(t.Change(), tx, view, requests, cstrs, user)
}

func readViewIntoChange(chg *state.Change, tx *Transaction, view *confdb.View, requests []string, constraints map[string]any, user confdb.Access) error {
	...
	result, err := GetViaView(tx, view, requests, constraints, user)
	if err != nil {
