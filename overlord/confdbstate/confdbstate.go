...
var (
	...
	// testBlockingChan is closed right before blocking to wait for access.
	blockingSignalChan chan struct{}
...
// the transaction.
func GetViaView(bag confdb.Databag, view *confdb.View, requests []string, constraints map[string]any, user confdb.Access) (any, error) {
	if err := view.CheckAllConstraintsAreUsed(requests, constraints); err != nil {
		return nil, err
	}

	if len(requests) == 0 {
		val, err := view.Get(bag, "", constraints, user)
		if err != nil {
			return nil, err
		}
	...
	for _, request := range requests {
		value, err := view.Get(bag, request, constraints, user)
		if err != nil {
...
func waitForAccess(ctx context.Context, st *state.State, view *confdb.View, access accessType) (err error) {
	...
	if blockingSignalChan != nil {
		// signal we're about to block for testing
		close(blockingSignalChan)
	}
...
func GetTransactionForSnapctlGet(hookCtx *hookstate.Context, view *confdb.View, paths []string, constraints map[string]any) (*Transaction, error) {
	...
	// TODO: use txs.CanStartReadTx() once we support blocking access here
	if txs.WriteTxID != "" || len(txs.pending) > 0 {
...
func ReadConfdb(ctx context.Context, st *state.State, view *confdb.View, requests []string, constraints map[string]any, user confdb.Access) (changeID string, err error) {
	defer func() {
		if err != nil {
			uerr := unblockNextAccess(st, view.Schema().Account, view.Schema().Name)
			if uerr != nil {
	...
	if ts != nil {
		...
		err := readViewIntoChange(chg, tx, view, requests, constraints, user)
		if err != nil {
			return "", err
		}
