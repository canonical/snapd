...
func (s *confdbTestSuite) TestGetView(c *C) {
	...
	res, err := confdbstate.GetViaView(bag, view, []string{"ssid"}, nil, confdb.DefaultVisibility)
	c.Assert(err, IsNil)
...
func (s *confdbTestSuite) TestGetNotFound(c *C) {
	...
	res, err = confdbstate.GetViaView(bag, view, []string{"ssid", "ssids"}, nil, confdb.DefaultVisibility)
	c.Assert(err, FitsTypeOf, &confdb.NoDataError{})
	c.Assert(err, ErrorMatches, fmt.Sprintf(`cannot get "ssid", "ssids" through %s/network/setup-wifi: no data`, s.devAccID))
	c.Check(res, IsNil)

	res, err = confdbstate.GetViaView(bag, view, []string{"other-field"}, nil, confdb.DefaultVisibility)
...
func (s *confdbTestSuite) TestSetView(c *C) {
	...
	err = confdbstate.SetViaView(bag, view, map[string]any{"ssid": "foo"}, confdb.DefaultVisibility)
	c.Assert(err, IsNil)
...
func (s *confdbTestSuite) TestSetNotFound(c *C) {
	...
	err = confdbstate.SetViaView(bag, view, map[string]any{"foo": "bar"}, confdb.DefaultVisibility)
	c.Assert(err, FitsTypeOf, &confdb.NoMatchError{})
...
func (s *confdbTestSuite) TestUnsetView(c *C) {
	...
	err = confdbstate.SetViaView(bag, view, map[string]any{"ssid": nil}, confdb.DefaultVisibility)
	c.Assert(err, IsNil)
...
func (s *confdbTestSuite) TestConfdbstateGetEntireView(c *C) {
	...
	res, err := confdbstate.GetViaView(bag, view, nil, nil, confdb.SecretVisibility)
	c.Assert(err, IsNil)
...
func (s *confdbTestSuite) TestGetTransactionForAPI(c *C) {
	...
	view := s.dbSchema.View("setup-wifi")
	chgID, err := confdbstate.LoadConfdbAsync(s.state, view, []string{"ssid", "ssids"}, map[string]string{"foo": "bar"}, confdb.SecretVisibility)
	c.Assert(err, IsNil)
	c.Assert(s.state.Changes(), HasLen, 1)
	...
	var visibility confdb.Visibility
	c.Assert(loadTask.Get("visibility", &visibility), IsNil)
	c.Assert(visibility, Equals, confdb.SecretVisibility)
...
func (s *confdbTestSuite) TestGetTransactionForAPINoHooks(c *C) {
	...
	chgID, err := confdbstate.LoadConfdbAsync(s.state, view, requests, constraints, confdb.SecretVisibility)
	c.Assert(err, IsNil)
	c.Assert(s.state.Changes(), HasLen, 1)
...
func (s *confdbTestSuite) TestGetTransactionForAPINoHooksError(c *C) {
	...
	view := s.dbSchema.View("setup-wifi")
	chgID, err := confdbstate.LoadConfdbAsync(s.state, view, []string{"ssid"}, nil, confdb.DefaultVisibility)
	c.Assert(err, IsNil)
	c.Assert(s.state.Changes(), HasLen, 1)
...
func (s *confdbTestSuite) TestConcurrentAccessWithOngoingWrite(c *C) {
	...
	view := s.dbSchema.View("setup-wifi")
	// reading from API
	_, err = confdbstate.LoadConfdbAsync(s.state, view, []string{"ssid"}, nil, confdb.DefaultVisibility)
...
func (s *confdbTestSuite) TestReadCoveringEphemeralMustDefineLoadViewHook(c *C) {
	...
	defer s.state.Unlock()
	// can't read an ephemeral path w/o a load-view hook
	_, err = confdbstate.LoadConfdbAsync(s.state, view, []string{"eph"}, nil, confdb.DefaultVisibility)
	c.Assert(err, ErrorMatches, fmt.Sprintf("cannot schedule tasks to access %s/network/setup-wifi: read might cover ephemeral data but no custodian has a load-view hook", s.devAccID))
...
func (s *confdbTestSuite) TestBadPathHookChecks(c *C) {
	...
	_, err = confdbstate.LoadConfdbAsync(s.state, view, []string{"foo"}, nil, confdb.DefaultVisibility)
	c.Assert(err, ErrorMatches, fmt.Sprintf(`cannot get "foo" through %s/network/setup-wifi: no matching rule`, s.devAccID))
...
func (s *confdbTestSuite) TestGetTransactionWithSecretVisibility(c *C) {
	...
	view := s.dbSchema.View("setup-wifi")
	chgID, err := confdbstate.LoadConfdbAsync(s.state, view, []string{"private"}, nil, confdb.DefaultVisibility)
	c.Assert(err, IsNil)
	c.Assert(s.state.Changes(), HasLen, 1)
	...
	var visibility confdb.Visibility
	c.Assert(loadTask.Get("visibility", &visibility), IsNil)
	c.Assert(visibility, Equals, confdb.DefaultVisibility)
