...
// applyConfigHandler is the handler for the config-changing hook.
...
// Before is called by the HookManager before the config-changing hook is run.
...
// Done is called by the HookManager after the config-changing hook has exited
// successfully.
func (h *applyConfigHandler) Done() error {
	return nil
}

// Error is called by the HookManager after the config-changing hook has exited
