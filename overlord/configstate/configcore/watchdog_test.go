...
func (s *watchdogSuite) TestConfigureWatchdogAllConfDirExistsAlready(c *C) {
	mylog.
		...
		Check(os.MkdirAll(dirs.SnapSystemdConfDir, 0755))
...
func (s *watchdogSuite) TestConfigureWatchdogNoFileUpdate(c *C) {
	mylog.Check(os.MkdirAll(dirs.SnapSystemdConfDir, 0755))

	times := []int{10, 100}
	content := "[Manager]\n" +
		fmt.Sprintf("RuntimeWatchdogSec=%d\n", times[0]) +
		fmt.Sprintf("ShutdownWatchdogSec=%d\n", times[1])
	mylog.Check(os.WriteFile(s.mockEtcEnvironment, []byte(content), 0644))

	info := mylog.Check2(os.Stat(s.mockEtcEnvironment))
