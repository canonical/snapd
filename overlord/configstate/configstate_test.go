...
func (s *configcoreHijackSuite) SetUpTest(c *C) {
	...
	r := configstate.MockConfigcoreExportExperimentalFlags(func(_ config.ConfReader, _ *config.ApplyOptions) error {
		return nil
	})
	s.AddCleanup(r)
...
func (s *configcoreExportSuite) TestExportHappy(c *C) {
	...
	r := configstate.MockConfigcoreExportExperimentalFlags(func(conf config.ConfReader, opts *config.ApplyOptions) error {
		calls++
