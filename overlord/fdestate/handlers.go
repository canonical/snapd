...
func (m *FDEManager) doAddProtectedKeys(t *state.Task, _ *tomb.Tomb) (err error) {
	...
	var authOpts map[string]*device.VolumesAuthOptions
	// authentication options are only relevant for PINs and passphrases.
	switch authMode {
	case device.AuthModePassphrase, device.AuthModePIN:
		cached := m.state.Cached(keyslotAuthOptionsKey{})
		if cached == nil {
			return errors.New("cannot find authentication options in memory: unexpected snapd restart")
		}
		var ok bool
		authOpts, ok = cached.(map[string]*device.VolumesAuthOptions)
		if !ok {
			return fmt.Errorf("internal error: wrong data type under keyslotAuthOptionsKey: %T", cached)
		}
		for keyslotRefKey, opts := range authOpts {
			if err := opts.Validate(); err != nil {
				return fmt.Errorf("internal error: invalid authentication options for %s: %v", keyslotRefKey, err)
			}
		}
	}
	...
	for _, ref := range missingRefs {
		...
		if !hasParamters {
			return fmt.Errorf("cannot find parameters (key role: %s, container role: %s)", role, ref.ContainerRole)
		}
		...
		params := secboot.ProtectKeyParams{
			...
			KeyRole:                role,
			VolumesAuth:            authOpts[ref.String()],
		}
		if err := secboot.AddContainerTPMProtectedKey(devicePath, ref.Name, &params); err != nil {
			return fmt.Errorf("cannot add passphrase protected key slot: %v", err)
		}
