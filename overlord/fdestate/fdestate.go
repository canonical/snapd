...
func initializeState(st *state.State) error {
	...
	disks, err := devicestate.GetEncryptedDisks(st)
	if err != nil {
	...
	for _, disk := range disks {
		...
		if !hasStatePrimaryKey {
			digest, err := getPrimaryKeyDigest(disk.GetDevPath())
			if err != nil {
				if !errors.Is(err, secboot.ErrKernelKeyNotFound) {
					return fmt.Errorf("cannot obtain primary key digest for data device %s: %w", disk.GetDevPath(), err)
				}
				logger.Noticef("cannot obtain primary key digest for data device %s: %v", disk.GetDevPath(), err)
			} else {
				s.PrimaryKeys[0] = PrimaryKeyInfo{Digest: digest}
			}
		} else {
			sameDigest, err := statePrimaryKey.Digest.verifyPrimaryKeyDigest(disk.GetDevPath())
			if err != nil {
				if !errors.Is(err, secboot.ErrKernelKeyNotFound) {
					return fmt.Errorf("cannot obtain primary key digest for data device %s: %w", disk.GetDevPath(), err)
				}
		...
		for _, keyName := range []string{"default", "default-fallback"} {
			if keyName == "default" && disk.Role == "ubuntu-save" {
				continue
			}
			legacyKey, _ := disk.LegacyKeys[keyName]
			handle, err := secbootGetPCRHandle(disk.GetDevPath(), keyName, legacyKey)
			if err != nil {
				return fmt.Errorf("cannot obtain counter handle for %s (default): %w", disk.GetDevPath(), err)
			}
			var profiles []string
			switch keyName {
			case "default":
