...
func (s *deviceMgrInstallAPISuite) testInstallFinishStep(c *C, opts finishStepOpts) {
		...
		if opts.hasSystemSeed && opts.installClassic {
			...
			restore = installLogic.MockSecbootSaveCheckResult(func(pcc *secboot.PreinstallCheckContext, filename string) error {
				...
				if pcc != preinstallCheckContext {
					err = fmt.Errorf("test error: MockSecbootSaveCheckResult received unexpected check context")
				}
				if !strings.HasSuffix(filename, "run/mnt/ubuntu-save/device/fde/preinstall-check-result") {
					err = fmt.Errorf("test error: MockSecbootSaveCheckResult received unexpected filename %s", filename)
				}
	...
	if opts.encrypted {
		...
		if opts.hasSystemSeed && opts.installClassic {
			// hybrid classic
			expectedFiles = append(expectedFiles, filepath.Join(dirs.RunDir, "mnt/ubuntu-save/device/fde/preinstall-check-result"))
		}
	}
	if opts.hasKernelModsComps {
