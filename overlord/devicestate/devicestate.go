...
func (r *remodeler) maybeInstallOrUpdate(ctx context.Context, st *state.State, rt remodelTarget) (remodelAction, []*state.TaskSet, error) {
	...
	for _, c := range components {
		if snapst.CurrentComponentSideInfo(naming.NewComponentRef(rt.name, c)) == nil {
			needsNewComponents = true
		}
	}
	...
	// do at the moment. afaict, there it is impossible to remodel from a model
	// that requires a snap that is invalid in the incoming model.
...
func (r *remodeler) updateGoal(sn remodelTarget, components []string) (snapstate.UpdateGoal, error) {
	if r.offline {
		...
		for _, c := range components {
			...
			// when installing a local component, we must have all of the
			// assertions that we need. thus, we can't really fall back to an
			// old revision of the snap here safely, since we don't know if all
			// of the required assertions are around.
	...
	return snapstate.StoreUpdateGoal(snapstate.StoreUpdate{
		...
		// sn.components will be the full list of components needed by the new
