...
`
	...
	restore = hookstate.MockRunHook(func(ctx *hookstate.Context, _ *tomb.Tomb) ([]byte, error) {
		if ctx.HookName() == "prepare-device" {
			...
			_, _, _, err := ctlcmd.Run(ctx, []string{"set", fmt.Sprintf("device-service.url=%q", pDBhv.DeviceSvcURL)}, 0, nil)
			c.Assert(err, IsNil)

			if len(pDBhv.Headers) != 0 {
				h, err := json.Marshal(pDBhv.Headers)
				c.Assert(err, IsNil)
				_, _, _, err = ctlcmd.Run(ctx, []string{"set", fmt.Sprintf("device-service.headers=%s", string(h))}, 0, nil)
				c.Assert(err, IsNil)
			}

			if pDBhv.ProposedSerial != "" {
				_, _, _, err = ctlcmd.Run(ctx, []string{"set", fmt.Sprintf("registration.proposed-serial=%q", pDBhv.ProposedSerial)}, 0, nil)
				c.Assert(err, IsNil)
			}
			...
			if len(pDBhv.RegBody) != 0 {
				...
				c.Assert(err, IsNil)
				_, _, _, err = ctlcmd.Run(ctx, []string{"set", fmt.Sprintf("registration.body=%q", d)}, 0, nil)
				c.Assert(err, IsNil)
			}
			...
			stdout, _, _, err := ctlcmd.Run(ctx, []string{"get", "registration.request-id"}, 0, nil)
			c.Assert(err, IsNil)
			c.Assert(string(stdout), Equals, ReqIDPrepareSerialHook+"\n")

			// snapctl set the registration params
			if pSRBhv.RegBody != "" {
				_, _, _, err = ctlcmd.Run(ctx, []string{"set", fmt.Sprintf("registration.body=%s", pSRBhv.RegBody)}, 0, nil)
				c.Assert(err, IsNil)
			}
