...
func MockBootMakeSystemRunnable(f func(model *asserts.Model, bootWith *boot.BootableSet, trusted *boot.TrustedAssets, encryption *boot.EncryptionParameters, bootEntryUpdater boot.BootEntryUpdater) error) (restore func()) {
	restore = testutil.Backup(&bootMakeRunnable)
	bootMakeRunnable = f
	return restore
}

func MockBootMakeSystemRunnableAfterDataReset(f func(model *asserts.Model, bootWith *boot.BootableSet, trusted *boot.TrustedAssets, encryption *boot.EncryptionParameters, bootEntryUpdater boot.BootEntryUpdater) error) (restore func()) {
