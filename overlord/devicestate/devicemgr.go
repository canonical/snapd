...
func (m *DeviceManager) Ensure() error {
	...
	if !m.preseed {
		...
		// A seeded system without an acknowledged device context may still
		// need to register before one can be resolved.
		if seeded && errors.Is(deviceCtxErr, state.ErrNoState) {
