...
func (m *DeviceManager) encryptionSupportInfoUnlocked(constraints install.EncryptionConstraints, encInfoFromCache bool) (*install.EncryptionSupportInfo, error) {
	...
	encryptionSupportInfoFromCache := m.readCacheEncryptionSupportInfoUnlocked(constraints.SystemLabel)

	if constraints.CheckAction != nil {
		// a check action requires only the check context from the cache
		// and need to run the check
		if encryptionSupportInfoFromCache == nil {
			return nil, errors.New("cannot use check action without cached encryption information")
		}
		checkContextFromCache := encryptionSupportInfoFromCache.CheckContext()
		if constraints.CheckContext != nil {
			return nil, errors.New("cannot use check action without cached check context")
		}
		constraints.CheckContext = checkContextFromCache
	} else if encInfoFromCache && encryptionSupportInfoFromCache != nil {
		return encryptionSupportInfoFromCache, nil
	}

	encryptionSupportInfo, err := install.GetEncryptionSupportInfo(constraints, m.runFDESetupHook)
	if err == nil {
		m.refreshCacheEncryptionSupportInfoUnlocked(constraints.SystemLabel, &encryptionSupportInfo)
	}
	return &encryptionSupportInfo, err
}
...
func (m *DeviceManager) encryptionSupportInfoLocked(constraints install.EncryptionConstraints, encInfoFromCache bool) (*install.EncryptionSupportInfo, error) {
	...
	encryptionSupportInfoFromCache := m.readCacheEncryptionSupportInfoLocked(constraints.SystemLabel)
	...
	encryptionSupportInfo, err := install.GetEncryptionSupportInfo(constraints, m.runFDESetupHook)
	if err == nil {
		m.refreshCacheEncryptionSupportInfoLocked(constraints.SystemLabel, &encryptionSupportInfo)
	}
	return &encryptionSupportInfo, err
}
