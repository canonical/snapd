...
func (s *deviceMgrSuite) TestEnsureEarlyBootXKBConfigUpdatedOnHybrid(c *C) {
	...
	s.state.Lock()
	checkExtraSnapdAppends(c, s.state, map[string]string{"xkb": `snapd.xkb="uk,pc105,,"`})
	checkPendingExtraSnapdAppends(c, s.state, true)
	s.state.Set("kcmdline-pending-extra-snapd-appends", false)
	s.state.Unlock()
	c.Check(logbuf.String(), testutil.Contains, `Extra snapd kernel cmdline argument is updated (snapd.xkb="uk,pc105,,")`)
	logbuf.Reset()
	...
	s.state.Lock()
	checkExtraSnapdAppends(c, s.state, map[string]string{"xkb": `snapd.xkb="us,pc104,,"`})
	checkPendingExtraSnapdAppends(c, s.state, true)
	s.state.Set("kcmdline-pending-extra-snapd-appends", false)
	s.state.Unlock()
	c.Check(logbuf.String(), testutil.Contains, `Extra snapd kernel cmdline argument is updated (snapd.xkb="us,pc104,,")`)
	logbuf.Reset()
