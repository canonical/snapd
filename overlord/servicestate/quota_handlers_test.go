...
func (s *quotaHandlersSuite) TestDoQuotaCreate(c *C) {
	r := s.mockSystemctlCalls(c, join(
		// CreateQuota for foo-group
		systemctlCallsForCreateQuota("foo-group", "test-snap"),
	))
	...
	// try to create a quota with no memory limit
	...
	args := servicestate.QuotaControlArgs{
		St:      st,
		AllGrps: map[string]*quota.Group{},
		Action:  qc,
	}

	err := servicestate.DoCreateQuota(args)
	c.Assert(err, ErrorMatches, "internal error, MemoryLimit option is mandatory for create action")

	// with a memory limit but with non-existent snaps we also fail
	args.Action.MemoryLimit = quantity.SizeGiB
	args.Action.AddSnaps = []string{"not-a-snap"}

	err = servicestate.DoCreateQuota(args)
	c.Assert(err, ErrorMatches, `cannot use snap "not-a-snap" in group "foo-group": snap "not-a-snap" is not installed`)

	// with a valid memory limit and a valid snap now it works
	args.Action.AddSnaps = []string{"test-snap"}
	...
	args.AllGrps = allGrps

	// can't create an existing quota group though
	err = servicestate.DoCreateQuota(args)
	c.Assert(err, ErrorMatches, `group \"foo-group\" already exists`)
}
...
func (s *quotaHandlersSuite) TestDoQuotaRemove(c *C) {
	r := s.mockSystemctlCalls(c, join(
		...
		// doRemoveQuota removes the group
		[]expectedSystemctl{{expArgs: []string{"daemon-reload"}}},
		systemctlCallsForSliceStop("foo-group"),
		[]expectedSystemctl{{expArgs: []string{"daemon-reload"}}},
		systemctlCallsForServiceRestart("test-snap"),
	...
	err = servicestate.DoRemoveQuota(args)
	c.Assert(err, IsNil)

	checkQuotaState(c, st, nil)

	// trying to remove the same quota again fails
	err = servicestate.DoRemoveQuota(args)
	c.Assert(err, ErrorMatches, `group "foo-group" does not exist`)
}
