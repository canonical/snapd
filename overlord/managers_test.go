...
`
	...
	ts, err := snapstate.Remove(st, "foo", snap.R(0), false)
	c.Assert(err, IsNil)
...
func (ms *mgrsSuite) removeSnap(c *C, name string) {
	...
	ts, err := snapstate.Remove(st, name, snap.R(0), false)
	c.Assert(err, IsNil)
	chg := st.NewChange("remove-snap", "...")
	chg.AddAll(ts)
...
func (ms *mgrsSuite) TestRemoveAndInstallWithAutoconnectHappy(c *C) {
	...
	ts, err := snapstate.Remove(st, "snap1", snap.R(0), false)
	c.Assert(err, IsNil)
	chg := st.NewChange("remove-snap", "...")
	chg.AddAll(ts)
...
func (ms *mgrsSuite) testUpdateWithAutoconnectRetry(c *C, updateSnapName, removeSnapName string) {
	...
	// remove other-snap
	ts2, err := snapstate.Remove(st, removeSnapName, snap.R(0), false)
	c.Assert(err, IsNil)
...
`
	...
	ts, err := snapstate.Remove(st, "some-snap", snap.R(0), false)
	c.Assert(err, IsNil)
	chg := st.NewChange("uninstall", "...")
	chg.AddAll(ts)

	// remove other-snap
	ts2, err := snapstate.Remove(st, "other-snap", snap.R(0), false)
	c.Assert(err, IsNil)
