...
func (ms *gadgetUpdatesSuite) TestGadgetKernelRefreshFromOldBrokenSnap(c *C) {
	...
	snapstate.SetTestingLeaveOutKernelUpdateGadgetAssets(true)
	defer func() { snapstate.SetTestingLeaveOutKernelUpdateGadgetAssets(false) }()
	affected, tasksets, err := snapstate.UpdateMany(context.TODO(), st, nil, 0, &snapstate.Flags{})
	c.Assert(err, IsNil)
	sort.Strings(affected)
	c.Check(affected, DeepEquals, []string{"pi", "pi-kernel"})
	...
	st.Lock()
	c.Assert(err, IsNil)
	c.Check(chg.Err(), ErrorMatches, "cannot perform the following tasks:\n.*Mount snap \"pi-kernel\" \\(2\\) \\(cannot complete refreshing kernel through an old snapd change without a gadget update task\\)")
}
