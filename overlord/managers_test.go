...
func (s *mgrsSuiteCore) testRemodelUC20WithRecoverySystem(c *C, encrypted bool) {
	...
	restore = fdestate.MockGetPrimaryKeyHash(func(devicePath string, alg crypto.Hash) ([]byte, []byte, error) {
		return []byte("aaaa"), []byte("bbbb"), nil
	})
	defer restore()

	restore = fdestate.MockVerifyPrimaryKeyHash(func(devicePath string, alg crypto.Hash, salt []byte, digest []byte) (bool, error) {
		return true, nil
	})
	defer restore()
