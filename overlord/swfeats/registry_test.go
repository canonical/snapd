...
type swfeatsSuite struct {
	testutil.BaseTest
	ChangeReg *swfeats.ChangeRegistry
...
func (s *swfeatsSuite) SetUpTest(c *C) {
	s.ChangeReg = swfeats.NewChangeRegistry()
	s.EnsureReg = swfeats.NewEnsureRegistry()
}

func (s *swfeatsSuite) TestNewChangeKind(c *C) {
	changeKind := s.ChangeReg.NewChangeKind("my-change")
	c.Assert(changeKind, Equals, "my-change")
}

func (s *swfeatsSuite) TestKnownChangeKinds(c *C) {
	my_change1 := s.ChangeReg.NewChangeKind("my-change1")
	...
	my_change1 = s.ChangeReg.NewChangeKind("my-change1")
	c.Assert(my_change1, Equals, "my-change1")
	my_change2 := s.ChangeReg.NewChangeKind("my-change2")
...
func (s *swfeatsSuite) TestNewChangeTemplateKnown(c *C) {
	changeKind := s.ChangeReg.NewChangeKind("my-change-%s")
	changeKind2 := s.ChangeReg.NewChangeKind("my-change-%s")
	...
	s.ChangeReg.AddPossibleValues(changeKind, []string{"1", "2", "3"})
	kinds = s.ChangeReg.KnownChangeKinds()
...
func (s *swfeatsSuite) TestNewEnsure(c *C) {
	c.Assert(s.EnsureReg.KnownEnsures(), HasLen, 0)
	s.EnsureReg.NewEnsure("MyManager", "myFunction")
...
func (s *swfeatsSuite) TestDuplicateNewEnsure(c *C) {
	s.EnsureReg.NewEnsure("MyManager", "myFunction1")
	s.EnsureReg.NewEnsure("MyManager", "myFunction1")
	s.EnsureReg.NewEnsure("MyManager", "myFunction2")
	s.EnsureReg.NewEnsure("MyManager", "myFunction2")
	knownEnsures := s.EnsureReg.KnownEnsures()
