...
func (s *snapmgrTestSuite) testUpdateCreatesGCTasks(c *C, expectedDiscards int) {
	...
	ts, err := snapstate.Update(s.state, s.user.ID, &snapstate.UpdateOptions{
		Name: "some-snap",
	})
	c.Assert(err, IsNil)
...
func (s *snapmgrTestSuite) TestSwitchTasks(c *C) {
	...
	ts, err := snapstate.Switch(s.state, &snapstate.SwitchOptions{
		Name:    "some-snap",
...
func (s *snapmgrTestSuite) TestSwitchConflict(c *C) {
	...
	_, err = snapstate.Switch(s.state, &snapstate.SwitchOptions{
		Name:    "some-snap",
...
func (s *snapmgrTestSuite) TestDoUpdateHadSlots(c *C) {
	...
	ts, err := snapstate.Update(s.state, 0, &snapstate.UpdateOptions{
		Name: "some-snap",
	})
	c.Assert(err, IsNil)
...
func (s *snapmgrTestSuite) TestUpdateTasksPropagatesErrors(c *C) {
	...
	_, err := snapstate.Update(s.state, s.user.ID, &snapstate.UpdateOptions{
		Name:    "some-snap",
		Channel: "some-channel",
	})
...
func (s *snapmgrTestSuite) TestUpdateUnderDeviceContext(c *C) {
	...
	ts, err := snapstate.UpdateUnderDeviceContext(s.state, s.user.ID, &snapstate.UpdateOptions{Name: "some-snap", Channel: "some-channel"}, deviceCtx)
	c.Assert(err, IsNil)
	verifyUpdateTasks(c, unlinkBefore|cleanupAfter|doesReRefresh, 0, ts, s.state)
	c.Assert(s.state.TaskCount(), Equals, len(ts.Tasks()))
...
func (s *snapmgrTestSuite) TestUpdateUnderDeviceContextToRevision(c *C) {
	...
	ts, err := snapstate.UpdateUnderDeviceContext(s.state, 0, &snapstate.UpdateOptions{
		Name:     "some-snap",
		Channel:  "some-channel",
		Revision: snap.R(11),
	}, deviceCtx)
...
func (s *snapmgrTestSuite) TestUpdateClassicFromClassic(c *C) {
	...
	ts, err = snapstate.Update(s.state, 0, &snapstate.UpdateOptions{
		Name:  "some-snap",
		Flags: snapstate.Flags{JailMode: true},
	})
	c.Assert(err, IsNil)
	c.Assert(ts.Tasks(), Not(HasLen), 0)
	snapsup, err = snapstate.TaskSnapSetup(ts.Tasks()[0])
	...
	ts, err = snapstate.Update(s.state, 0, &snapstate.UpdateOptions{
		Name:  "some-snap",
		Flags: snapstate.Flags{Classic: true, JailMode: true},
	})
	c.Assert(err, IsNil)
	c.Assert(ts.Tasks(), Not(HasLen), 0)
	snapsup, err = snapstate.TaskSnapSetup(ts.Tasks()[0])
...
func (s *snapmgrTestSuite) TestUpdateStrictFromClassic(c *C) {
	...
	_, err := snapstate.Update(s.state, 0, &snapstate.UpdateOptions{
