...
func (s *snapmgrTestSuite) TestInstallDevModeConfinementFiltering(c *C) {
	...
	_, err := snapstate.Install(nil, s.state, "some-snap", opts, s.user.ID, snapstate.Flags{})
	c.Assert(err, ErrorMatches, `.* requires devmode or confinement override`)

	// if a snap is devmode, you *can* install it with --devmode
	_, err = snapstate.Install(nil, s.state, "some-snap", opts, s.user.ID, snapstate.Flags{DevMode: true})
	c.Assert(err, IsNil)
...
func (s *snapmgrTestSuite) TestInstallClassicConfinementFiltering(c *C) {
	...
	_, err = snapstate.Install(nil, s.state, "some-snap", opts, s.user.ID, snapstate.Flags{Classic: true})
	c.Assert(err, IsNil)
...
func (s *snapmgrTestSuite) TestInstallTasks(c *C) {
	...
	ts, err := snapstate.Install(nil, s.state, "some-snap", opts, 0, snapstate.Flags{})
	c.Assert(err, IsNil)
...
func (s *snapmgrTestSuite) TestDoInstallWithSlots(c *C) {
	...
	ts, err := snapstate.Install(nil, s.state, "snap-content-slot", nil, 0, snapstate.Flags{})
	c.Assert(err, IsNil)
...
func (s *snapmgrTestSuite) TestDoInstallChannelDefault(c *C) {
	...
	ts, err := snapstate.Install(nil, s.state, "some-snap", nil, 0, snapstate.Flags{})
	c.Assert(err, IsNil)
...
func (s *snapmgrTestSuite) TestInstallTooEarly(c *C) {
	...
	_, err := snapstate.Install(nil, s.state, "some-snap", nil, 0, snapstate.Flags{})
	c.Check(err, FitsTypeOf, &snapstate.ChangeConflictError{})
	c.Assert(err, ErrorMatches, `too early for operation, device not yet seeded or device model not acknowledged`)
}
...
func (s *snapmgrTestSuite) TestInstallAliasConflict(c *C) {
	...
	_, err := snapstate.Install(nil, s.state, "foo", nil, 0, snapstate.Flags{})
...
func (s *snapmgrTestSuite) TestInstallStrictIgnoresClassic(c *C) {
	...
	ts, err := snapstate.Install(nil, s.state, "some-snap", opts, s.user.ID, snapstate.Flags{Classic: true})
	c.Assert(err, IsNil)
...
func (s *snapmgrTestSuite) TestParallelInstanceInstallNotAllowed(c *C) {
	...
	_, err := snapstate.Install(nil, s.state, "core_foo", nil, 0, snapstate.Flags{})
	c.Check(err, ErrorMatches, `cannot install snap of type os as "core_foo"`)

	_, err = snapstate.Install(nil, s.state, "some-base_foo", nil, 0, snapstate.Flags{})
	c.Check(err, ErrorMatches, `cannot install snap of type base as "some-base_foo"`)

	_, err = snapstate.Install(nil, s.state, "some-gadget_foo", nil, 0, snapstate.Flags{})
	c.Check(err, ErrorMatches, `cannot install snap of type gadget as "some-gadget_foo"`)

	_, err = snapstate.Install(nil, s.state, "some-kernel_foo", nil, 0, snapstate.Flags{})
	c.Check(err, ErrorMatches, `cannot install snap of type kernel as "some-kernel_foo"`)

	_, err = snapstate.Install(nil, s.state, "some-snapd_foo", nil, 0, snapstate.Flags{})
...
func (s *snapmgrTestSuite) TestInstallRunThrough(c *C) {
	...
	opts := &snapstate.RevisionOptions{Channel: "some-channel"}
	ts, err := snapstate.Install(nil, s.state, "some-snap", opts, s.user.ID, snapstate.Flags{})
	c.Assert(err, IsNil)
	chg.AddAll(ts)
...
func (s *snapmgrTestSuite) TestParallelInstanceInstallRunThrough(c *C) {
	...
	chg := s.state.NewChange("install", "install a snap")
	opts := &snapstate.RevisionOptions{Channel: "some-channel"}
	ts, err := snapstate.Install(nil, s.state, "some-snap_instance", opts, s.user.ID, snapstate.Flags{})
	c.Assert(err, IsNil)
	chg.AddAll(ts)
...
func (s *snapmgrTestSuite) TestInstallStartOrder(c *C) {
	...
	chg := s.state.NewChange("install", "install a snap")
	opts := &snapstate.RevisionOptions{Channel: "some-channel"}
	ts, err := snapstate.Install(nil, s.state, "services-snap", opts, s.user.ID, snapstate.Flags{})
	c.Assert(err, IsNil)
	chg.AddAll(ts)
...
func (s *snapmgrTestSuite) TestInstallWithoutCoreTwoSnapsRunThrough(c *C) {
	...
	opts := &snapstate.RevisionOptions{Channel: "some-channel", Revision: snap.R(42)}
	ts1, err := snapstate.Install(nil, s.state, "snap1", opts, s.user.ID, snapstate.Flags{})
	c.Assert(err, IsNil)
	chg1.AddAll(ts1)
	...
	ts2, err := snapstate.Install(nil, s.state, "snap2", opts, s.user.ID, snapstate.Flags{})
	c.Assert(err, IsNil)
	chg2.AddAll(ts2)
...
func (s *snapmgrTestSuite) TestInstallDefaultProviderRunThrough(c *C) {
	...
	ts, err := snapstate.Install(nil, s.state, "snap-content-plug", opts, s.user.ID, snapstate.Flags{})
	c.Assert(err, IsNil)
	chg.AddAll(ts)
...
func (s *snapmgrTestSuite) TestInstallDefaultProviderCircular(c *C) {
	...
	chg := s.state.NewChange("install", "install a snap")
	opts := &snapstate.RevisionOptions{Channel: "some-channel", Revision: snap.R(42)}
	ts, err := snapstate.Install(nil, s.state, "snap-content-circular1", opts, s.user.ID, snapstate.Flags{})
	c.Assert(err, IsNil)
	chg.AddAll(ts)
...
func (s *snapmgrTestSuite) TestInstallDefaultProviderCompat(c *C) {
	...
	chg := s.state.NewChange("install", "install a snap")
	opts := &snapstate.RevisionOptions{Channel: "some-channel", Revision: snap.R(42)}
	ts, err := snapstate.Install(nil, s.state, "snap-content-plug-compat", opts, s.user.ID, snapstate.Flags{})
	c.Assert(err, IsNil)
	chg.AddAll(ts)
...
func (s *snapmgrTestSuite) TestNoConfigureForBasesTask(c *C) {
	...
	ts, err = snapstate.Install(nil, s.state, "some-base", opts, s.user.ID, snapstate.Flags{})
	c.Assert(err, IsNil)
...
func (s *snapmgrTestSuite) TestNoSnapdSnapOnSystemsWithoutBaseOnUbuntuCore(c *C) {
	...
	_, err := snapstate.Install(nil, s.state, "snapd", nil, s.user.ID, snapstate.Flags{})
...
func (s *snapmgrTestSuite) TestNoConfigureForSnapdSnap(c *C) {
	...
	// but snapd do not for install
	ts, err := snapstate.Install(nil, s.state, "snapd", nil, s.user.ID, snapstate.Flags{})
	c.Assert(err, IsNil)
	c.Check(hasConfigureTask(ts), Equals, false)
...
func (s *snapmgrTestSuite) TestRequestSalt(c *C) {
	...
	_, err := snapstate.Install(nil, s.state, "some-snap", nil, s.user.ID, snapstate.Flags{})
	c.Assert(err, ErrorMatches, "internal error: request salt is unset")

	s.state.Set("refresh-privacy-key", "privacy-key")

	chg := s.state.NewChange("install", "install a snap")
	ts, err := snapstate.Install(nil, s.state, "some-snap", nil, s.user.ID, snapstate.Flags{})
	c.Assert(err, IsNil)
	chg.AddAll(ts)
...
func (s *snapmgrTestSuite) TestInstallValidatesInstanceNames(c *C) {
	...
	_, err := snapstate.Install(nil, s.state, "foo--invalid", nil, 0, snapstate.Flags{})
	c.Assert(err, ErrorMatches, `invalid instance name: invalid snap name: "foo--invalid"`)

	_, err = snapstate.Install(nil, s.state, "foo_123_456", nil, 0, snapstate.Flags{})
...
func (s *snapmgrTestSuite) TestGadgetUpdateTaskAddedOnInstall(c *C) {
	...
	ts, err := snapstate.Install(nil, s.state, "brand-gadget", nil, 0, snapstate.Flags{})
	c.Assert(err, IsNil)
