...
func (s *prereqSuite) TestDoPrereqWithBaseNone(c *C) {
	...
	for _, t := range chg.Tasks() {
		if t.Kind() == "link-snap" {
			...
			linkedSnaps = append(linkedSnaps, snapsup.InstanceName().TODOInstanceName())
...
func (s *prereqSuite) TestDoPrereqSkipsSameChangeInFlightPrereq(c *C) {
	s.runner.AddHandler("link-snap", func(task *state.Task, _ *tomb.Tomb) error {
		...
		snapstate.Get(st, snapsup.InstanceName().TODOInstanceName(), &snapst)
		...
		snapstate.Set(st, snapsup.InstanceName().TODOInstanceName(), &snapst)
