...
func (sc *snapInstallChoreographer) UpToLinkSnapAndBeforeReboot(st *state.State, s *taskChainSpan, ic installContext) ([]*state.Task, error) {
	...
	if sc.runRefreshHooks() {
		...
		// further down.
		hook := SetupPreRefreshHook(st, sc.snapsup.InstanceName().TODOInstanceName())
		s.Append(hook)
	}
...
func (sc *snapInstallChoreographer) AfterLinkSnapAndPostReboot(st *state.State, s *taskChainSpan, ic installContext) ([]*state.Task, error) {
	...
	if shouldScheduleUpdateCertDBForRefresh(sc.snapsup.InstanceName().TODOInstanceName(), sc.snapsup.Type, ic.DeviceCtx) {
	...
	if sc.snapsup.QuotaGroupName != "" {
		quotaAddSnapTask, err := AddSnapToQuotaGroup(st, sc.snapsup.InstanceName().TODOInstanceName(), sc.snapsup.QuotaGroupName)
		if err != nil {
			return nil, err
		}
	...
	if !sc.snapst.IsInstalled() && isDefaultConfigureAllowed(sc.snapsup) {
		defaultCfg := DefaultConfigure(st, sc.snapsup.InstanceName().TODOInstanceName())
	...
	if isConfigureAllowed(sc.snapsup) {
		...
		configSet := ConfigureSnap(st, sc.snapsup.InstanceName().TODOInstanceName(), confFlags)
		s.AppendTSWithoutData(configSet)
	}

	healthCheck := CheckHealthHook(st, sc.snapsup.InstanceName().TODOInstanceName(), sc.snapsup.Revision())
...
func (sc *snapInstallChoreographer) addLinkComponentThroughHooks(
	...
	if sc.runRefreshHooks() {
		hook := SetupPostRefreshHook(st, sc.snapsup.InstanceName().TODOInstanceName())
		s.Append(hook)
	}

	if !sc.snapst.IsInstalled() {
		// only run install hook if installing the snap for the first time
		hook := SetupInstallHook(st, sc.snapsup.InstanceName().TODOInstanceName())
		s.Append(hook)
...
func (sc *snapInstallChoreographer) addCleanupTasks(st *state.State, s *taskChainSpan, ic installContext) error {
	...
	for i := currentIndex + 1; i < len(seq); i++ {
		...
		ts, err := removeInactiveRevision(st, sc.snapst, sc.snapsup.InstanceName().TODOInstanceName(), si.Snap.SnapID, si.Snap.Revision, sc.snapsup.Type)
		if err != nil {
			return err
		}
	...
	for i := 0; i <= currentIndex-retain; i++ {
		...
		si := seq[i]
		if inUse(sc.snapsup.InstanceName().TODOInstanceName(), si.Snap.Revision) {
			continue
		}
		ts, err := removeInactiveRevision(st, sc.snapst, sc.snapsup.InstanceName().TODOInstanceName(), si.Snap.SnapID, si.Snap.Revision, sc.snapsup.Type)
		if err != nil {
...
func doInstallOrPreDownload(st *state.State, snapst *SnapState, snapsup *SnapSetup, compsups []ComponentSetup, ic installContext) (snapInstallTaskSet, error) {
	...
	// caller to handle
	if busyErr != nil {
		existing, err := findTasksMatchingKindAndSnap(st, "pre-download-snap", snapsup.InstanceName().TODOInstanceName(), snapsup.Revision())
		if err != nil {
			return snapInstallTaskSet{}, err
		}
	...
	if experimentalGateAutoRefreshHook && snapst.IsInstalled() {
		...
		if err := resetGatingForRefreshed(st, snapsup.InstanceName().TODOInstanceName()); err != nil {
			return snapInstallTaskSet{}, err
		}
	}
...
func checkInstallPreconditions(st *state.State, snapst *SnapState, snapsup *SnapSetup, ic installContext) error {
	...
	if !snapst.IsInstalled() {
		if err := checkSnapAliasConflict(st, snapsup.InstanceName().TODOInstanceName()); err != nil {
			return err
		}
	}
	...
	if err := checkChangeConflictIgnoringOneChange(st, snapsup.InstanceName().TODOInstanceName(), snapst, ic.ConflictOptions); err != nil {
		return err
	}
...
func findTasksMatchingKindAndSnap(st *state.State, kind string, snapName string, revision snap.Revision) ([]*state.Task, error) {
	...
	for _, t := range st.Tasks() {
		...
		if snapsup.InstanceName().TODOInstanceName() == snapName && snapsup.Revision() == revision {
...
func isDefaultConfigureAllowed(snapsup *SnapSetup) bool {
	return isConfigureAllowed(snapsup) && !isCoreSnap(snapsup.InstanceName().TODOInstanceName())
}
...
func configureSnapFlags(snapst *SnapState, snapsup *SnapSetup) int {
	...
	if !snapst.IsInstalled() && hasSnapID && !isCoreSnap(snapsup.InstanceName().TODOInstanceName()) {
