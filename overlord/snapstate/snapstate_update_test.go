...
func (s *snapmgrTestSuite) testAutoRefreshRecordsFailures(c *C, afterReboot bool) {
	...
	getRefreshFailed := func(chg *state.Change) []interface{} {
		...
		sort.Slice(refreshFailed, func(i, j int) bool {
			return refreshFailed[i].(string) < refreshFailed[j].(string)
		})
	...
	c.Check(getRefreshFailures(badSnap).ToRevision, Equals, badSnapRevision)
