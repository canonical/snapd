...
func (s *autoRefreshTestSuite) TestAutoRefreshSplitRefresh(c *C) {
	s.state.Lock()
	defer s.state.Unlock()
	...
	bases := map[string]string{
		// create an implicit dependency between the two task sets
	...
	for _, sn := range snaps {
		...
		path, info := snaptest.MakeTestSnapInfoWithFiles(c, yaml, nil, newSi)
	...
	// don't start with snapd because everything depends on it
	t := findTaskForSnap(c, chg, "prerequisites", "core18")
	waitChain, err := getWaitChain(t)
	c.Assert(err, IsNil)
	c.Assert(waitChain, testutil.DeepUnsortedMatches, []string{"core18", "some-snap-with-core18-base", "gadget", "kernel"})

	t = findTaskForSnap(c, chg, "prerequisites", "some-base")
	waitChain, err = getWaitChain(t)
	c.Assert(err, IsNil)
	c.Assert(waitChain, DeepEquals, []string{"some-base", "some-base-snap"})
}

func getWaitChain(t *state.Task) ([]string, error) {
	var waitChain []string
	set := map[string]bool{t.ID(): true}
	queue := []*state.Task{t}
	...
	for len(queue) > 0 {
		...
		if t.Kind() == "prerequisites" {
			...
			waitChain = append(waitChain, snapsup.InstanceName())
		}

		for _, next := range t.HaltTasks() {
			if !set[next.ID()] {
				set[next.ID()] = true
