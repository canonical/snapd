...
func (m *SnapManager) doMountSnap(t *state.Task, _ *tomb.Tomb) error {
	...
	if snapsup.IntegrityDataParams != nil {
		setupOpts.IntegrityRootHash = snapsup.IntegrityDataParams.Digest
	}
...
func (m *SnapManager) doDownloadIntegrityData(t *state.Task, tomb *tomb.Tomb) error {
	...
	timings.Run(perfTimings, "download", fmt.Sprintf("download integrity data for snap %q", snapsup.SnapName()), func(timings.Measurer) {
		err = theStore.Download(ctx, snapsup.SnapName(), targetFn, &snapsup.IntegrityDataParams.DownloadInfo, meter, user, dlOpts)
	})
