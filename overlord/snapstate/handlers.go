...
func (m *SnapManager) doSetAutoAliasesV2(t *state.Task, _ *tomb.Tomb) error {
	...
	newAliases, nAuto, err := refreshAliases(st, curInfo, curAliases)
	if err != nil {
		return err
	}
...
func (m *SnapManager) doRemoveAliasesV2(t *state.Task, _ *tomb.Tomb) error {
	...
	snapst.AliasesStatus = snapst.AliasesStatus.ToPending()
	Set(st, snapName, snapst)
	return nil
}
...
func (m *SnapManager) doSetupAliasesV2(t *state.Task, _ *tomb.Tomb) error {
	...
	snapName := snapsup.Name()
	status := snapst.AliasesStatus.FromPending()
	curAliases := snapst.Aliases

	err = applyAliasesChange(st, snapName, DisabledAliases, nil, status, curAliases, m.backend)
	if err != nil {
		return err
	}
...
func (m *SnapManager) undoRefreshAliasesV2(t *state.Task, _ *tomb.Tomb) error {
	...
	status := curStatus
	if oldStatus.NoAutoYet() {
		// old state represented not having any auto aliases yet at all
		status = oldStatus
	}
