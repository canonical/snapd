...
func (s *setupSuite) SetUpTest(c *C) {
	...
	err := os.MkdirAll(filepath.Join(dirs.GlobalRootDir, "etc", "systemd", "system", "multi-user.target.wants"), 0o755)
	c.Assert(err, IsNil)
...
func (s *setupSuite) TestSetupAndRemoveKernelSnapSetup(c *C) {
	...
	c.Assert(os.WriteFile(filepath.Join(fwdir, "bar.bin"), []byte{}, 0o644), IsNil)
...
func createKModsComps(c *C, idx, num int, ksnap string, kernRev snap.Revision) []*snap.ComponentSideInfo {
	...
	for i := range comps {
		...
		c.Assert(os.MkdirAll(snapCompForRevDir, 0o755), IsNil)
...
func (s *setupSuite) testSetupAndRemoveKernelModulesComponentsWithModulesInSnapData(c *C, withKernelYaml bool) {
	...
	if withKernelYaml {
		...
		c.Assert(os.MkdirAll(metadir, 0o755), IsNil)
	...
	c.Assert(os.MkdirAll(kernModsDir, 0o755), IsNil)
