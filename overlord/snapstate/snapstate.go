...
// to the pre-reboot or post-reboot taskSequence, so the taskSequence is provided by the caller.
...
func (sb *snapInstallBuilder) BeforeLocalSystemMod(st *state.State, s *taskSequence, ic installContext) error {
...
func (sb *snapInstallBuilder) BeforeReboot(st *state.State, s *taskSequence, ic installContext) error {
	revisionIsLocal := sb.snapst.LastIndex(sb.snapsup.Revision()) >= 0
	revisionStr := fmt.Sprintf(" (%s)", sb.snapsup.Revision())
...
func (sb *snapInstallBuilder) PostReboot(st *state.State, s *taskSequence, ic installContext) error {
	revisionStr := fmt.Sprintf(" (%s)", sb.snapsup.Revision())
...
func (sb *snapInstallBuilder) build(st *state.State, ic installContext) (snapInstallTaskSet, *state.TaskSet, error) {
	...
	beforeReboot := b.NewTaskSequence()
	if err := sb.BeforeReboot(st, &beforeReboot, ic); err != nil {
		return snapInstallTaskSet{}, nil, err
	}

	postReboot := b.NewTaskSequence()
...
func (sb *snapInstallBuilder) addCleanupTasks(st *state.State, s *taskSequence, ic installContext) error {
