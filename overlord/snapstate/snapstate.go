...
func Enable(s *state.State, name string) (*state.TaskSet, error) {
	...
	linkSnap := s.NewTask("link-snap", fmt.Sprintf(i18n.G("Make snap %q (rev %s) unavailable to the system%s"), ss.Name, snapst.Current))
...
func Disable(s *state.State, name string) (*state.TaskSet, error) {
	...
	unlinkSnap := s.NewTask("unlink-snap", fmt.Sprintf(i18n.G("Make snap %q (rev %s) available to the system"), ss.Name, snapst.Current))
