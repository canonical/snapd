...
func (s *snapmgrTestSuite) TestClearSnapDataStopsMountControlMountsLastRevision(c *C) {
	s.state.Lock()
	defer s.state.Unlock()

	// Single revision; removing it stops all mount-control mounts for the snap.
	snapstate.Set(s.state, "some-snap", &snapstate.SnapState{
		Active: true,
		Sequence: snapstatetest.NewSequenceFromSnapSideInfos([]*snap.SideInfo{
	...
	// doClearSnapData calls StopMountUnits once when removing the last revision.
	c.Check(s.fakeBackend.ops.Count("stop-mount-units"), Equals, 1)
}

func (s *snapmgrTestSuite) TestClearSnapDataStopsMountControlMountsSingleRevision(c *C) {
	s.state.Lock()
	defer s.state.Unlock()

	// Two revisions present; removing only rev3 stops its revision-specific mounts.
	snapstate.Set(s.state, "some-snap", &snapstate.SnapState{
		Active: true,
		Sequence: snapstatetest.NewSequenceFromSnapSideInfos([]*snap.SideInfo{
	...
	// doClearSnapData calls StopMountUnits once for the removed revision.
	c.Check(s.fakeBackend.ops.Count("stop-mount-units"), Equals, 1)
}
