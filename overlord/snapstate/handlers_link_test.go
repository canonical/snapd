...
func checkHasCookieForSnap(c *C, st *state.State, instanceName string) {
	...
	mylog.Check(st.Get("snap-cookies", &contexts))
...
func (s *linkSnapSuite) TestDoLinkSnapSuccessNoUserID(c *C) {
	...
	mylog.Check(s.state.Get("snaps", &snaps))
...
func (s *linkSnapSuite) TestDoLinkSnapSuccessUserIDAlreadySet(c *C) {
	...
	// the user
	user := mylog.Check2(auth.NewUser(s.state, auth.NewUserParams{
		Username:   "username",
...
func (s *linkSnapSuite) TestDoLinkSnapSeqFile(c *C) {
	...
	seqContent := mylog.Check2(os.ReadFile(filepath.Join(dirs.SnapSeqDir, "foo.json")))
...
func (s *linkSnapSuite) TestDoUndoLinkSnap(c *C) {
	...
	lp := &testLinkParticipant{
		linkageChanged: func(st *state.State, snapsup *snapstate.SnapSetup) error {
			var snapst snapstate.SnapState
			mylog.Check(snapstate.Get(st, snapsup.InstanceName(), &snapst))
			linkChangeCount++
	...
	mylog.Check(config.SetSnapConfig(s.state, "foo", &cfg))
	...
	mylog.Check(s.state.Get("config", &config))
...
func (s *linkSnapSuite) TestDoUndoUnlinkCurrentSnapWithVitalityScore(c *C) {
	...
	mylog.Check(config.SetSnapConfig(s.state, "core", &cfg))
...
func (s *linkSnapSuite) TestDoLinkSnapSuccessCoreRestarts(c *C) {
	...
	var snapst snapstate.SnapState
	mylog.Check(snapstate.Get(s.state, "core", &snapst))
...
func (s *linkSnapSuite) TestLinkSnapSetsLastRefreshTime(c *C) {
	...
	now := mylog.Check2(time.Parse(time.RFC3339, "2021-07-20T10:00:00Z"))
...
func (s *linkSnapSuite) TestLinkSnapUpdatesLastRefreshTime(c *C) {
	...
	lastRefresh := mylog.Check2(time.Parse(time.RFC3339, "2021-02-20T10:00:00Z"))
...
func (s *linkSnapSuite) TestDoUndoLinkSnapRestoresLastRefreshTime(c *C) {
	...
	lastRefresh := mylog.Check2(time.Parse(time.RFC3339, "2021-06-10T10:00:00Z"))
...
func (s *linkSnapSuite) TestMaybeUndoRemodelBootChangesUnrelatedAppDoesNothing(c *C) {
	...
	restartRequested, _ := mylog.Check3(s.snapmgr.MaybeUndoRemodelBootChanges(t))
...
func (s *linkSnapSuite) TestMaybeUndoRemodelBootChangesNeedsUndo(c *C) {
	...
	restartRequested, rebootRequired := mylog.Check3(s.snapmgr.MaybeUndoRemodelBootChanges(t))
