...
func (s *linkSnapSuite) TestDoLinkSnapSeqFile(c *C) {
	...
	c.Assert(err, IsNil)
	c.Check(string(seqContent), Equals, `{"sequence":[{"name":"foo","snap-id":"","revision":"11"},{"name":"foo","snap-id":"","revision":"33"}],"current":"33","migrated-hidden":false,"use-exposed-dir":false}`)
}
...
func (s *linkSnapSuite) TestDoUndoLinkSnap(c *C) {
	...
	seqContent, err := ioutil.ReadFile(filepath.Join(dirs.SnapSeqDir, "foo.json"))
	c.Assert(err, IsNil)
	c.Check(string(seqContent), Equals, `{"sequence":[],"current":"unset","migrated-hidden":false,"use-exposed-dir":false}`)
