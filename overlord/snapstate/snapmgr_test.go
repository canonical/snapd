...
func (s *snapmgrTestSuite) TestInstallPathWithSideInfoValid(c *C) {
	sideInfoYaml := `{
  "name":"official-mock",
  "snap-id":"somesnapid",
  "revision":17,
  "channel":"beta"
	...
	mockSnap := makeTestSnap(c, `name: mock
version: 1.0`)
	err := ioutil.WriteFile(mockSnap+".sideinfo", []byte(sideInfoYaml), 0644)
	c.Assert(err, IsNil)

	s.state.Lock()
	defer s.state.Unlock()
	ts, err := snapstate.InstallPathWithSideInfo(s.state, mockSnap, "", 0)
	c.Assert(err, IsNil)
	...
	c.Assert(err, IsNil)
	c.Check(ss.SideInfo.OfficialName, Equals, "official-mock")
	c.Check(ss.SideInfo.SnapID, Equals, "somesnapid")
	c.Check(ss.SideInfo.Revision, Equals, snap.R(17))
	c.Check(ss.SideInfo.Channel, Equals, "beta")
}
