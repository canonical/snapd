...
func (s *fdeSetupSuite) TestFdeSetupRequestOpInvalid(c *C) {
	...
	stdout, stderr, _, err := ctlcmd.Run(s.mockContext, []string{"fde-setup-request"}, 0, nil)
...
func (s *fdeSetupSuite) TestFdeSetupRequestNoFdeSetupOpData(c *C) {
	...
	stdout, stderr, _, err := ctlcmd.Run(context, []string{"fde-setup-request"}, 0, nil)
	c.Check(err, ErrorMatches, `cannot use fde-setup-request outside of the fde-setup hook`)
	c.Check(string(stdout), Equals, "")
	c.Check(string(stderr), Equals, "")

	// check "fde-setup-result" error
	stdout, stderr, _, err = ctlcmd.Run(context, []string{"fde-setup-result"}, 0, nil)
...
func (s *fdeSetupSuite) TestFdeSetupResult(c *C) {
	...
	stdout, stderr, _, err := ctlcmd.Run(s.mockContext, []string{"fde-setup-result"}, 0, nil)
	c.Assert(err, IsNil)
	c.Check(string(stdout), Equals, "")
	c.Check(string(stderr), Equals, "")
