...
func (s *rebootSuite) TestBadHook(c *C) {
	...
	_, _, _, err = ctlcmd.Run(ctx, []string{"reboot", "--halt"}, 0, nil)
...
func (s *rebootSuite) TestBadArgs(c *C) {
	...
	for i, t := range table {
		_, _, _, err := ctlcmd.Run(s.mockContext, t.args, 0, nil)
...
func (s *rebootSuite) TestRegularRunHalt(c *C) {
	...
	_, _, _, err := ctlcmd.Run(s.mockContext, []string{"reboot", "--halt"}, 0, nil)
	c.Assert(err, IsNil)
...
func (s *rebootSuite) TestRegularRunPoweroff(c *C) {
	...
	_, _, _, err := ctlcmd.Run(s.mockContext, []string{"reboot", "--poweroff"}, 0, nil)
	c.Assert(err, IsNil)
