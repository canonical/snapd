...
func (s *confdbSuite) TestFailAbortsConfdbTransaction(c *C) {
	...
	stdout, stderr, _, err := ctlcmd.Run(mockContext, []string{"fail", "don't like changes"}, 0, nil)
	c.Assert(err, IsNil)
...
func (s *confdbSuite) TestFailErrors(c *C) {
	...
	stdout, stderr, _, err := ctlcmd.Run(s.mockContext, []string{"fail", "reason"}, 0, nil)
	...
	stdout, stderr, _, err = ctlcmd.Run(s.mockContext, []string{"fail"}, 0, nil)
