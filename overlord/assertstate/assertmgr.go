...
func doValidateSnap(t *state.Task, _ *tomb.Tomb) error {
	...
	db := DB(st)
	verifiedRev, err := snapasserts.CrossCheck(snapsup.InstanceName().TODOInstanceName(), sha3_384, expectedProv, snapSize, snapsup.SideInfo, modelAs, db)
	if err != nil {
...
func fetchAssertsAndValidateComponent(st *state.State, compsup *snapstate.ComponentSetup, snapsup *snapstate.SnapSetup, t *state.Task) error {
	...
	if compPath == "" {
		compPath = compsup.BlobPath(snapsup.InstanceName().TODOInstanceName())
	}
