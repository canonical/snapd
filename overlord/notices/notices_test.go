...
func (s *noticesSuite) TestDoNoticesFilter(c *C) {
	...
	// If passed a nil filter, Before filter should be set according to now
	var given *state.NoticeFilter
	expected := &state.NoticeFilter{Before: now}
	testDoNoticesFilter(c, backends, given, expected, now)

	// If passed a non-nil filter with zero Before, Before filter should be
	// set to now
	...
	// If passed a filter with Before in the past, it should be unchanged.
	given = &state.NoticeFilter{
		Keys:   []string{"foo", "bar"},
	...
	// If passed a filter with Before in the future, it should be set to now.
	given = &state.NoticeFilter{
	...
	// If passed a filter with After in the future, Before will be set as usual.
	// XXX: we allow After to be after Before, and pass the filter on to the
...
func (s *noticesSuite) TestWaitNotices(c *C) {
	...
	// existing notices before the current time, and if one or more already
	// has notices, then return them immediately, once all backends return.
	bknd1.noticesChan <- []*state.Notice{n3, n1}
	beforeTime := time.Now()
	beforeFilterTime := beforeTime.Add(time.Hour)
	...
	filter = &state.NoticeFilter{
		...
		Before: beforeFilterTime,
	}
	result, err = nm.WaitNotices(context.Background(), filter)
	afterTime := time.Now()
	c.Assert(err, IsNil)
	c.Check(result, DeepEquals, []*state.Notice{n1, n2, n3})
	// Check that the backends received a filter with Before set to the current time when called.
	for _, bknd := range []*testNoticeBackend{bknd1, bknd2} {
		...
		c.Check(beforeTime.Before(receivedFilter.Before), Equals, true)
		c.Check(receivedFilter.Before.Before(afterTime), Equals, true)
		c.Check(receivedFilter.Before.Before(beforeFilterTime), Equals, true)
	}
	// Check that the original filter was not overwritten
	c.Check(filter.Before, Equals, beforeFilterTime)

	// If filter matches multiple backends, then each should queried for
	// existing notices before the current time, and if none have notices, and
	// the Before filter is in the past, then return immediately, once all backends return.
	bknd1.noticesChan <- nil
	beforeTime = time.Now()
	...
	filter = &state.NoticeFilter{
		Types:  []state.NoticeType{state.WarningNotice},
		Before: beforeTime, // this is in the past at call time
	}
	result, err = nm.WaitNotices(context.Background(), filter)
	afterTime = time.Now()
	...
	// Check that the backends received a filter with Before unchanged from the original filter.
	for _, bknd := range []*testNoticeBackend{bknd1, bknd2} {
		receivedFilter := <-bknd.noticesFilterChan
		c.Check(receivedFilter, DeepEquals, filter)
	}
	// Check that the original filter was not overwritten
	c.Check(filter.Before, Equals, beforeTime)
	...
	// Before set to the current time when called.
	for _, bknd := range []*testNoticeBackend{bknd1, bknd2} {
		receivedFilter := <-bknd.noticesFilterChan
		c.Check(beforeTime.Before(receivedFilter.Before), Equals, true)
		c.Check(receivedFilter.Before.Before(afterTime), Equals, true)
	...
	// Check that the original filter was not overwritten
	c.Check(filter.Before.IsZero(), Equals, true)
	...
	for _, bknd := range []*testNoticeBackend{bknd1, bknd2} {
		receivedFilter := <-bknd.noticesFilterChan
		c.Check(receivedFilter.Before.IsZero(), Equals, false)
	}
	// Check that the backends received a filter to BackendWaitNotices with
	// Before unset.
	for _, bknd := range []*testNoticeBackend{bknd1, bknd2} {
	...
	// again queried for BackendNotices, but with a Before filter set to
	// one nanosecond after the last return notice's timestamp.
	bknd1.noticesChan <- nil
	bknd2.noticesChan <- nil
	...
	// with Before set to the current time when called.
	for _, bknd := range []*testNoticeBackend{bknd1, bknd2} {
		receivedFilter := <-bknd.noticesFilterChan
		c.Check(beforeTime.Before(receivedFilter.Before), Equals, true)
		c.Check(receivedFilter.Before.Before(afterTime), Equals, true)
	...
	// with Before set to the lastRepeated timestamp of the final notice
	// returned by the calls to BackendWaitNotices.
	for i, bknd := range []*testNoticeBackend{bknd1, bknd2} {
		receivedFilter := <-bknd.noticesFilterChan
		c.Check(receivedFilter.Before.Equal(n4.LastRepeated().Add(time.Nanosecond)), Equals, true, Commentf("Backend %d receivedFilter.Before: %v", i+1, receivedFilter.Before))
	}
	// Check that the original filter was not overwritten
	c.Check(filter.Before.IsZero(), Equals, true)
}
