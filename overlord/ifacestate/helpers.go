...
func (m *InterfaceManager) initialize(extraInterfaces []interfaces.Interface, extraBackends []interfaces.SecurityBackend, tm *timings.Span) error {
	m.state.Lock()
...
func (m *InterfaceManager) regenerateAllSecurityProfiles(tm *timings.Span) error {
	...
	for _, snapInfo := range snaps {
		...
		for _, backend := range securityBackends {
			...
			tm.Run("setup security backend", fmt.Sprintf("setup security backend %q for snap %q", backend.Name(), snapInfo.InstanceName()), func(nesttm *timings.Span) {
...
func (m *InterfaceManager) setupSecurityByBackend(task *state.Task, snaps []*snap.Info, opts []interfaces.ConfinementOptions, tm *timings.Span) error {
	st := task.State()
...
func (m *InterfaceManager) setupSnapSecurity(task *state.Task, snapInfo *snap.Info, opts interfaces.ConfinementOptions, tm *timings.Span) error {
	st := task.State()
