...
func (s *interfaceManagerSuite) mockLinkComponent(c *C) {
	s.o.TaskRunner().AddHandler("mock-link-component-n-witness", func(task *state.Task, tomb *tomb.Tomb) error { // do handler
		...
		var snapst snapstate.SnapState
		err = snapstate.Get(s.state, snapsup.InstanceName().TODOInstanceName(), &snapst)
		if err != nil {
			return err
		}
		...
		snapstate.Set(s.state, snapsup.InstanceName().TODOInstanceName(), &snapst)
...
func (s *interfaceManagerSuite) addSetupSnapSecurityChangeWithOptions(c *C, snapsup *snapstate.SnapSetup, opts setupSnapSecurityChangeOptions) *state.Change {
	...
	s.o.TaskRunner().AddHandler(linkSnapTask, func(task *state.Task, tomb *tomb.Tomb) error { // do handler
		...
		if opts.linkSnapRestarts {
			...
			return restart.FinishTaskWithRestart(task, state.DoneStatus, restart.RestartSystem, snapsup.InstanceName().TODOInstanceName(), nil)
		}
		return nil
	}, func(task *state.Task, tomb *tomb.Tomb) error { // undo handler
		s.state.Lock()
		...
		if opts.install {
			// unlink completely
			snapstate.Set(s.state, snapsup.InstanceName().TODOInstanceName(), nil)
		} else {
			snapst.Active = false
			snapstate.Set(s.state, snapsup.InstanceName().TODOInstanceName(), &snapst)
		}
		...
		if !opts.install {
			...
			c.Assert(snapstate.Get(s.state, snapsup.InstanceName().TODOInstanceName(), &snapst), IsNil)
...
func (s *interfaceManagerSuite) TestShouldUndoSetupProfiles(c *C) {
	...
	c.Check(ifacestate.ShouldUndoSetupProfiles(setupBeforeLink, snapsup.InstanceName().TODOInstanceName()), Equals, true)
	c.Check(ifacestate.ShouldUndoSetupProfiles(setupAfterAutoConnect, snapsup.InstanceName().TODOInstanceName()), Equals, true)
	...
	c.Check(ifacestate.ShouldUndoSetupProfiles(otherPrepareProfiles, otherSnapsup.InstanceName().TODOInstanceName()), Equals, true)
	c.Check(ifacestate.ShouldUndoSetupProfiles(otherSetupProfiles, otherSnapsup.InstanceName().TODOInstanceName()), Equals, false)
}
...
func (s *interfaceManagerSuite) TestDelayedEffectsHandlingOfRestartRequestsNotBreakingEarly(c *C) {
	...
	s.o.TaskRunner().AddHandler("unlink-current-snap", func(task *state.Task, tomb *tomb.Tomb) error {
		...
		return restart.FinishTaskWithRestart(task, state.UndoneStatus, restart.RestartSystem, snapsup.InstanceName().TODOInstanceName(), nil)
	})
