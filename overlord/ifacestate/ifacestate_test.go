...
func (s *interfaceManagerSuite) manager(c *C) *ifacestate.InterfaceManager {
	if s.privateMgr == nil {
		mgr, err := ifacestate.Manager(s.state, s.hookManager(c), s.o.TaskRunner(), s.extraIfaces, s.extraBackends, s.noticeMgr)
		c.Assert(err, IsNil)
...
func (s *interfaceManagerSuite) TestInterfacesRequestsManagerNoHandlerService(c *C) {
	...
	mgr, err := ifacestate.Manager(s.state, nil, s.o.TaskRunner(), nil, nil, nil)
	c.Assert(err, IsNil)
