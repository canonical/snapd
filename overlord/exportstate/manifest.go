...
type Manifest struct {
	...
	SnapRevision              snap.Revision `json:"snap-revision"`
...
// ExportedFile describes a single file exported from one place to another.
//
...
// NewManifestForSnap returns the manifest of a given snap or nil if there is
// no manifest.
//
...
func NewManifestForSnap(info *snap.Info) *Manifest {
	...
	default:
		// TODO: add support for regular snaps
		return nil
	}
}
...
//
// Classic systems have an export manifest which points to the snapd
// tools from the classic (deb/rpm) packages. Ubuntu Core systems do
// not need this, the snapd tools come from the snapd snap only.
...
// exportedFilePath returns the path of an exported file.
