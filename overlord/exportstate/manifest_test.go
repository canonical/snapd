...
func (s *manifestSuite) TestNewManifestForRegularSnap(c *C) {
	...
	m := exportstate.NewManifestForSnap(info)
	// no manifest for regular snaps yet
	c.Assert(m, IsNil)
}
...
func (s *manifestSuite) TestNewManifestForSnapdSnap(c *C) {
	info := snaptest.MockInfo(c, snapdYaml, &snap.SideInfo{Revision: snap.R(1)})
	m := exportstate.NewManifestForSnap(info)
...
func (s *manifestSuite) TestNewManifestForCoreSnap(c *C) {
	info := snaptest.MockInfo(c, coreYaml, &snap.SideInfo{Revision: snap.R(2)})
	m := exportstate.NewManifestForSnap(info)
