...
func (b *fakeStateBackend) EnsureAfter(d time.Duration) {
	b.ensureAfter = d
}
...
func (ss *stateSuite) TestEnsureAfter(c *C) {
	b := new(fakeStateBackend)
	st := state.New(b)

	st.EnsureAfter(10 * time.Second)

	c.Check(b.ensureAfter, Equals, 10*time.Second)
}
