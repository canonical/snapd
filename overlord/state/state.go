...
func (s *State) Prune(operationalTime time.Time, pruneWait, abortWait time.Duration, maxReadyChanges int) {
	now := time.Now()
	...
	for _, chg := range changes {
		...
		if readyTime.IsZero() {
			...
			} else if spawnTime.Before(abortLimit) && spawnTime.After(operationalTime) {
				chg.Abort()
			}
			continue
		}
