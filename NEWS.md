...
* FDE: refactor listing of the disks based on run mode model and model to correctly resolve paths
...
* AppArmor prompting (experimental): add handling for expiration of kernel listener and requests
...
* Non-suid snap-confine: replace snap-confine suid with capabilities to achieve the required permissions
...
* When running snap, on system key mismatch, wait for a pending security profiles regeneration
...
* Modified core-initrd to use systemctl reboot instead of /sbin/reboot
...
* Interfaces: add support for reinitialization in AppArmor security backend
