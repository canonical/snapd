...
// strace and processes its's stderr to identify a moment when strace attached
...
func TraceExecCommandForPid(pid int, straceLogPath string) (*exec.Cmd, error) {
	extraStraceOpts := []string{
		...
		"--silence=attach",                     // silence [ Process NNNN attached ] messages
...
func StraceAttachedStart(s string) bool {
	...
	//
	// where the timestamp's presence depends on the format parameters stace
	// was started with
