...
func (ts *mapperSuite) TestCreateLinearSizeOffsetErr(c *C) {
	for _, tc := range []struct {
		...
		_, err := osutil.CreateLinearMapper("/dev/sda1", "mapper-name", "", tc.offset, tc.size)
...
func (ts *mapperSuite) TestCreateLinearMapperErr(c *C) {
	...
	_, err := osutil.CreateLinearMapper("/dev/sda1", "mapper-name", "", 512, 1024)
...
func (ts *mapperSuite) TestCreateLinearMapperHappy(c *C) {
	...
	mapperDevice, err := osutil.CreateLinearMapper("/dev/sda1", "mapper-name", "", 512, 2048)
	c.Assert(err, IsNil)
