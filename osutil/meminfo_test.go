...
func (s *meminfoSuite) TestMemInfoHappy(c *C) {
	...
	c.Assert(os.WriteFile(p, []byte(meminfoExampleFromLiveSystem), 0o644), IsNil)

	mem, err := osutil.TotalUsableMemory()
	c.Assert(err, IsNil)
	c.Check(mem, Equals, uint64(32876680)*1024)

	c.Assert(os.WriteFile(p, []byte(`MemTotal:    1234 kB`), 0o644), IsNil)
...
MemFree:         3478104 kB
`
	c.Assert(os.WriteFile(p, []byte(meminfoReorderedWithEmptyLine), 0o644), IsNil)

	mem, err = osutil.TotalUsableMemory()
	c.Assert(err, IsNil)
	c.Check(mem, Equals, uint64(32876699)*1024)

	c.Assert(os.WriteFile(p, []byte(meminfoExampleFromPi3), 0o644), IsNil)
...
`
	...
	for _, tc := range []struct {
		...
		c.Assert(os.WriteFile(p, []byte(tc.content), 0o644), IsNil)
		mem, err := osutil.TotalUsableMemory()
