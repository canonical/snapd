...
// RegisterDeviceMapperHandler takes a callback function which is used when the
// disks package through some of it's various methods to locate/create a disk
// needs to trace a device mapper node back to the original device node location
// such as /dev/mapper/something -> /dev/sda1 -> /dev/sda. The parameters the
// callback is provided are the device mapper UUID and name parameters from the
// kernel, and if and only if the device mapper handler matches this device
// mapper node, the callback should return the source device node for the mapper
// device and true. If the handler does not match a provided device mapper, the
// function should return "ok" as false.
