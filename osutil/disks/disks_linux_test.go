...
var (
	...
	mapperDiskUdevPropMap = map[string]string{
		"DEVNAME": "/dev/dm-0",
...
func (s *diskSuite) TestMountPointIsFromDiskForVerityAutoGeneratedMapperDevice(c *C) {
	mockUdevadm := testutil.MockCommand(c, "udevadm", ``)
	defer mockUdevadm.Restore()

	restore := osutil.MockMountInfo(`28 85 253:15 / /boot/efi rw,relatime shared:50 - vfat /dev/vda15 rw
61 59 252:0 / /run/mnt/data ro,relatime shared:20 - ext4 /dev/mapper/libmnt_vda1 ro
`)
	...
	restore = disks.MockUdevPropertiesForDevice(func(typeOpt, dev string) (map[string]string, error) {
		...
		switch n {
		case 1:
			c.Assert(dev, Equals, "/dev/vda15")
			return diskUdevPropMap, nil
		case 2:
			c.Assert(dev, Equals, "/dev/block/42:0")
			return diskUdevPropMap, nil
		case 3:
			if strings.Contains(dev, "/dev/mapper/libmnt") {
				return mapperDiskUdevPropMap, nil
			}
			c.Assert(dev, Equals, "/dev/vda1")
			return diskUdevPropMap, nil
		case 4:
			c.Assert(dev, Equals, "/dev/block/42:0")
			return diskUdevPropMap, nil
	...
	defer restore()
	disk, err := disks.DiskFromMountPoint("/boot/efi", nil)
	c.Assert(err, IsNil)
	c.Assert(disk, Not(IsNil))
	c.Assert(disk.Dev(), Equals, "42:0")

	matches, err := disk.MountPointIsFromDisk("/run/mnt/data", nil)
	c.Assert(err, IsNil)
	c.Assert(matches, Equals, true)
}
