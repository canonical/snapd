...
type Word struct {
	Name  string
	Chain lists.HeadlessList[Word, viaWords]
}
type viaWords struct{}

func (viaWords) Offset(w *Word) uintptr { return unsafe.Offsetof(w.Chain) }
...
func TestHeadlessList_Smoke(t *testing.T) {
	...
	marry.Chain.LinkBefore(&had).LinkBefore(&a).LinkBefore(&little).LinkBefore(&lamb)
