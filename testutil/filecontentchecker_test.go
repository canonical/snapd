...
func (s *fileContentCheckerSuite) TestFileEquals(c *check.C) {
	...
	c.Assert(os.WriteFile(equalRefereceFilename, []byte(content), 0o644), check.IsNil)
	notEqualRefereceFilename := filepath.Join(d, "canary-reference-not-equal")
	c.Assert(os.WriteFile(notEqualRefereceFilename, []byte("not-equal"), 0o644), check.IsNil)
