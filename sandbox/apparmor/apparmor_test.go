...
func (*apparmorSuite) TestAppArmorParser(c *C) {
	...
	c.Check(cmd.Args, DeepEquals, []string{mockParserCmd.Exe(), "--policy-features=<kernel>"})
...
func (s *apparmorSuite) TestProbeAppArmorParserFeatures(c *C) {
	...
	for i := 0; i < int(math.Pow(2, float64(len(features)))); i++ {
		...
		for j, f := range features {
			...
			expectedCalls = append(expectedCalls, []string{"apparmor_parser", "--policy-features=<kernel>", "--preprocess"})
