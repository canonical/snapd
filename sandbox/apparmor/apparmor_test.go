...
func (s *apparmorSuite) TestUpdateHomedirsTunableHappy(c *C) {
	var passedDirPath string
	var passedDirMode os.FileMode
	restore := apparmor.MockMkdirAll(func(path string, mode os.FileMode) error {
		passedDirPath = path
		passedDirMode = mode
		return nil
	})
	...
	var passedFilePath string
	fileContents := make([]byte, 512)
	var passedFileMode os.FileMode
	var passedFlags osutil.AtomicWriteFlags
	restore = apparmor.MockAtomicWrite(func(path string, r io.Reader, mode os.FileMode, flags osutil.AtomicWriteFlags) error {
		passedFilePath = path
		passedFileMode = mode
		passedFlags = flags
		n, err := r.Read(fileContents)
		fileContents = fileContents[0:n]
		c.Assert(err, IsNil)
		return nil
	})
	defer restore()

	baseDir := dirs.GlobalRootDir
	err := apparmor.UpdateHomedirsTunable([]string{"/home/a", "/dir2"})
	c.Assert(err, IsNil)
	c.Check(passedDirPath, Equals, baseDir+"/etc/apparmor.d/tunables/home.d")
	c.Check(passedDirMode, Equals, os.FileMode(0755))
	c.Check(passedFilePath, Equals, baseDir+"/etc/apparmor.d/tunables/home.d/snapd")
	c.Check(passedFileMode, Equals, os.FileMode(0644))
	c.Check(passedFlags, Equals, osutil.AtomicWriteFlags(0))
...
func (s *apparmorSuite) TestUpdateHomedirsTunableHappyNoDirs(c *C) {
	...
	restore := apparmor.MockFileExists(func(path string) bool {
		passedPath = path
		return false
	})
	defer restore()
	...
	c.Check(err, IsNil)
	c.Check(passedPath, Equals, baseDir+"/etc/apparmor.d/tunables/home.d/snapd")
	c.Check(writeCalled, Equals, false)
}
