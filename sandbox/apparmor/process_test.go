...
func (s *apparmorSuite) TestSecurityLabelFromPidUnsupported(c *C) {
	restore := apparmor.MockSecurityLabelProbeLevel(func() apparmor.LevelType {
		return apparmor.Unsupported
	})
	defer restore()

	for _, rel := range []string{"proc/42/attr/apparmor/current", "proc/42/attr/current"} {
		procFile := filepath.Join(s.fakeroot, rel)
		c.Assert(os.MkdirAll(filepath.Dir(procFile), 0755), IsNil)
		c.Assert(os.WriteFile(procFile, []byte("snap.foo.app (enforce)\n"), 0644), IsNil)
	}
