...
func (r *Request) Key() string {
	return fmt.Sprintf("kernel:%16X", r.id)
}
...
func (l *Listener) decodeAndDispatchRequest(buf []byte) error {
	for len(buf) > 0 {
		...
		// Build request, but don't handle error until after we account for
		...
		// new request, so we need to auto-deny it, else the triggering
		// application will just hang until the message times out in the kernel.
		if newReqErr != nil {
			l.denyMalformedRequest(msg)
			return newReqErr
		}
...
func (l *Listener) newRequest(msg notify.MsgNotificationGeneric) (*Request, error) {
	...
	iface, err := prompting.InterfaceFromTagsets(tagsets)
	if err != nil {
		if errors.Is(err, prompting_errors.ErrNoInterfaceTags) {
			...
			return nil, fmt.Errorf("cannot select interface from metadata tags: %v", err)
		}
	}
...
func (l *Listener) denyMalformedRequest(msg notify.MsgNotificationGeneric) {
	aaAllowed, aaDenied, _ := msg.AllowedDeniedPermissions()
	// If error occurs, aaAllowed and aaDenied will be nil, and BuildResponse
	// should handle them. Build response with nil allowed perms to deny all.
	resp := notify.BuildResponse(l.protocolVersion, msg.ID(), aaAllowed, aaDenied, nil)
