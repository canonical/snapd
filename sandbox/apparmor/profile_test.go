...
func (s *appArmorSuite) TestLoadProfilesRunsAppArmorParserReplace(c *C) {
	...
	c.Assert(cmd.Calls(), DeepEquals, [][]string{
		{"apparmor_parser", "--policy-features=<kernel>", "--replace", "--write-cache", "--cache-loc=/var/cache/apparmor", "--quiet", "/path/to/snap.samba.smbd"},
	})
}
...
func (s *appArmorSuite) TestLoadProfilesMany(c *C) {
	...
	c.Assert(err, IsNil)
	c.Assert(cmd.Calls(), DeepEquals, [][]string{
		{"apparmor_parser", "--policy-features=<kernel>", "--replace", "--write-cache", "--cache-loc=/var/cache/apparmor", "--quiet", "/path/to/snap.samba.smbd", "/path/to/another.profile"},
	})
}
...
func (s *appArmorSuite) TestLoadProfilesRunsAppArmorParserReplaceWithSnapdDebug(c *C) {
	...
	c.Assert(err, IsNil)
	c.Assert(cmd.Calls(), DeepEquals, [][]string{
		{"apparmor_parser", "--policy-features=<kernel>", "--replace", "--write-cache", "--cache-loc=/var/cache/apparmor", "/path/to/snap.samba.smbd"},
	})
}
