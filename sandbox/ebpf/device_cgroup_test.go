...
func (s *ebpfSuite) TestDeviceKeyMarshalRoundTrip(c *C) {
	...
	data := key.MarshalBytes()
	c.Assert(data, HasLen, ebpf.DeviceKeySize)

	// Verify raw bytes
	c.Check(data[0], Equals, byte('c'))
	c.Check(binary.LittleEndian.Uint32(data[1:5]), Equals, uint32(1))
	c.Check(binary.LittleEndian.Uint32(data[5:9]), Equals, uint32(3))

	// Round-trip
	var key2 ebpf.DeviceKey
	key2.UnmarshalBytes(data)
...
func (s *ebpfSuite) TestDecodeDeviceDenyEvent(c *C) {
	...
	// padding at [2:4]
	binary.LittleEndian.PutUint32(data[4:8], 1)          // major
	binary.LittleEndian.PutUint32(data[8:12], 7)         // minor
	binary.LittleEndian.PutUint32(data[12:16], 12345)    // pid
	binary.LittleEndian.PutUint64(data[16:24], 99999999) // timestamp
