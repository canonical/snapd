...
// SnapDeviceCgroupOptions represents the options for device cgroup tracking.
type SnapDeviceCgroupOptions struct {
	NonStrict   bool // NonStrict being true implies the application is installed with --devmode
...
func (opts *SnapDeviceCgroupOptions) MarshalText() ([]byte, error) {
	...
	if opts.NonStrict {
		buf.WriteString("# snap uses non-strict confinement.\n")
		buf.WriteString("non-strict=true\n")
	}
...
func (opts *SnapDeviceCgroupOptions) UnmarshalText(b []byte) error {
	...
	for sc.Scan() {
		...
		left, right, ok := bytes.Cut(l, []byte("="))
		if !ok {
			continue
		}
		...
		var bp *bool
		switch {
		case bytes.Equal(left, []byte("self-managed")):
			bp = &opts.SelfManaged
		case bytes.Equal(left, []byte("non-strict")):
			bp = &opts.NonStrict
		default:
			continue
		}

		*bp = bytes.Equal(right, []byte("true"))
	}
