...
var cgroupContentCommon = `#subsys_name	hierarchy	num_cgroups	enabled
cpuset	6	3	1
cpu	3	133	1
devices	10	135	1`
...
func (s *memorySuite) TestCheckMemoryCgroupHappy(c *C) {
	...
	mylog.Check(os.WriteFile(s.mockCgroupsFile, []byte(content), 0644))

	mylog.Check(cgroup.CheckMemoryCgroup())
...
func (s *memorySuite) TestCheckMemoryCgroupMissing(c *C) {
	mylog.
		...
		Check(cgroup.CheckMemoryCgroup())
