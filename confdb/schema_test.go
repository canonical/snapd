...
func (s *schemaSuite) TestPruneMapPlaceholder(c *C) {
	...
	data, err = schema.PruneByVisibility(parsePath(c, "foo.{n}"), confdb.SecretVisibility, data)
	c.Assert(err, IsNil)
...
func (*schemaSuite) TestPruneVisibilityMapSecretValues(c *C) {
	...
	data, err = schema.PruneByVisibility(parsePath(c, "foo"), confdb.SecretVisibility, data)
	c.Assert(err, IsNil)
	c.Assert(data, IsNil)
}
...
func (s *schemaSuite) TestPruneMapPlaceholderLongerPath(c *C) {
	...
	data, err = schema.PruneByVisibility(parsePath(c, "foo.{n}.eph"), confdb.SecretVisibility, data)
	c.Assert(err, IsNil)
	c.Assert(data, IsNil)
}
...
func (*schemaSuite) TestPruneMapPlaceholderNotEndOfPath(c *C) {
	...
	prunedData, err = schema.PruneByVisibility(parsePath(c, "foo.{n}.eph"), confdb.SecretVisibility, data)
	c.Assert(err, IsNil)
...
func (s *schemaSuite) TestPruneMapDataKeyNotFound(c *C) {
	...
	schema, err := confdb.ParseStorageSchema(schemaStr)
	c.Assert(err, IsNil)
	var data any = map[string]any{"eph": "a"}
...
func (*schemaSuite) TestPruneVisibilityMapSecretValuesNested(c *C) {
	...
	data = "a-secret"
	data, err = schema.PruneByVisibility(parsePath(c, "foo.a.bar"), confdb.SecretVisibility, data)
	c.Assert(err, IsNil)
	c.Assert(data, IsNil)
}
...
func (s *schemaSuite) TestPruneMapValueSchemaSecretValues(c *C) {
	...
	data, err = schema.PruneByVisibility(parsePath(c, "foo.{n}.bar"), confdb.SecretVisibility, data)
	c.Assert(err, IsNil)
	c.Assert(data, IsNil)
}
...
func (*schemaSuite) TestPruneVisibilityArrayElements(c *C) {
	...
	data, err = schema.PruneByVisibility(parsePath(c, "foo[1].bar"), confdb.SecretVisibility, data)
	c.Assert(err, IsNil)
...
func (*schemaSuite) TestPruneVisibilityIdenticalPathWithDataAlternatives(c *C) {
	...
	schema, err := confdb.ParseStorageSchema(schemaStr)
	c.Assert(err, IsNil)
	var data any = map[string]any{"bar": map[string]any{"baz": "a"}}
	data, err = schema.PruneByVisibility(parsePath(c, "foo"), confdb.SecretVisibility, data)
	c.Assert(err, IsNil)
	c.Assert(data, IsNil)
}
...
func (*schemaSuite) TestPruneVisibilitySimilarBeginningAlternatives(c *C) {
	...
	c.Assert(err, IsNil)
	c.Assert(data, Equals, "a")
	data, err = schema.PruneByVisibility(parsePath(c, "foo.bar.baz"), confdb.SecretVisibility, data)
	c.Assert(err, IsNil)
	c.Assert(data, IsNil)
}
...
func (*schemaSuite) TestPruneVisibilitySimilarBeginningShortPathAlternatives(c *C) {
	...
	data, err = schema.PruneByVisibility(parsePath(c, "foo"), confdb.SecretVisibility, data)
	c.Assert(err, IsNil)
	c.Assert(data, DeepEquals, map[string]any{"bar": map[string]any{"eph": "a"}})
	data = map[string]any{"bar": map[string]any{"baz": "a"}}
	data, err = schema.PruneByVisibility(parsePath(c, "foo"), confdb.SecretVisibility, data)
	c.Assert(err, IsNil)
	c.Assert(data, IsNil)
}
...
func (*schemaSuite) TestPruneSecretAllTypesShortPath(c *C) {
	...
	"schema": {
		...
		schema, err := confdb.ParseStorageSchema(schemaStr)
		c.Assert(err, IsNil, cmt)
		var data any = map[string]any{"bar": values[typ], "baz": true}
		data, err = schema.PruneByVisibility(parsePath(c, "foo"), confdb.SecretVisibility, data)
		c.Assert(err, IsNil, cmt)
