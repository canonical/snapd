...
func (s *confdbCtrlSuite) TestGroupWithAuthentication(c *C) {
	operator := confdb.Operator{ID: "canonical"}
	err := operator.Delegate([]string{"canonical/network/control-interface"}, []string{"operator-key"})
	c.Assert(err, IsNil)

	err = operator.Delegate([]string{"canonical/network/observe-device"}, []string{"store"})
	c.Assert(err, IsNil)

	err = operator.Delegate(
		[]string{"canonical/network/observe-interface"}, []string{"store", "operator-key"},
	)
	c.Assert(err, IsNil)
...
func (s *confdbCtrlSuite) TestDelegateOK(c *C) {
	...
	c.Assert(operator.Groups[0].Views, DeepEquals, expectedViews)
	expectedAuth := []confdb.AuthenticationMethod{confdb.OperatorKey, confdb.Store}
	c.Assert(operator.Groups[0].Authentication, DeepEquals, expectedAuth)

	// test idempotency
	err = operator.Delegate([]string{"canonical/network/control-device"}, []string{"operator-key", "store"})
	c.Assert(err, IsNil)
...
func (s *confdbCtrlSuite) TestRevoke(c *C) {
	...
	operator.Revoke([]string{"canonical/network/control-interface"}, []string{"operator-key"})
	...
	// test idempotency
	operator.Revoke([]string{"canonical/network/control-interface"}, []string{"operator-key"})
	delegated, _ = operator.IsDelegated("canonical/network/control-interface", []string{"operator-key"})
	c.Check(delegated, Equals, false)
