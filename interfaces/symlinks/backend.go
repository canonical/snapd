...
func (b *Backend) Setup(appSet *interfaces.SnapAppSet, opts interfaces.ConfinementOptions, repo *interfaces.Repository, tm timings.Measurer) error {
	...
	for _, iface := range repo.AllInterfaces() {
		if symlnIface, ok := iface.(interfaces.SymlinksUser); ok {
			for _, d := range symlnIface.Directories() {
				symlinkDirs[d] = true
			}
		}
	}
...
func (b *Backend) ensureSymlinks(spec *Specification, symlinkDirs map[string]bool) error {
	...
	for dir := range symlinkDirs {
		...
		// Loop to remove now unwanted symlinks
		for _, node := range entries {
			...
			// link is active
			if _, ok := lnsToTarget[node.Name()]; ok {
				continue
			}
		...
		// Now ensure state of the links we want in this directory
