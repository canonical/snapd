...
func (s *requestrulesSuite) TestIsPathPermAllowedExpiration(c *C) {
	...
	// Current time starts after all rules have been added but before any have
	// expired.
	current := prompting.At{
		Time:      time.Now(),
...
func (s *requestrulesSuite) TestIsPathPermAllowedSession(c *C) {
	...
	// Same if the user session ends (current.SessionID = 0)
	current.SessionID = 0
	allowed, err = rdb.IsPathPermAllowed(user, snap, iface, path, permission, current)
	c.Check(err, Equals, prompting_errors.ErrNoMatchingRule)
	c.Check(allowed, Equals, false)
	...
	current.SessionID = otherSession
	_, err = rdb.IsPathPermAllowed(user, snap, iface, path, permission, current)
	c.Check(err, IsNil)
}
