...
func (rdb *RuleDB) load() (retErr error) {
	...
	if errInvalid != nil {
		...
		logger.Debugf("calling notifyRules() because DB was invalid and all rules dropped")
...
func (rdb *RuleDB) addRulePermissionToTree(rule *Rule, permission string, permissionEntry *prompting.RulePermissionEntry, at prompting.At) []prompting_errors.RuleConflict {
	...
	for ruleID := range partiallyExpiredRules {
		...
		// Error shouldn't occur. If it does, the rule was already removed.
		if err == nil {
			logger.Debugf("calling notifyRules() for rule with ID %s because it was expired when another rule was added", maybeExpired.ID)
			rdb.notifyRules([]*Rule{maybeExpired}, expiredData)
		} else {
			logger.Debugf("not calling notifyRules() for rule with ID %s because an error occurred when trying to remove it", maybeExpired.ID)
		}
	}
...
func (rdb *RuleDB) AddRule(user uint32, snap string, iface string, constraints *prompting.Constraints) (*Rule, error) {
	...
	logger.Debugf("calling notifyRules() for rule with ID %s when it was first added", newRule.ID)
...
func (rdb *RuleDB) RemoveRule(user uint32, id prompting.IDType) (*Rule, error) {
	...
	logger.Debugf("calling notifyRules() for rule with ID %s when it was removed", id)
	rdb.notifyRules([]*Rule{rule}, data)
	return rule, nil
}
...
func (rdb *RuleDB) removeRulesInternal(user uint32, rules []*Rule) error {
	...
	data := map[string]string{"removed": "removed"}
	logger.Debugf("calling notifyRules() for %d rules which were removed", len(rules))
...
func (rdb *RuleDB) PatchRule(user uint32, id prompting.IDType, constraintsPatch *prompting.RuleConstraintsPatch) (r *Rule, err error) {
	...
	logger.Debugf("calling notifyRules() for rule with ID %s when it was patched", newRule.ID)
	rdb.notifyRules([]*Rule{newRule}, nil)
	return newRule, nil
}
