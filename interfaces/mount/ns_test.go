...
func (s *nsSuite) TestUpdateNamespaceMnt(c *C) {
	for _, t := range []struct {
		...
		// The command doesn't fail and there's no error.
		{cmd: "", mnt: true, locked: false, errStr: "", res: [][]string{{"snap-update-ns", "snap-name"}}},
		// The mnt file is not present so we don't do anything.
		{cmd: "", mnt: false, locked: false, errStr: "", res: nil},
		...
		// The command fails silently and we forward this fact using a generic message.
		{
			...
			res:    [][]string{{"snap-update-ns", "snap-name"}}},
		// use snap-update-ns with --snap-already-locked
		{cmd: "", mnt: true, locked: true, errStr: "", res: [][]string{{"snap-update-ns", "--snap-already-locked", "snap-name"}}},
		// and an error variant of it
		{
			cmd:    "echo 'locked variant, failure'; exit 1;",
			mnt:    true,
			locked: true,
			errStr: `cannot update preserved mount namespace of snap "snap-name": locked variant, failure`,
			res:    [][]string{{"snap-update-ns", "--snap-already-locked", "snap-name"}}},
	} {
		...
		var err error
		if t.locked {
			err = mount.UpdateLockedSnapNamespace("snap-name")
		} else {
