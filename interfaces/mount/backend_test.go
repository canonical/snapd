...
func (s *backendSuite) TestSetupSetsupSimple(c *C) {
	...
	s.Iface.RecordPermanentPlugCallback = func(recorder interface{}, plug *interfaces.Plug) error {
		switch recorder := recorder.(type) {
		case *mount.Recorder:
			recorder.AddMountEntry(fsEntry1)
		}
		return nil
	}
	// Give the slot a permanent effect
	s.iface2.RecordPermanentSlotCallback = func(recorder interface{}, slot *interfaces.Slot) error {
		switch recorder := recorder.(type) {
		case *mount.Recorder:
			recorder.AddMountEntry(fsEntry2)
		}
		return nil
	}
...
func (s *backendSuite) TestSetupSetsupWithoutDir(c *C) {
	s.Iface.RecordPermanentPlugCallback = func(recorder interface{}, plug *interfaces.Plug) error {
		switch recorder := recorder.(type) {
		case *mount.Recorder:
			recorder.AddMountEntry(mount.Entry{})
		}
		return nil
	}
