...
func (s *backendSuite) TestRemove(c *C) {
	...
	hookCanaryToGo := filepath.Join(dirs.SnapMountPolicyDir, "snap.hello-world.hook.config-changing.fstab")
...
func (s *backendSuite) TestSetupSetsupSimple(c *C) {
	...
	for _, binary := range []string{"app1", "app2", "hook.config-changing"} {
