...
func (b *Backend) Configure(snapInfo *snap.Info, developerMode bool, repo *interfaces.Repository) error {
	...
	snippets, err := repo.SecuritySnippetsForSnap(snapInfo.ZName(), interfaces.SecuritySecComp)
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("cannot synchronize security files for snap %q: %s", snapInfo.ZName(), err)
	}
	return nil
}
