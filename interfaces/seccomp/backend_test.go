...
func (s *backendSuite) TestInstallingSnapWritesHookProfiles(c *C) {
	...
	profile := filepath.Join(dirs.SnapSeccompDir, "snap.foo.hook.config-changing")

	// Verify that profile named "snap.foo.hook.config-changing" was created.
	_, err := os.Stat(profile)
	c.Check(err, IsNil)
}
...
func (s *backendSuite) TestRemovingSnapRemovesHookProfiles(c *C) {
	for _, devMode := range []bool{true, false} {
		...
		// Verify that profile "snap.foo.hook.config-changing" was removed.
		_, err := os.Stat(profile)
		c.Check(os.IsNotExist(err), Equals, true)
	}
}
...
func (s *backendSuite) TestUpdatingSnapToOneWithHooks(c *C) {
	for _, devMode := range []bool{true, false} {
		...
		profile := filepath.Join(dirs.SnapSeccompDir, "snap.samba.hook.config-changing")
...
func (s *backendSuite) TestUpdatingSnapToOneWithNoHooks(c *C) {
	for _, devMode := range []bool{true, false} {
		...
		// Verify that profile snap.samba.hook.config-changing was removed.
		_, err := os.Stat(profile)
		c.Check(os.IsNotExist(err), Equals, true)
		s.RemoveSnap(c, snapInfo)
	}
