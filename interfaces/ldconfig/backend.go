...
func (b *Backend) setupLdconfigCache(spec *Specification) error {
	...
	if len(spec.libDirs) > 0 {
		...
		sort.Slice(sortedSnapSlots, func(i, j int) bool {
			n1 := sortedSnapSlots[i].snapSlot.InstanceName.String() + "_" + sortedSnapSlots[i].snapSlot.SlotName
			n2 := sortedSnapSlots[j].snapSlot.InstanceName.String() + "_" + sortedSnapSlots[j].snapSlot.SlotName
			return n1 < n2
		})
		var content strings.Builder
		content.WriteString("## This file is automatically generated by snapd\n")
		for _, ssLibs := range sortedSnapSlots {
			content.WriteString(fmt.Sprintf("\n# Directories from %s snap, %s slot\n",
				ssLibs.snapSlot.InstanceName, ssLibs.snapSlot.SlotName))
			content.WriteString(strings.Join(ssLibs.dirs, "\n"))
			content.WriteString("\n")
		}
