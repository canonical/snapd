...
func (s *backendSuite) TestInstallingSnapWithHookWritesConfigFiles(c *C) {
	...
	for _, devMode := range []bool{true, false} {
		...
		profile := filepath.Join(dirs.SnapBusPolicyDir, "snap.foo.hook.config-changing.conf")

		// Verify that "snap.foo.hook.config-changing.conf" was created
		_, err := os.Stat(profile)
		c.Check(err, IsNil)
		s.RemoveSnap(c, snapInfo)
	}
...
func (s *backendSuite) TestRemovingSnapWithHookRemovesConfigFiles(c *C) {
	...
	for _, devMode := range []bool{true, false} {
		...
		// Verify that "snap.foo.hook.config-changing.conf" was removed
		_, err := os.Stat(profile)
		c.Check(os.IsNotExist(err), Equals, true)
	}
}
...
func (s *backendSuite) TestUpdatingSnapToOneWithMoreHooks(c *C) {
	...
	for _, devMode := range []bool{true, false} {
		...
		profile := filepath.Join(dirs.SnapBusPolicyDir, "snap.samba.hook.config-changing.conf")

		// Verify that "snap.samba.hook.config-changing.conf" was created
		_, err := os.Stat(profile)
		c.Check(err, IsNil)
		s.RemoveSnap(c, snapInfo)
	}
...
func (s *backendSuite) TestUpdatingSnapToOneWithFewerHooks(c *C) {
	...
	for _, devMode := range []bool{true, false} {
		...
		// Verify that "snap.samba.hook.config-changing.conf" was removed
		_, err := os.Stat(profile)
		c.Check(os.IsNotExist(err), Equals, true)
		s.RemoveSnap(c, snapInfo)
	}
