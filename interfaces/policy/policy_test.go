...
func (s *policySuite) TestBaselineDefaultIsAllow(c *C) {
	cand := policy.ConnectCandidate{
		Plug:            interfaces.NewPlugData(s.plugSnap.Plugs["random"], nil),
		Slot:            interfaces.NewSlotData(s.slotSnap.Slots["random"], nil),
...
func (s *policySuite) TestInterfaceMismatch(c *C) {
	cand := policy.ConnectCandidate{
		Plug:            interfaces.NewPlugData(s.plugSnap.Plugs["mismatchy"], nil),
		Slot:            interfaces.NewSlotData(s.slotSnap.Slots["mismatchy"], nil),
		BaseDeclaration: s.baseDecl,
	}
...
func (s *policySuite) TestBaseDeclAllowDenyConnection(c *C) {
	...
	for _, t := range tests {
		cand := policy.ConnectCandidate{
			Plug:            interfaces.NewPlugData(s.plugSnap.Plugs[t.iface], nil),
			Slot:            interfaces.NewSlotData(s.slotSnap.Slots[t.iface], nil),
			BaseDeclaration: s.baseDecl,
		}
...
`, nil)
	...
	cand := policy.ConnectCandidate{
		Plug:            interfaces.NewPlugData(gadgetSnap.Plugs["gadgethelp"], nil),
		Slot:            interfaces.NewSlotData(coreSnap.Slots["gadgethelp"], nil),
		BaseDeclaration: s.baseDecl,
	}
	c.Check(cand.Check(), IsNil)

	cand = policy.ConnectCandidate{
		Plug:            interfaces.NewPlugData(s.plugSnap.Plugs["gadgethelp"], nil),
		Slot:            interfaces.NewSlotData(coreSnap.Slots["gadgethelp"], nil),
		BaseDeclaration: s.baseDecl,
	}
	...
	for _, trustedSide := range []*snap.Info{coreSnap, gadgetSnap} {
		cand = policy.ConnectCandidate{
			Plug:                interfaces.NewPlugData(s.plugSnap.Plugs["trustedhelp"], nil),
			PlugSnapDeclaration: s.plugDecl,
			Slot:                interfaces.NewSlotData(trustedSide.Slots["trustedhelp"], nil),
			BaseDeclaration:     s.baseDecl,
		}
		c.Check(cand.Check(), IsNil)
	}
	...
	cand = policy.ConnectCandidate{
		Plug:                interfaces.NewPlugData(s.plugSnap.Plugs["trustedhelp"], nil),
		PlugSnapDeclaration: s.plugDecl,
		Slot:                interfaces.NewSlotData(s.slotSnap.Slots["trustedhelp"], nil),
		BaseDeclaration:     s.baseDecl,
	}
	c.Check(cand.Check(), ErrorMatches, "connection not allowed.*")
}
...
func (s *policySuite) TestPlugSnapIDCheckConnection(c *C) {
	...
	cand := policy.ConnectCandidate{
		Plug:                interfaces.NewPlugData(s.randomSnap.Plugs["precise-plug-snap-id"], nil),
		Slot:                interfaces.NewSlotData(s.slotSnap.Slots["precise-plug-snap-id"], nil),
		SlotSnapDeclaration: s.slotDecl,
		BaseDeclaration:     s.baseDecl,
	}
	c.Check(cand.Check(), ErrorMatches, "connection not allowed.*")
	...
	// right snap-id
	cand = policy.ConnectCandidate{
		Plug:                interfaces.NewPlugData(s.plugSnap.Plugs["precise-plug-snap-id"], nil),
		PlugSnapDeclaration: s.plugDecl,
		Slot:                interfaces.NewSlotData(s.slotSnap.Slots["precise-plug-snap-id"], nil),
		SlotSnapDeclaration: s.slotDecl,
		BaseDeclaration:     s.baseDecl,
...
func (s *policySuite) TestSlotSnapIDCheckConnection(c *C) {
	...
	cand := policy.ConnectCandidate{
		Plug:                interfaces.NewPlugData(s.plugSnap.Plugs["precise-slot-snap-id"], nil),
		PlugSnapDeclaration: s.plugDecl,
		Slot:                interfaces.NewSlotData(s.randomSnap.Slots["precise-slot-snap-id"], nil),
		BaseDeclaration:     s.baseDecl,
	}
	c.Check(cand.Check(), ErrorMatches, "connection not allowed.*")
	...
	cand = policy.ConnectCandidate{
		Plug:                interfaces.NewPlugData(s.plugSnap.Plugs["precise-slot-snap-id"], nil),
		PlugSnapDeclaration: s.plugDecl,
		Slot:                interfaces.NewSlotData(s.slotSnap.Slots["precise-slot-snap-id"], nil),
		SlotSnapDeclaration: s.slotDecl,
		BaseDeclaration:     s.baseDecl,
	}
	c.Check(cand.Check(), IsNil)
...
func (s *policySuite) TestPlugPublisherIDCheckConnection(c *C) {
	// no plug-side declaration
	cand := policy.ConnectCandidate{
		Plug:                interfaces.NewPlugData(s.randomSnap.Plugs["checked-plug-publisher-id"], nil),
		Slot:                interfaces.NewSlotData(s.slotSnap.Slots["checked-plug-publisher-id"], nil),
		SlotSnapDeclaration: s.slotDecl,
		BaseDeclaration:     s.baseDecl,
	}
	c.Check(cand.Check(), ErrorMatches, "connection not allowed.*")
	...
	// right publisher-id
	cand = policy.ConnectCandidate{
		Plug:                interfaces.NewPlugData(s.plugSnap.Plugs["checked-plug-publisher-id"], nil),
		PlugSnapDeclaration: s.plugDecl,
		Slot:                interfaces.NewSlotData(s.slotSnap.Slots["checked-plug-publisher-id"], nil),
		SlotSnapDeclaration: s.slotDecl,
		BaseDeclaration:     s.baseDecl,
...
func (s *policySuite) TestSlotPublisherIDCheckConnection(c *C) {
	// no slot-side declaration
	cand := policy.ConnectCandidate{
		Plug:                interfaces.NewPlugData(s.plugSnap.Plugs["checked-slot-publisher-id"], nil),
		PlugSnapDeclaration: s.plugDecl,
		Slot:                interfaces.NewSlotData(s.randomSnap.Slots["checked-slot-publisher-id"], nil),
		BaseDeclaration:     s.baseDecl,
	}
	c.Check(cand.Check(), ErrorMatches, "connection not allowed.*")
	...
	cand = policy.ConnectCandidate{
		Plug:                interfaces.NewPlugData(s.plugSnap.Plugs["checked-slot-publisher-id"], nil),
		PlugSnapDeclaration: s.plugDecl,
		Slot:                interfaces.NewSlotData(s.slotSnap.Slots["checked-slot-publisher-id"], nil),
		SlotSnapDeclaration: s.slotDecl,
		BaseDeclaration:     s.baseDecl,
	}
	c.Check(cand.Check(), IsNil)
...
func (s *policySuite) TestDollarPlugPublisherIDCheckConnection(c *C) {
	// no known publishers
	cand := policy.ConnectCandidate{
		Plug:            interfaces.NewPlugData(s.plugSnap.Plugs["same-plug-publisher-id"], nil),
		Slot:            interfaces.NewSlotData(s.randomSnap.Slots["same-plug-publisher-id"], nil),
		BaseDeclaration: s.baseDecl,
	}
	c.Check(cand.Check(), ErrorMatches, "connection not allowed.*")
...
AXNpZw==`))
	...
	cand = policy.ConnectCandidate{
		Plug:                interfaces.NewPlugData(s.plugSnap.Plugs["same-plug-publisher-id"], nil),
		PlugSnapDeclaration: s.plugDecl,
		Slot:                interfaces.NewSlotData(samePubSlotSnap.Slots["same-plug-publisher-id"], nil),
...
func (s *policySuite) TestDollarSlotPublisherIDCheckConnection(c *C) {
	// no known publishers
	cand := policy.ConnectCandidate{
		Plug:            interfaces.NewPlugData(s.randomSnap.Plugs["same-slot-publisher-id"], nil),
		Slot:            interfaces.NewSlotData(s.slotSnap.Slots["same-slot-publisher-id"], nil),
		BaseDeclaration: s.baseDecl,
	}
	c.Check(cand.Check(), ErrorMatches, "connection not allowed.*")
...
AXNpZw==`))
	...
	cand = policy.ConnectCandidate{
		Plug:                interfaces.NewPlugData(samePubPlugSnap.Plugs["same-slot-publisher-id"], nil),
...
func (s *policySuite) TestSlotDollarSlotAttrConnection(c *C) {
	...
	cand := policy.ConnectCandidate{
		Plug:            interfaces.NewPlugData(s.randomSnap.Plugs["slot-slot-attr"], nil),
		Slot:            interfaces.NewSlotData(s.slotSnap.Slots["slot-slot-attr"], nil),
		BaseDeclaration: s.baseDecl,
	}
	c.Check(cand.Check(), ErrorMatches, "connection not allowed.*")
	...
	cand = policy.ConnectCandidate{
		Plug:            interfaces.NewPlugData(s.plugSnap.Plugs["slot-slot-attr-mismatch"], nil),
		Slot:            interfaces.NewSlotData(s.slotSnap.Slots["slot-slot-attr"], nil),
		BaseDeclaration: s.baseDecl,
	}
	c.Check(cand.Check(), ErrorMatches, "connection not allowed.*")
	...
	// plug attr == slot attr
	cand = policy.ConnectCandidate{
		Plug:            interfaces.NewPlugData(s.plugSnap.Plugs["slot-slot-attr-match"], nil),
		Slot:            interfaces.NewSlotData(s.slotSnap.Slots["slot-slot-attr"], nil),
		BaseDeclaration: s.baseDecl,
	}
	c.Check(cand.Check(), IsNil)
...
func (s *policySuite) TestSlotDollarPlugAttrConnection(c *C) {
	// different attr values
	cand := policy.ConnectCandidate{
		Plug:            interfaces.NewPlugData(s.plugSnap.Plugs["slot-plug-attr-mismatch"], nil),
		Slot:            interfaces.NewSlotData(s.slotSnap.Slots["slot-plug-attr"], nil),
		BaseDeclaration: s.baseDecl,
	}
	c.Check(cand.Check(), ErrorMatches, "connection not allowed.*")
	...
	// plug attr == slot attr
	cand = policy.ConnectCandidate{
		Plug:            interfaces.NewPlugData(s.plugSnap.Plugs["slot-plug-attr-match"], nil),
		Slot:            interfaces.NewSlotData(s.slotSnap.Slots["slot-plug-attr"], nil),
		BaseDeclaration: s.baseDecl,
	}
	c.Check(cand.Check(), IsNil)
...
func (s *policySuite) TestPlugDollarPlugAttrConnection(c *C) {
	// different attr values
	cand := policy.ConnectCandidate{
		Plug:            interfaces.NewPlugData(s.plugSnap.Plugs["plug-plug-attr"], nil),
		Slot:            interfaces.NewSlotData(s.slotSnap.Slots["plug-plug-attr-mismatch"], nil),
		BaseDeclaration: s.baseDecl,
	}
	c.Check(cand.Check(), ErrorMatches, "connection not allowed.*")
	...
	cand = policy.ConnectCandidate{
		Plug:            interfaces.NewPlugData(s.plugSnap.Plugs["plug-plug-attr"], nil),
		Slot:            interfaces.NewSlotData(s.slotSnap.Slots["plug-plug-attr-match"], nil),
		BaseDeclaration: s.baseDecl,
	}
	c.Check(cand.Check(), IsNil)
}
...
func (s *policySuite) TestPlugDollarSlotAttrConnection(c *C) {
	// different attr values
	cand := policy.ConnectCandidate{
		Plug:            interfaces.NewPlugData(s.plugSnap.Plugs["plug-slot-attr"], nil),
		Slot:            interfaces.NewSlotData(s.slotSnap.Slots["plug-slot-attr-mismatch"], nil),
		BaseDeclaration: s.baseDecl,
	}
	c.Check(cand.Check(), ErrorMatches, "connection not allowed.*")
	...
	cand = policy.ConnectCandidate{
		Plug:            interfaces.NewPlugData(s.plugSnap.Plugs["plug-slot-attr"], nil),
		Slot:            interfaces.NewSlotData(s.slotSnap.Slots["plug-slot-attr-match"], nil),
		BaseDeclaration: s.baseDecl,
	}
	c.Check(cand.Check(), IsNil)
}
...
func (s *policySuite) TestDollarMissingConnection(c *C) {
	// not missing
	cand := policy.ConnectCandidate{
		Plug:            interfaces.NewPlugData(s.plugSnap.Plugs["slot-plug-missing-mismatch"], nil),
		Slot:            interfaces.NewSlotData(s.slotSnap.Slots["slot-plug-missing"], nil),
		BaseDeclaration: s.baseDecl,
	}
	c.Check(cand.Check(), ErrorMatches, "connection not allowed.*")
	...
	// missing
	cand = policy.ConnectCandidate{
		Plug:            interfaces.NewPlugData(s.plugSnap.Plugs["slot-plug-missing-match"], nil),
		Slot:            interfaces.NewSlotData(s.slotSnap.Slots["slot-plug-missing"], nil),
		BaseDeclaration: s.baseDecl,
	}
	c.Check(cand.Check(), IsNil)
...
func (s *policySuite) TestSlotHookAttrs(c *C) {
	// the slot doesn't have 'c' attribute, but it's provided by the hooks.
	hookSlotAttrs := map[string]interface{}{"c": "C"}
	cand := policy.ConnectCandidate{
		Plug:            interfaces.NewPlugData(s.plugSnap.Plugs["plug-plug-attr"], nil),
		Slot:            interfaces.NewSlotData(s.slotSnap.Slots["plug-plug-attr-no-c"], hookSlotAttrs),
		BaseDeclaration: s.baseDecl,
	}
	c.Check(cand.Check(), IsNil)
}
...
func (s *policySuite) TestSlotHookAttrsMismatch(c *C) {
	// the slot doesn't have 'c' attribute, but it's provided by the hooks but its value doesn't match plug.
	hookSlotAttrs := map[string]interface{}{"c": "not matching"}
	cand := policy.ConnectCandidate{
		Plug:            interfaces.NewPlugData(s.plugSnap.Plugs["plug-plug-attr"], nil),
		Slot:            interfaces.NewSlotData(s.slotSnap.Slots["plug-plug-attr-no-c"], hookSlotAttrs),
		BaseDeclaration: s.baseDecl,
...
func (s *policySuite) TestPlugHookAttrs(c *C) {
	hookPlugAttrs := map[string]interface{}{
	...
	cand := policy.ConnectCandidate{
		Plug:            interfaces.NewPlugData(s.plugSnap.Plugs["slot-slot-attr-no-a"], hookPlugAttrs),
		Slot:            interfaces.NewSlotData(s.slotSnap.Slots["slot-slot-attr"], nil),
		BaseDeclaration: s.baseDecl,
	}
	c.Check(cand.Check(), IsNil)
...
func (s *policySuite) TestPlugSlotHookAttrs(c *C) {
	hookPlugAttrs := map[string]interface{}{
		"a": map[string]interface{}{
			"b": []interface{}{"x", "y"},
		},
	}
	hookSlotAttrs := map[string]interface{}{
		"a": map[string]interface{}{
			"b": []interface{}{"x", "y"},
		},
	}
	cand := policy.ConnectCandidate{
		Plug:            interfaces.NewPlugData(s.plugSnap.Plugs["slot-slot-attr-no-a"], hookPlugAttrs),
		Slot:            interfaces.NewSlotData(s.slotSnap.Slots["slot-slot-attr-no-a"], hookSlotAttrs),
		BaseDeclaration: s.baseDecl,
	}
	c.Check(cand.Check(), IsNil)
}

func (s *policySuite) TestPlugSlotHookAttrsMismatch(c *C) {
	hookPlugAttrs := map[string]interface{}{
		"a": map[string]interface{}{
			"b": []interface{}{"x", "y"},
		},
