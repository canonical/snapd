...
func (s *backendSuite) TestPreseed(c *C) {
	...
	c.Check(cgroupFname, testutil.FilePresent)
	fname := filepath.Join(dirs.SnapUdevRulesDir, "70-snap.samba.rules")
	info := s.InstallSnap(c, interfaces.ConfinementOptions{}, "", ifacetest.SambaYamlV1, 0)
	// device cgroup self manage flag is gone now
	c.Check(cgroupFname, testutil.FileEquals, "# This file is automatically generated.\n")
	// and we have the rules file
	c.Check(fname, testutil.FileEquals, "# This file is automatically generated.\nsample\n")
...
func (s *backendSuite) TestRemovingSnapWithInputSubsystemTriggersInputReload(c *C) {
	// NOTE: Hand out a permanent snippet and input subsystem trigger so that
	// the trigger is recorded in the rules file on install.
	s.Iface.UDevPermanentSlotCallback = func(spec *udev.Specification, slot *snap.SlotInfo) error {
		spec.TriggerSubsystem("input")
		spec.AddSnippet("sample")
		return nil
	}
	// Install in strict mode — the trigger comment is only written when rules
	// are active (not devmode/classic without jailmode).
	snapInfo := s.InstallSnap(c, interfaces.ConfinementOptions{}, "", ifacetest.SambaYamlV1, 0)
	s.udevadmCmd.ForgetCalls()
	s.RemoveSnap(c, snapInfo)
	// Removal must trigger the input subsystem so that input devices have
	// the snap's tag removed from their udev database entries.
	c.Check(s.udevadmCmd.Calls(), DeepEquals, [][]string{
		{"udevadm", "control", "--reload-rules"},
		{"udevadm", "trigger", "--subsystem-nomatch=input"},
		{"udevadm", "trigger", "--subsystem-match=input"},
...
func (s *backendSuite) TestRemovingSnapWithInputKeySubsystemTriggersInputKeyReload(c *C) {
	// NOTE: Hand out a permanent snippet and input/key subsystem trigger.
	s.Iface.UDevPermanentSlotCallback = func(spec *udev.Specification, slot *snap.SlotInfo) error {
	...
	s.udevadmCmd.ForgetCalls()
	s.RemoveSnap(c, snapInfo)
	// Removal must trigger the input/key subsystem so that key devices have
	// the snap's tag removed from their udev database entries.
	c.Check(s.udevadmCmd.Calls(), DeepEquals, [][]string{
		{"udevadm", "control", "--reload-rules"},
		{"udevadm", "trigger", "--subsystem-nomatch=input"},
		{"udevadm", "trigger", "--property-match=ID_INPUT_KEY=1", "--property-match=ID_INPUT_KEYBOARD!=1"},
		{"udevadm", "trigger", "--property-match=ID_INPUT_JOYSTICK=1"},
		{"udevadm", "settle", "--timeout=10"},
	})
}

func (s *backendSuite) TestRemovingSnapWithInputJoystickSubsystemTriggersJoystickReload(c *C) {
	// NOTE: Hand out a permanent snippet and input/joystick subsystem trigger.
	s.Iface.UDevPermanentSlotCallback = func(spec *udev.Specification, slot *snap.SlotInfo) error {
		spec.TriggerSubsystem("input/joystick")
		spec.AddSnippet("sample")
		return nil
	...
	s.udevadmCmd.ForgetCalls()
	s.RemoveSnap(c, snapInfo)
	// Removal must trigger the joystick property so that joystick devices have
	// the snap's tag removed from their udev database entries.
	c.Check(s.udevadmCmd.Calls(), DeepEquals, [][]string{
		{"udevadm", "control", "--reload-rules"},
		{"udevadm", "trigger", "--subsystem-nomatch=input"},
...
func (s *backendSuite) TestSetupWritesSubsystemTriggersInRulesFileForStrictSnap(c *C) {
	s.Iface.UDevPermanentSlotCallback = func(spec *udev.Specification, slot *snap.SlotInfo) error {
		spec.TriggerSubsystem("input")
		spec.AddSnippet("sample")
		return nil
	...
	fname := filepath.Join(dirs.SnapUdevRulesDir, "70-snap.samba.rules")
	c.Check(fname, testutil.FileEquals, "# This file is automatically generated.\n# subsystem-triggers: [\"input\"]\nsample\n")
}

func (s *backendSuite) TestSetupDoesNotWriteSubsystemTriggersInRulesFileForNonStrictSnap(c *C) {
	s.Iface.UDevPermanentSlotCallback = func(spec *udev.Specification, slot *snap.SlotInfo) error {
		spec.TriggerSubsystem("input")
		spec.AddSnippet("sample")
		return nil
	}
	s.InstallSnap(c, interfaces.ConfinementOptions{DevMode: true}, "", ifacetest.SambaYamlV1, 0)
	fname := filepath.Join(dirs.SnapUdevRulesDir, "70-snap.samba.rules")
	c.Check(fname, testutil.FileEquals, "# This file is automatically generated.\n# udev tagging/device cgroups disabled with non-strict mode snaps\n#sample\n")
}

func (s *backendSuite) TestReadRulesSubsystemTriggersWhenFileAbsent(c *C) {
	triggers := udev.ReadRulesSubsystemTriggers(filepath.Join(c.MkDir(), "nonexistent.rules"))
	c.Check(triggers, IsNil)
}

func (s *backendSuite) TestReadRulesSubsystemTriggersWithOpenError(c *C) {
	if os.Geteuid() == 0 {
		c.Skip("the test cannot be run by the root user")
	}
	logbuf, restore := logger.MockLogger()
	defer restore()
	path := filepath.Join(c.MkDir(), "test.rules")
	c.Assert(os.WriteFile(path, []byte("# This file is automatically generated.\n"), 0000), IsNil)
	c.Check(udev.ReadRulesSubsystemTriggers(path), IsNil)
	c.Check(logbuf.String(), testutil.Contains, "cannot open udev rules file")
}

func (s *backendSuite) TestReadRulesSubsystemTriggersWithScannerError(c *C) {
	logbuf, restore := logger.MockLogger()
	defer restore()
	path := filepath.Join(c.MkDir(), "test.rules")
	// A comment line longer than bufio.MaxScanTokenSize (64KiB) causes scanner.Err() != nil.
	longLine := append([]byte{'#'}, bytes.Repeat([]byte{'x'}, 64*1024+1)...)
	longLine = append(longLine, '\n')
	c.Assert(os.WriteFile(path, longLine, 0644), IsNil)
	c.Check(udev.ReadRulesSubsystemTriggers(path), IsNil)
	c.Check(logbuf.String(), testutil.Contains, "cannot read udev rules file")
}

func (s *backendSuite) TestReadRulesSubsystemTriggersWithSingleTrigger(c *C) {
	path := filepath.Join(c.MkDir(), "test.rules")
	c.Assert(os.WriteFile(path, []byte("# This file is automatically generated.\n# subsystem-triggers: [\"input\"]\nsample\n"), 0644), IsNil)
	c.Check(udev.ReadRulesSubsystemTriggers(path), DeepEquals, []string{"input"})
}

func (s *backendSuite) TestReadRulesSubsystemTriggersWithMultipleTriggers(c *C) {
	path := filepath.Join(c.MkDir(), "test.rules")
	c.Assert(os.WriteFile(path, []byte("# This file is automatically generated.\n# subsystem-triggers: [\"input\",\"input/key\"]\nsample\n"), 0644), IsNil)
	c.Check(udev.ReadRulesSubsystemTriggers(path), DeepEquals, []string{"input", "input/key"})
}

func (s *backendSuite) TestReadRulesSubsystemTriggersWithEmptyTriggerLine(c *C) {
	logbuf, restore := logger.MockLogger()
	defer restore()
	path := filepath.Join(c.MkDir(), "test.rules")
	c.Assert(os.WriteFile(path, []byte("# This file is automatically generated.\n# subsystem-triggers: \nsample\n"), 0644), IsNil)
	c.Check(udev.ReadRulesSubsystemTriggers(path), IsNil)
	c.Check(logbuf.String(), testutil.Contains, "cannot unmarshal subsystem triggers")
}

func (s *backendSuite) TestReadRulesSubsystemTriggersWithUnknownTrigger(c *C) {
	logbuf, restore := logger.MockLogger()
	defer restore()
	path := filepath.Join(c.MkDir(), "test.rules")
	c.Assert(os.WriteFile(path, []byte("# This file is automatically generated.\n# subsystem-triggers: [\"unknown\"]\nsample\n"), 0644), IsNil)
	c.Check(udev.ReadRulesSubsystemTriggers(path), IsNil)
	c.Check(logbuf.String(), testutil.Contains, `unknown subsystem trigger "unknown"`)
}

func (s *backendSuite) TestReadRulesSubsystemTriggersWithInvalidJSON(c *C) {
	logbuf, restore := logger.MockLogger()
	defer restore()
	path := filepath.Join(c.MkDir(), "test.rules")
	c.Assert(os.WriteFile(path, []byte("# This file is automatically generated.\n# subsystem-triggers: not-json\nsample\n"), 0644), IsNil)
	c.Check(udev.ReadRulesSubsystemTriggers(path), IsNil)
	c.Check(logbuf.String(), testutil.Contains, "cannot unmarshal subsystem triggers")
}

func (s *backendSuite) TestReadRulesSubsystemTriggersStopsAtNonCommentLine(c *C) {
	// non-comment line before the trigger comment: function does not scan past it
	path := filepath.Join(c.MkDir(), "test.rules")
	c.Assert(os.WriteFile(path, []byte("# This file is automatically generated.\nsome-rule\n# subsystem-triggers: [\"input\"]\n"), 0644), IsNil)
	c.Check(udev.ReadRulesSubsystemTriggers(path), IsNil)
}

func (s *backendSuite) TestReadRulesSubsystemTriggersWithNoTriggerLine(c *C) {
	// comment-only file (e.g. devmode snap) has no trigger line
	path := filepath.Join(c.MkDir(), "test.rules")
	c.Assert(os.WriteFile(path, []byte("# This file is automatically generated.\n# udev tagging/device cgroups disabled\n"), 0644), IsNil)
	c.Check(udev.ReadRulesSubsystemTriggers(path), IsNil)
}
