...
func (s *backendSuite) TestInstallingSnapWritesAndLoadsRules(c *C) {
	...
	s.Iface.UDevPermanentSlotCallback = func(spec *udev.Specification, slot *interfaces.SlotData) error {
...
func (s *backendSuite) TestInstallingSnapWithHookWritesAndLoadsRules(c *C) {
	...
	s.Iface.UDevPermanentPlugCallback = func(spec *udev.Specification, slot *interfaces.PlugData) error {
		spec.AddSnippet("dummy")
		return nil
	}
	for _, opts := range testedConfinementOpts {
