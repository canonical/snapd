...
func (b *Backend) setUpDropIns(instanceName naming.InstanceName, spec *Specification) (madeChanges bool, err error) {
	...
	if changes, err := b.removeOtherDropIns(instanceName, dropIns); err == nil {
		if changes {
			madeChanges = true
		}
	} else {
...
func (b *Backend) removeOtherDropIns(instanceName naming.InstanceName, dropIns map[string]map[string]string) (madeChanges bool, err error) {
	entries, err := os.ReadDir(dirs.SnapServicesDir)
	if err != nil {
		return false, err
	}
	pattern := snap.AppSecurityTag(instanceName.String(), "*") + ".service.d"
