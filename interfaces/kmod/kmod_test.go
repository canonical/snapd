...
func (s *kmodSuite) TestModulesFile(c *C) {
	modules := [][]byte{
		[]byte("module1"),
		[]byte("module2")}
	var err error
	err = kmod.WriteModulesFile(modules, "foo")
	c.Assert(err, IsNil)

	path := filepath.Join(dirs.SnapKModModulesDir, fmt.Sprintf("snapd-%s.conf", snap.SecurityTag("foo")))
