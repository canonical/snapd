...
func (r *Repository) AddPlug(plug *Plug) error {
	...
	if err := snap.ValidateName(plug.Snap.ZName()); err != nil {
		return err
	}
	...
	if _, ok := r.plugs[plug.Snap.ZName()][plug.Name]; ok {
		return fmt.Errorf("cannot add plug, snap %q already has plug %q", plug.Snap.ZName(), plug.Name)
	}
	if r.plugs[plug.Snap.ZName()] == nil {
		r.plugs[plug.Snap.ZName()] = make(map[string]*Plug)
	}
	r.plugs[plug.Snap.ZName()][plug.Name] = plug
	return nil
}
...
func (r *Repository) AddSlot(slot *Slot) error {
	...
	// Reject snaps with invalid names
	if err := snap.ValidateName(slot.Snap.ZName()); err != nil {
		return err
	}
	// Reject plug with invalid names
	...
	if _, ok := r.slots[slot.Snap.ZName()][slot.Name]; ok {
		return fmt.Errorf("cannot add slot, snap %q already has slot %q", slot.Snap.ZName(), slot.Name)
	}
	if r.slots[slot.Snap.ZName()] == nil {
		r.slots[slot.Snap.ZName()] = make(map[string]*Slot)
	}
	r.slots[slot.Snap.ZName()][slot.Name] = slot
	return nil
}
...
func (r *Repository) Connect(plugSnapName, plugName, slotSnapName, slotName string) error {
	...
	slot.Connections = append(slot.Connections, PlugRef{plug.Snap.ZName(), plug.Name})
	plug.Connections = append(plug.Connections, SlotRef{slot.Snap.ZName(), slot.Name})
	return nil
}
...
func (r *Repository) disconnect(plug *Plug, slot *Slot) {
	...
	for i, plugRef := range slot.Connections {
		if plugRef.Snap == plug.Snap.ZName() && plugRef.Name == plug.Name {
	...
	for i, slotRef := range plug.Connections {
		if slotRef.Snap == slot.Snap.ZName() && slotRef.Name == slot.Name {
