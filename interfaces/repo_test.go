...
func (s *RepositorySuite) TestRemovePlugFailsWhenPlugIsConnected(c *C) {
	...
	_, err = s.testRepo.Connect(*connRef, nil, nil)
	c.Assert(err, IsNil)
...
func (s *RepositorySuite) TestDisconnectSucceeds(c *C) {
	c.Assert(s.testRepo.AddPlug(s.plug), IsNil)
	c.Assert(s.testRepo.AddSlot(s.slot), IsNil)
	_, err := s.testRepo.Connect(*NewConnRef(s.plug, s.slot), nil, nil)
	c.Assert(err, IsNil)
	_, err = s.testRepo.Connect(*NewConnRef(s.plug, s.slot), nil, nil)
	c.Assert(err, IsNil)
	err = s.testRepo.Disconnect(s.plug.Snap.Name(), s.plug.Name, s.slot.Snap.Name(), s.slot.Name)
...
func (s *RepositorySuite) TestConnectedFindsCoreSnap(c *C) {
	...
	_, err := s.testRepo.Connect(*NewConnRef(s.plug, slot), nil, nil)
	c.Assert(err, IsNil)
...
func (s *AddRemoveSuite) TestRemoveSnapErrorsOnStillConnectedPlug(c *C) {
	...
	_, err = s.repo.Connect(connRef, nil, nil)
	c.Assert(err, IsNil)
...
`), nil)
	...
	_, err := r.Connect(ConnRef{PlugRef: PlugRef{Snap: "s1", Name: "i1"}, SlotRef: SlotRef{Snap: "s2", Name: "i1"}}, nil, nil)
	c.Assert(err, IsNil)
	_, err = r.Connect(ConnRef{PlugRef: PlugRef{Snap: "s1", Name: "i1"}, SlotRef: SlotRef{Snap: "s2", Name: "i1"}}, nil, nil)
	c.Assert(err, IsNil)
	_, err = r.Connect(ConnRef{PlugRef: PlugRef{Snap: "s1", Name: "i2"}, SlotRef: SlotRef{Snap: "s3", Name: "i2"}}, nil, nil)
	c.Assert(err, IsNil)
