...
`
	...
	rec := &mount.Recorder{}
	c.Assert(s.iface.RecordConnectedPlug(rec, plug, slot), IsNil)
	expectedMnt := []mount.Entry{{
		FsName:  "/snap/producer/5/export",
		Dir:     "/snap/consumer/7/import",
		Options: []string{"bind", "ro"},
	}}
	c.Assert(rec.MountEntries, DeepEquals, expectedMnt)
}
...
`
	...
	c.Assert(s.iface.RecordConnectedPlug(rec, plug, slot), IsNil)
	expectedMnt := []mount.Entry{{
		FsName:  "/var/snap/producer/5/export",
...
`
	...
	c.Assert(s.iface.RecordConnectedPlug(rec, plug, slot), IsNil)
	expectedMnt := []mount.Entry{{
		FsName:  "/var/snap/producer/common/export",
