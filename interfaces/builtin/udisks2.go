...
## Block Device Objects can implement Block and Filesystem, Partition, or Loop interfaces
## Block
dbus (receive)
    bus=system
    path=/org/freedesktop/UDisks2/block_devices/*
    interface=org.freedesktop.UDisks2.Block
    member={Add,Remove,Update}ConfigurationItem
    peer=(label=###PLUG_SECURITY_TAGS###),
...
dbus (receive)
    ...
    path=/org/freedesktop/UDisks2/block_devices/*
    interface=org.freedesktop.UDisks2.Block
    member=OpenFor{Backup,Restore,Benchmark}
    peer=(label=###PLUG_SECURITY_TAGS###),
...
# Drive Objects can implement Drive and Drive.Ata interfaces
## Drive
dbus (receive)
    bus=system
...
dbus (receive)
    ...
    member=Smart{Update,GetAttributes,SelftestStart,SelftestAbort,SetEnabled}
    peer=(label=###PLUG_SECURITY_TAGS###),
...
dbus (receive)
    ...
    path=/org/freedesktop/UDisks2/drives/*
    interface=org.freedesktop.UDisks2.Drive.Ata
    member=Pm{GetState,Standby,Wakeup}
    peer=(label=###PLUG_SECURITY_TAGS###),
...
dbus (receive)
    ...
    path=/org/freedesktop/UDisks2/drives/*
    interface=org.freedesktop.UDisks2.Drive.Ata
    member=SecurityEraseUnit
    peer=(label=###PLUG_SECURITY_TAGS###),

# UDisks2 MDRaid interface
dbus (receive)
    bus=system
    path=/org/freedesktop/UDisks2/mdraid/*
...
# Udisks2 Job interface
dbus (receive)
    bus=system
    path=/org/freedesktop/UDisks2/jobs/*
...
dbus (receive)
    bus=system
    path=/org/freedesktop/UDisks2.Job
    interface=org.freedesktop.DBus.Properties
    member=Get
    peer=(label=###PLUG_SECURITY_TAGS###),
