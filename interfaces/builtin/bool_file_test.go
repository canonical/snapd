...
func (s *BoolFileInterfaceSuite) TestPlugSecuritySnippetHandlesSymlinkErrors(c *C) {
	...
	snippet, err := s.iface.PlugSecuritySnippet(s.gpioSlot, s.plug, interfaces.SecurityAppArmor)
...
func (s *BoolFileInterfaceSuite) TestPlugSecuritySnippetDereferencesSymlinks(c *C) {
	...
	// The path uses dereferenced symbolic links.
	snippet, err = s.iface.PlugSecuritySnippet(s.ledSlot, s.plug, interfaces.SecurityAppArmor)
	c.Assert(err, IsNil)
	c.Assert(snippet, DeepEquals, []byte(
...
func (s *BoolFileInterfaceSuite) TestPlugSecurityDoesNotContainSlotSecurity(c *C) {
	...
	plugSnippet, err = s.iface.PlugSecuritySnippet(s.gpioSlot, s.plug, interfaces.SecurityAppArmor)
	c.Assert(err, IsNil)
	slotSnippet, err = s.iface.SlotSecuritySnippet(s.gpioSlot, s.plug, interfaces.SecurityAppArmor)
	c.Assert(err, IsNil)
...
func (s *BoolFileInterfaceSuite) TestPlugSecuritySnippetPanicksOnUnsanitizedSlots(c *C) {
	...
	c.Assert(func() {
		s.iface.PlugSecuritySnippet(s.missingPathSlot, s.plug, interfaces.SecurityAppArmor)
...
func (s *BoolFileInterfaceSuite) TestPlugSecuritySnippetUnusedSecuritySystems(c *C) {
	for _, slot := range []*interfaces.Slot{s.ledSlot, s.gpioSlot} {
		...
		snippet, err := s.iface.PlugSecuritySnippet(slot, s.plug, interfaces.SecuritySecComp)
		c.Assert(err, IsNil)
		c.Assert(snippet, IsNil)
		// No extra dbus permissions for plug
		snippet, err = s.iface.PlugSecuritySnippet(slot, s.plug, interfaces.SecurityDBus)
		c.Assert(err, IsNil)
		c.Assert(snippet, IsNil)
		// No extra udev permissions for plug
		snippet, err = s.iface.PlugSecuritySnippet(slot, s.plug, interfaces.SecurityUDev)
		c.Assert(err, IsNil)
		c.Assert(snippet, IsNil)
		// No extra udev permissions for plug
		snippet, err = s.iface.PlugSecuritySnippet(slot, s.plug, interfaces.SecurityUDev)
		...
		snippet, err = s.iface.PlugSecuritySnippet(slot, s.plug, "foo")
...
`)
	snippet, err := s.iface.SlotSecuritySnippet(s.gpioSlot, s.plug, interfaces.SecurityAppArmor)
	c.Assert(err, IsNil)
...
func (s *BoolFileInterfaceSuite) TestSlotSecuritySnippetGivesNoExtraPermissionsToConfigureLEDs(c *C) {
	...
	snippet, err := s.iface.SlotSecuritySnippet(s.ledSlot, s.plug, interfaces.SecurityAppArmor)
	c.Assert(err, IsNil)
	c.Assert(snippet, IsNil)
}
...
func (s *BoolFileInterfaceSuite) TestSlotSecuritySnippetPanicksOnUnsanitizedSlots(c *C) {
	// Unsanitized slots should never be used and cause a panic.
	c.Assert(func() {
		s.iface.SlotSecuritySnippet(s.missingPathSlot, s.plug, interfaces.SecurityAppArmor)
	}, PanicMatches, "slot is not sanitized")
}
...
func (s *BoolFileInterfaceSuite) TestSlotSecuritySnippetUnusedSecuritySystems(c *C) {
	for _, slot := range []*interfaces.Slot{s.ledSlot, s.gpioSlot} {
		...
		snippet, err := s.iface.SlotSecuritySnippet(slot, s.plug, interfaces.SecuritySecComp)
		c.Assert(err, IsNil)
		c.Assert(snippet, IsNil)
		// No extra dbus permissions for slot
		snippet, err = s.iface.SlotSecuritySnippet(slot, s.plug, interfaces.SecurityDBus)
		c.Assert(err, IsNil)
		c.Assert(snippet, IsNil)
		// No extra udev permissions for slot
		snippet, err = s.iface.SlotSecuritySnippet(slot, s.plug, interfaces.SecurityUDev)
		c.Assert(err, IsNil)
		c.Assert(snippet, IsNil)
		// Other security types are not recognized
		snippet, err = s.iface.SlotSecuritySnippet(slot, s.plug, "foo")
		c.Assert(err, ErrorMatches, `unknown security system`)
		c.Assert(snippet, IsNil)
	}
}
