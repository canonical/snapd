...
func (s *WaylandInterfaceSuite) TestSanitizeSlot(c *C) {
	...
	c.Assert(err, ErrorMatches, "wayland slots are reserved for the operating system snap")
}
...
func (s *WaylandInterfaceSuite) TestSanitizeIncorrectInterface(c *C) {
	c.Assert(func() { s.iface.SanitizeSlot(&interfaces.Slot{SlotInfo: &snap.SlotInfo{Interface: "other"}}) },
		PanicMatches, `slot is not of interface "wayland"`)
	c.Assert(func() { s.iface.SanitizePlug(&interfaces.Plug{PlugInfo: &snap.PlugInfo{Interface: "other"}}) },
		PanicMatches, `plug is not of interface "wayland"`)
}
