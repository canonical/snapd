...
func (s *ModemManagerInterfaceSuite) TestUsedSecuritySystems(c *C) {
	...
	err = dbusSpec.AddConnectedPlug(s.iface, interfaces.NewPlugData(plug, nil), interfaces.NewSlotData(s.slot, nil))
	c.Assert(err, IsNil)
	...
	c.Assert(udevSpec.AddPermanentSlot(s.iface, interfaces.NewSlotData(s.slot, nil)), IsNil)
