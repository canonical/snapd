...
func (s *SerialPortInterfaceSuite) TestPermanentSlotUDevSnippets(c *C) {
	...
	for _, slot := range []*interfaces.Slot{s.testSlot1, s.testSlot2, s.testSlot3, s.testSlot4} {
		err := spec.AddPermanentSlot(s.iface, interfaces.NewSlotData(slot, nil))
		c.Assert(err, IsNil)
...
SUBSYSTEM=="tty", SUBSYSTEMS=="usb", ATTRS{idVendor}=="0001", ATTRS{idProduct}=="0001", SYMLINK+="serial-port-zigbee"`
	err := spec.AddPermanentSlot(s.iface, interfaces.NewSlotData(s.testUDev1, nil))
	c.Assert(err, IsNil)
...
SUBSYSTEM=="tty", SUBSYSTEMS=="usb", ATTRS{idVendor}=="ffff", ATTRS{idProduct}=="ffff", SYMLINK+="serial-port-mydevice"`
	err = spec.AddPermanentSlot(s.iface, interfaces.NewSlotData(s.testUDev2, nil))
	c.Assert(err, IsNil)
	c.Assert(spec.Snippets(), HasLen, 1)
	snippet = spec.Snippets()[0]
...
func (s *SerialPortInterfaceSuite) TestConnectedPlugUDevSnippets(c *C) {
	spec := &udev.Specification{}
	err := spec.AddConnectedPlug(s.iface, interfaces.NewPlugData(s.testPlugPort1, nil), interfaces.NewSlotData(s.testSlot1, nil))
	c.Assert(err, IsNil)
	c.Assert(spec.Snippets(), HasLen, 0)

	expectedSnippet1 := `IMPORT{builtin}="usb_id"
SUBSYSTEM=="tty", SUBSYSTEMS=="usb", ATTRS{idVendor}=="0001", ATTRS{idProduct}=="0001", TAG+="snap_client-snap_app-accessing-2-ports"`
	err = spec.AddConnectedPlug(s.iface, interfaces.NewPlugData(s.testPlugPort1, nil), interfaces.NewSlotData(s.testUDev1, nil))
	c.Assert(err, IsNil)
	c.Assert(spec.Snippets(), HasLen, 1)
	snippet := spec.Snippets()[0]
	c.Assert(snippet, Equals, expectedSnippet1)
	...
	spec = &udev.Specification{}
	err = spec.AddConnectedPlug(s.iface, interfaces.NewPlugData(s.testPlugPort2, nil), interfaces.NewSlotData(s.testUDev2, nil))
	c.Assert(err, IsNil)
	c.Assert(spec.Snippets(), HasLen, 1)
	snippet = spec.Snippets()[0]
	expectedSnippet2 := `IMPORT{builtin}="usb_id"
