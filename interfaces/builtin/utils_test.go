...
func (s *utilsSuite) TestSanitizeSlotReservedForOS(c *C) {
	...
	c.Assert(builtin.SanitizeSlotReservedForOS(s.iface, interfaces.NewSlotData(s.slotOS.SlotInfo, nil)), IsNil)
	c.Assert(builtin.SanitizeSlotReservedForOS(s.iface, interfaces.NewSlotData(s.slotApp.SlotInfo, nil)), ErrorMatches, errmsg)
	c.Assert(builtin.SanitizeSlotReservedForOS(s.iface, interfaces.NewSlotData(s.slotGadget.SlotInfo, nil)), ErrorMatches, errmsg)
}

func (s *utilsSuite) TestSanitizeSlotReservedForOSOrGadget(c *C) {
	errmsg := "iface slots are reserved for the core and gadget snaps"
	c.Assert(builtin.SanitizeSlotReservedForOSOrGadget(s.iface, interfaces.NewSlotData(s.slotOS.SlotInfo, nil)), IsNil)
	c.Assert(builtin.SanitizeSlotReservedForOSOrGadget(s.iface, interfaces.NewSlotData(s.slotApp.SlotInfo, nil)), ErrorMatches, errmsg)
	c.Assert(builtin.SanitizeSlotReservedForOSOrGadget(s.iface, interfaces.NewSlotData(s.slotGadget.SlotInfo, nil)), IsNil)
}

func (s *utilsSuite) TestSanitizeSlotReservedForOSOrApp(c *C) {
	errmsg := "iface slots are reserved for the core and app snaps"
	c.Assert(builtin.SanitizeSlotReservedForOSOrApp(s.iface, interfaces.NewSlotData(s.slotOS.SlotInfo, nil)), IsNil)
	c.Assert(builtin.SanitizeSlotReservedForOSOrApp(s.iface, interfaces.NewSlotData(s.slotApp.SlotInfo, nil)), IsNil)
	c.Assert(builtin.SanitizeSlotReservedForOSOrApp(s.iface, interfaces.NewSlotData(s.slotGadget.SlotInfo, nil)), ErrorMatches, errmsg)
}
