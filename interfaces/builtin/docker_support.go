...
const dockerSupportConnectedPlugUdev = `
# This file contains udev rules for Docker Daemon.
#
# Do not edit this file, it will be overwritten on updates

KERNEL=="loop-control", TAG+="%[1]s"
KERNEL=="loop[0-9]*", TAG+="%[1]s"
KERNEL=="device-mapper", SYMLINK+="mapper/control", TAG+="%[1]s"

# When devicemapper is specified as backend storage driver(default is aufs on ubuntu),
# device nodes are created in /dev/mapper/docker-* and linked to /dev/dm-*
# The corresponding apparmor rule is
# /dev/mapper/docker* rw,
#
...
func (iface *dockerSupportInterface) UDevConnectedPlug(spec *udev.Specification, plug *interfaces.Plug, plugAttrs map[string]interface{}, slot *interfaces.Slot, slotAttrs map[string]interface{}) error {
	...
	if privileged {
		for appName := range plug.Apps {
			...
			spec.AddSnippet(fmt.Sprintf(dockerSupportConnectedPlugUdev, tag))
		}
	}
	return nil
}
