...
# connected which means we rules that are compatible to allow both transitioning
# to docker-default profile here AAAAAAND transitioning to any other profile 
# below in the privileged snippet, BUUUUUUUT also need to be triply compatible
# with the injected compatibility snap-confine transition rules to temporarily
# support executing other snaps from devmode snaps. So we are left with writing
# out these extremely verbose regexps because AARE does not have a negative 
# concept to exclude just the paths we want. 
# When we drop support for executing other snaps from devmode snaps this can go
# back to the much simpler rule:
...
# see above, this should just be the below rule, but can't be because we can't
# have nice things
...
# TODO: when we drop support for executing other snaps from devmode snaps, this
# rule can go back to this which is much simpler:
# change_profile unsafe /**,
# but until then we need this set of rules to avoid the conflict with the 
# inherit exec rules below, which themselves are needed to avoid a conflict with
# the generate snap-confine transition rules
...
# When we can stop supporting devmode snaps executing other devmode snaps, then
# this rule can go back to the simpler:
# /** rwlix,
# but until then we need this towering monstrosity:
/[^s]** rwlix,
/s[^n]** rwlix,
/sn[^a]** rwlix,
/sna[^p]** rwlix,
