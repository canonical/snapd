...
func (s *snapAppSetSuite) TestPlugLabelExpr(c *C) {
	...
	set := mylog.Check2(interfaces.NewSnapAppSet(info, nil))
...
slots:
  slot:`
	...
	set := mylog.Check2(interfaces.NewSnapAppSet(info, []*snap.ComponentInfo{compInfo}))


	tags := mylog.Check2(set.SecurityTagsForConnectedPlug(connectedPlug))
...
plugs:
  plug:`
	...
	_ = mylog.Check2(set.SecurityTagsForConnectedPlug(connectedPlug))
...
slots:
  slot:`
	...
	tags := mylog.Check2(set.SecurityTagsForConnectedSlot(connectedSlot))
...
slots:
  slot:`
	...
	_ = mylog.Check2(set.SecurityTagsForConnectedSlot(connectedSlot))
...
func (s *snapAppSetSuite) TestNewAppSetWithWrongComponent(c *C) {
	info := snaptest.MockInfo(c, yaml, nil)
	_ := mylog.Check2(interfaces.NewSnapAppSet(info, []*snap.ComponentInfo{{
