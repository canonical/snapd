...
const eglConsumer = `name: egl-consumer
version: 0
type: app
apps:
  app:
...
func (s *backendSuite) TestConnectDisconnect(c *C) {
	...
	iface := &ifacetest.TestConfigFilesInterface{
		...
		PathPatternsCallback: func() []string {
			return []string{filepath.Join(dirs.GlobalRootDir, "/etc/conf1.d/*.conf")}
		},
	}
	...
	checkConfigfilesFile(c, "/etc/conf1.d/a.conf", "aaaa")
	checkConfigfilesFile(c, "/etc/conf1.d/b.conf", "bbbb")
	...
	c.Check(filepath.Join(dirs.GlobalRootDir, "/etc/conf1.d/a.conf"), testutil.FileAbsent)
	checkConfigfilesFile(c, "/etc/conf1.d/b.conf", "bbbb")
}
...
func (s *backendSuite) TestTwoPlugs(c *C) {
	...
	iface2 := &ifacetest.TestConfigFilesInterface{
		...
		PathPatternsCallback: func() []string {
			return []string{filepath.Join(dirs.GlobalRootDir, "/etc/conf2.d/*.conf")}
		},
	}
	...
	checkConfigfilesFile(c, "/etc/conf1.d/a.conf", "a")
	checkConfigfilesFile(c, "/etc/conf2.d/a.conf", "a")
...
func (s *backendSuite) TestUnmatchedPattern(c *C) {
	...
	c.Assert(s.Backend.Setup(appSet, interfaces.ConfinementOptions{}, s.Repo, nil), ErrorMatches,
		`internal error: .*/etc/conf1.d/a.txt\] not in any registered configfiles pattern`)
